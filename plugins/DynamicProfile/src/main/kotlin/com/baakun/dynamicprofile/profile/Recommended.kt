...
object Recommended {
     ...
     * 看板を並べる
     */
    private fun signs(player: Player, page: Int, inv: Inventory, index: Int, filteredData: MutableList<SLData>){
        for(i in 0..44){
        ...
        for (i in 0..44){
            if(i+(page*45) >= filteredData.size){
                ...
                item.addText("&f>>&a${data.title} &fID:${data.id}", mutableListOf("&3イイね: ${data.likes.size}","&3作成日: ${data.time.toFormat()}"))
                item.guiRun{
