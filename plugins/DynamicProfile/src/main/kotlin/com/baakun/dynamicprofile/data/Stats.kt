...
data class Stats(
    ...
    @Expose
    var recommends: MutableMap<Int, Int> = mutableMapOf(Pair(0,-0),Pair(1,-0),Pair(2,-0),Pair(3,-0),Pair(4,-0)),
    @Expose
    var lastLogin: String = LocalDateTime.now().toString(),
    @Expose
...
)
{
    ...
    fun getLBStats(): LBStats{
        return LBStats(UUID.fromString(uuid),exp,move,vehicle,fly,jump,block, chat, vote, like, receiveLike, join)
    }
    /**
     * カウントのみ、経験値は引かれない
     */
    ...
    fun addCount(type: BehType, multiple: Int){
        ...
        if (onlinePlayer != null && System.currentTimeMillis() - lastCheck > 1000) {
            ...
            if (newLevel > lastLv) {
                ...
                // 通知
                if (this.notice) {
                    Bukkit.getLogger().info("${onlinePlayer.name} さんのレベルが $newLevel に上がりました！, LastAction: $type, Exp: $currentExp -> ${currentExp + expGain}")
                    onlinePlayer.playSound(onlinePlayer.eyeLocation, Sound.ENTITY_PLAYER_LEVELUP, SoundCategory.MASTER, 0.75F, 1F)
                ...
                if (Tools.plugin.config.getIntegerList("noticeLvs").contains(newLevel)) {
                    Bukkit.broadcast(Component.text("[§eLEVEL UP!§f] §a${onlinePlayer.name} §fさんのレベルが §6$newLevel §fに上がりました！"))
                }
            }
     ...
     * 動作をした回数
     */
    fun set(type: BehType, count: Int) = when(type) {
    ...
    fun get(type: BehType): Int = when(type) {
        BehType.MOVE -> move
