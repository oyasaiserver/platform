...
object ProfileUtil {
    ...
    fun setupProfileHead(
      ...
      soundPlayer: Player? = null
        ...
        if (!isSelfProfile) {
            titleLore.add("&a最終オンライン: &7${if (player.isOnline) {"現在オンライン"} else {statsData.getLastOnlineTime()}}")
        }
        if (statsData.title != -1) {
        ...
        playerHeadItem.guiRun {
            ...
            val list: MutableList<String> = if (playerHeadItemStatus) {
                mutableListOf(
                    "&a総プレイ時間: &7${statsData.getPlayTime()}",
                    "&a初ログイン: &7${statsData.getFirstPlayed()}"
                ).apply {
                    if (!isSelfProfile) add("&a最終オンライン: &7${if (player.isOnline) {"現在オンライン"} else {statsData.getLastOnlineTime()}}")
                    if (statsData.title != -1) {
                        val currentTitle = getTitleFromId(statsData.title)
                        if (!currentTitle.description.isNullOrEmpty()) {
                            add("")
                ...
                mutableListOf(
                    ...
                    if (isSelfProfile) "&a投票した回数: &7${statsData.vote}回" else "&a投票数: &7${statsData.vote}回",
