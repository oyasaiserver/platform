...
object OperatorCommand: CommandExecutor {
    ...
    private fun processUserStats(
        ...
        for (i in start until end) {
            ...
            val exp = Calculator.expAmount(BehType.PLAY_TIME) * ((pl.getStatistic(Statistic.PLAY_ONE_MINUTE) / 20) / 60)
            statsData.exp += exp
            ...
            when (args[0]) {
                ...
                "save" -> {
                    ...
                    Bukkit.getScheduler().runTaskAsynchronously(plugin, Runnable {
                        val file = File(plugin.dataFolder.absolutePath + "/UserStatsJSON/${player.uniqueId}.json")
                        try {
                            file.parentFile?.mkdirs()
                            if (!file.exists()) file.createNewFile()
                            FileWriter(file).use { writer ->
                    ...
                    if (args[1] == "kakutei") {
                        Bukkit.getScheduler().runTaskAsynchronously(plugin, Runnable {
                            try {
                                ...
                                for (data in slDatas) {
                                    for (likeuser in data.likes) {
                                        ...
                                        synchronized(userReceiveLikesLock) {
                                            userReceiveLikes[data.owner] = userReceiveLikes.getOrDefault(data.owner, 0) + 1
                                        }
                                ...
                                Bukkit.getScheduler().runTaskAsynchronously(plugin, Runnable {
                                    processUserStats(allUser, 0, half, blocks, userLikes, userReceiveLikes, "Thread 1", player)
                                    plugin.server.logger.info("1ブロック目完了しました。")
                                    player.sendMessage("1ブロック目かんりょうしました。")
                                })
                                Bukkit.getScheduler().runTaskAsynchronously(plugin, Runnable {
                                    processUserStats(allUser, half, alllUserSize, blocks, userLikes, userReceiveLikes, "Thread 2", player)
                ...
                "chest" -> {
                    if (player.getTargetBlock(null, 3).isEmpty || player.getTargetBlock(null, 3).type != Material.CHEST) return false
                    plugin.config.set("RewardChestXYZ", arrayListOf(player.getTargetBlock(null, 3).x, player.getTargetBlock(null, 3).y, player.getTargetBlock(null, 3).z))
                ...
                "beh" -> {
                    when (args[1]) {
                        "Move", "Vehicle", "Fly", "Jump", "Block", "Vote", "Like", "ReceiveLike", "Join" -> {
                    ...
                    when (arg) {
                        "list" -> {
                            ...
                            for (i in 0..<spl) {
                                ...
                                val message = Component.text(" - ${title.id} - ${title.title}")
                                    .hoverEvent(Component.text("&6[称号] ${title.title}")
                                        .appendNewline()
                        ...
                        "give" -> {
                            player.sendMessage(giveTitle(Bukkit.getOfflinePlayer(args[2]).uniqueId, Integer.parseInt(args[3])))
                        }
                        "add" -> {
                        ...
                        "deprive" -> {
                            player.sendMessage(deTitle(Bukkit.getOfflinePlayer(args[2]).uniqueId, Integer.parseInt(args[3])))
                        }
                        "reload" -> {
                            saveTitles()
                            loadTitles()
                ...
                "repair" -> {
                    ...
                    val recoverStats: Stats = gson.fromJson(recoverFile.readText(StandardCharsets.UTF_8), Stats::class.java)
...
object OperatorCommandCompleter: TabCompleter {
    override fun onTabComplete(
        ...
        if (args != null) {
            when (args.size) {
                1 -> return mutableListOf("required", "chest", "beh", "noticeLvs", "receiveStatus", "reloadConfig", "title", "setCount", "repair")
                2 -> {
                    when (args[0]) {
                        "required" -> return mutableListOf("100000", "50000")
                        "beh" -> return mutableListOf("Move", "Vehicle", "Fly", "Jump", "Block", "Vote", "Like", "ReceiveLike", "Join", "Chat", "PlayTime")
                        ...
                        "title" -> return mutableListOf("list", "add", "remove", "give", "deprive", "reload", "edit")
                        "repair",
                    ...
                    when (args[0]) {
                        "title" -> {
                            when (args[1]) {
                                ...
                                "edit",
                                "remove" -> return allTitles.keys.toList().map { i: Int -> i.toString() }.toMutableList()
                                "give",
