...
object TitleUtils {
    ...
    fun loadTitles() {
        ...
        allTitles = gson.fromJson(titleFile.readText(), object : TypeToken<MutableMap<Int, Title>>() {}.type)
    }

    fun saveTitles() {
        titleFile.parentFile.mkdirs()
        titleFile.createNewFile()
        FileWriter(titleFile, StandardCharsets.UTF_8).use { writer ->
            gson.toJson(allTitles, writer)
        }
    }

    fun createNewTitle(title: String, rarity: Int): String {
        val newId = (0..Int.MAX_VALUE).firstOrNull { it !in allTitles.keys }
    ...
    fun getTitles(uuid: UUID): MutableList<Int> {
        return allTitles.values.stream().filter { it: Title -> it.owners.contains(uuid) }.map { it.id }.collect(Collectors.toList())
    }
}