...
enum class LBType{LEVEL,LIKE,RECEIVE_LIKE,CHAT,MOVE,PLAYTIME}
...
object LeaderBoardCommand: CommandExecutor {
    override fun onCommand(p0: CommandSender, p1: Command, p2: String, p3: Array<out String>?): Boolean {
        val hi: MutableList<UUID> = if (p1.name.equals("dpleaderboard")) allUser else UUIDMap.keys.toMutableList()
        fun getSenderStatsRef(uuid: UUID) = if(p1.name.equals("dpleaderboard")) getStats(uuid).getLBStats() else getWeeklyLB(uuid)
        try {
            ...
            thread {
                ...
                val sorted = hi.sortedByDescending { when(type) {
                ...
                if(p0 is Player){
                    ...
                    p0.sendMessage("${sorted.indexOf(p0.uniqueId)+1}位 - ${
                        when(type) {
                            RECEIVE_LIKE->"${data.receiveLike}回"
                            LIKE->"${data.like}回"
                            CHAT->"${data.chat}回"
