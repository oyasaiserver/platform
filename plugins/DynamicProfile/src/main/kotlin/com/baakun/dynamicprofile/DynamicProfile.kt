...
class DynamicProfile: JavaPlugin() {
    ...
    override fun onEnable() {
        ...
        totalPlayTimes.right.addAll(allUser.sortedByDescending { Bukkit.getOfflinePlayer(it).getStatistic(Statistic.PLAY_ONE_MINUTE) })
        saveDefaultConfig()
        ...
        Bukkit.getScheduler().runTaskAsynchronously(this, Runnable {
            ...
            try {
                ...
                if (userFiles.isNotEmpty()) {
                    ...
                    userFiles.forEach { file ->
                        ...
                        val uuid = fileName.substringBeforeLast('.', "").let {
                        ...
                        allUser.add(uuid)
                        try {
                            val stats: Stats = gson.fromJson(file.readText(StandardCharsets.UTF_8), Stats::class.java)
                            allStats[uuid] = stats
                        } catch (je: Exception) {
            ...
            for (player in Bukkit.getOnlinePlayers()){
                ...
                GsonBuilder().excludeFieldsWithoutExposeAnnotation().setPrettyPrinting().create().toJson(getStats(player.uniqueId), writer)
        ...
        for (player in Bukkit.getOnlinePlayers()) {
            try {
                ...
                FileWriter(file, StandardCharsets.UTF_8).use { writer ->
                    GsonBuilder().excludeFieldsWithoutExposeAnnotation().setPrettyPrinting().create().toJson(userstats, writer)
                }
            } catch (e: Exception) {
