...
object LevelReward {
    ...
    fun display(player: Player){
        val world = plugin.config.getString("RewardChestWorld","lifeworld")?.let { Bukkit.getWorld(it) }
        ...
        if(loc.block.type == Material.CHEST){
                ...
                if(Calculator.getLevel(player) >= lv&&!statsData.getReceiveRewardStatus(lv)){
                    when(item.type){
                        Material.IRON_INGOT->{
                            ...
                            text.add(econ?.format(item.itemMeta.displayName.split(",").get(1).toDouble()).toString())
                        ...
                        Material.GOLD_INGOT->{
                            ...
                            reward.runnables.add{
                                token?.addTokens(
                                    player,
                                    item.itemMeta.displayName.split(",").get(1).toLong()
                                )
                            }
                        }
                        Material.COAL->{
                            ...
                            reward.runnables.add{
                                Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), "milepoint add ${item.itemMeta.displayName.split(",").get(1)}")
                            }
                        }
            ...
            for (reward in rewards.values){
                ...
                displayItem.addText("&a[Lv.${reward.lv}] &fレベル報酬", reward.lores).
                    guiRun{
                        if(player.gameMode == GameMode.CREATIVE||player.gameMode == GameMode.SPECTATOR){
                            player.playSound(player.eyeLocation, Sound.ENTITY_ENDERMAN_TELEPORT, 0.5F, 1F)
                            player.closeInventory()
                            val message = Component.text("クリエイティブ状態だと受け取れません。（クリックしてサバイバルに変更）")
