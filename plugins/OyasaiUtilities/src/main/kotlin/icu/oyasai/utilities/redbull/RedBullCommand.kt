...
object RedBullCommand : CommandExecutor, TabCompleter {
  override fun onCommand(
    ...
    when (command.name.lowercase()) {
      ...
      "buyredbull", "buyredbullset" -> {
        val targetName = args.singleOrNull() ?: run {
  ...
  override fun onTabComplete(
    ...
    return Bukkit.getOnlinePlayers().map(Player::getName).filter { it.startsWith(args[0], ignoreCase = true) }
  }
}
