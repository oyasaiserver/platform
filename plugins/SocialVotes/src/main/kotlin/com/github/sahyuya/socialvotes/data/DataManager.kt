...
class DataManager(private val plugin: Plugin) {
  ...
  fun load() {
    ...
    sSec?.getKeys(false)?.forEach { idStr ->
      ...
      if (world != null) {
        val loc = Location(world, sign.x.toDouble(), sign.y.toDouble(), sign.z.toDouble())
        locationToId[loc] = id
      }
    }
    ...
    pvSec?.getKeys(false)?.forEach { group ->
      ...
      sub.getKeys(false).forEach { uuidStr -> map[UUID.fromString(uuidStr)] = sub.getInt(uuidStr) }
  ...
  fun save() {
    ...
    playerVotes.forEach { (group, map) ->
      ...
      map.forEach { (uuid, count) -> yaml.set("$p.$uuid", count) }
    }
  ...
  fun writeSignIdToBlock(sign: Sign, id: Int) {
    sign.persistentDataContainer.set(SocialVotes.SV_SIGN_ID_KEY, PersistentDataType.INTEGER, id)
    sign.update(true)
  }

  fun readSignIdFromBlock(sign: Sign): Int? {
    return sign.persistentDataContainer.get(SocialVotes.SV_SIGN_ID_KEY, PersistentDataType.INTEGER)
  }
