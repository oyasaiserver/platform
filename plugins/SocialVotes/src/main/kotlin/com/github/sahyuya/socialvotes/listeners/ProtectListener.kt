...
class ProtectListener : Listener {
  ...
  private fun getAttachedBlock(
    ...
    return when (data) {
      ...
      // 床置き看板
      // ────────────────
      is org.bukkit.block.data.type.Sign -> signBlock.getRelative(0, -1, 0)

      // ────────────────
      // 壁看板
      // ────────────────
      is WallSign -> signBlock.getRelative(data.facing.oppositeFace)

      // ────────────────
      // 吊り看板（天井）
      // ────────────────
      is HangingSign -> signBlock.getRelative(0, 1, 0)

      // ────────────────
      // 壁吊り看板
      // ────────────────
      is WallHangingSign -> null
