...
object VoterListGUI {
  ...
  private fun head(player: OfflinePlayer, count: Int): ItemStack {
    ...
    meta.lore = listOf("§7${player.name ?: "unknown"} × §e$count")
    it.itemMeta = meta
    return it
  }
  ...
  fun open(p: Player, sign: SVSign, page: Int = 0) {
    ...
    // ▼ 0票を除外した投票者マップ
    val rawMap = dm.playerVotesPerSign[sign.id]?.filterValues { it > 0 } ?: emptyMap()
  ...
  fun onClick(p: Player, slot: Int) {
    ...
    when (slot) {
      ...
      50 -> {
        val dm = SocialVotes.dataManager
        val size = dm.playerVotesPerSign[sign.id]?.values?.count { it > 0 } ?: 0
