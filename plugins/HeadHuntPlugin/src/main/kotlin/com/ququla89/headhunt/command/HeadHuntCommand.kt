...
class HeadHuntCommand(
    ...
    private fun handleStart(sender: CommandSender, args: Array<out String>) {
        if (!requireAdmin(sender)) return
        val mode = when (args.getOrNull(1)?.lowercase()) {
            "solo" -> GameMode.SOLO
            "team" -> GameMode.TEAM
            else -> {
        ...
        val teamMode = if (mode == GameMode.TEAM) {
        ...
        val modeLabel = if (mode == GameMode.SOLO) {
            "個人戦"
        } else {
            "チーム戦 (" + (if (teamMode == TeamMode.SHARED) "共有" else "個人発見") + ")"
        }
        val message = PREFIX + "HeadHuntイベントを開始しました！ [" + modeLabel + "] 対象: " + treasureManager.size + "件"
    ...
    private fun handleList(sender: CommandSender) {
        ...
        for (treasure in treasures) {
            val line = "- " + treasure.id + " : " + treasure.worldName +
    ...
    private fun handleDeleteAll(sender: CommandSender, args: Array<out String>) {
        ...
        sender.sendMessage(PREFIX + "全ての宝HEAD情報を削除します。30秒以内に /headhunt deleteall confirm を実行して確定してください。")
    }
    ...
    private fun handleTeam(sender: CommandSender, args: Array<out String>) {
        when (args.getOrNull(1)?.lowercase()) {
            ...
            else -> sender.sendMessage(PREFIX + "使い方: /headhunt team <create|join|leave|delete|list|info>")
        }
    }
    ...
    private fun handleTeamList(sender: CommandSender) {
        ...
        for (team in teams) {
            val memberNames = team.members.joinToString(", ") { Bukkit.getOfflinePlayer(it).name ?: it.toString() }
    ...
    override fun suggest(commandSourceStack: CommandSourceStack, args: Array<String>): Collection<String> {
        return when (args.size) {
            1 -> listOf("setmode", "start", "reset", "stop", "list", "deleteall", "team")
            ...
            4 -> if (args[0].equals("team", true) && args[1].equals("join", true)) {
