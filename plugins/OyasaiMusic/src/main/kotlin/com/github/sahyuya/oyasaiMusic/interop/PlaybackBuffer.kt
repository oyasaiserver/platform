...
/**
 * Pure OYPB preparation. V1 keeps compatibility with older clients, while V2 carries the fixed
 * custom-sound pattern, cent pitch, pan, and optional resource-pack bank policy needed for
 * deterministic client-side playback.
 */
object PlaybackBuffer {
  ...
  fun prepareV2(
      ...
      try {
        require(
            notes.size in 1..100_000 &&
                spatialMode in 0..1 &&
                bankPolicy in 0..1 &&
                manifestHash.size == 32
        )
        require(
            (bankPolicy == 0 && manifestHash.all { it == 0.toByte() }) ||
                (bankPolicy == 1 && manifestHash.any { it != 0.toByte() })
        )
        val sorted = notes.sortedBy { it.timeMs }
        val canonicalCustom =
            sorted.mapNotNull { note -> note.customSound?.let(::canonicalEventId) }
        val strings =
            canonicalCustom.distinct().sortedWith(Comparator { a, b -> compareUtf8(a, b) })
        require(
            strings.size <= 4096 &&
                strings.sumOf { it.toByteArray(Charsets.UTF_8).size } <= 1_048_576
        )
        ...
        val raw =
            ByteArrayOutputStream().use { bytes ->
              DataOutputStream(bytes).use { out ->
                ...
                out.writeByte(2)
                val duration = max(0, sorted.lastOrNull()?.timeMs ?: 0)
                varUInt(out, duration)
                varUInt(out, sorted.size)
                varUInt(out, strings.size)
                out.writeByte(spatialMode)
                out.writeByte(bankPolicy)
                out.write(manifestHash)
                strings.forEach { value ->
                  val utf = value.toByteArray(Charsets.UTF_8)
                  require(utf.size in 1..256 && value.matches(Regex("[a-z0-9_.-]+:[a-z0-9/._-]+")))
                  varUInt(out, utf.size)
                  out.write(utf)
                }
                var previous = 0
                sorted.forEach { note ->
                  require(
                      note.timeMs >= previous &&
                          note.pitchCents in -5400..7300 &&
                          note.volume in 0..100 &&
                          note.pan in -100..100 &&
                          note.instrument in 0..255
                  )
                  ...
                  note.customSound?.let {
                    ...
                    val pattern = customPattern(note) ?: return null
                    varUInt(out, index.getValue(event))
                    varUInt(out, pattern)
        ...
        val chunks = compressed.asList().chunked(CHUNK_BYTES).map { it.toByteArray() }
        if (chunks.size !in 1..MAX_CHUNKS) return null
  ...
  fun prepare(notes: List<NoteEvent>): Prepared? =
      try {
        ...
        if (
            notes.size !in 1..100_000 ||
                notes.any {
                  it.customSound != null || it.pitchCents !in 0..2400 || it.pitchCents % 100 != 0
                }
        )
            return null
        val sorted = notes.sortedBy { it.timeMs }
  ...
  fun serverCapabilities(minecraftVersion: String): Int {
    val match = Regex("(?<!\\d)(\\d+)\\.(\\d+)(?:\\.(\\d+))?").find(minecraftVersion) ?: return 0
    val major = match.groupValues[1].toIntOrNull() ?: return 0
    val minor = match.groupValues[2].toIntOrNull() ?: return 0
    return if (major == 26 && minor == 2)
        CAP_BRASS_NOTE_BLOCK or CAP_OYPB_V2 or CAP_BANK_MANIFEST_V1 or CAP_SOUND_CATALOG_26_2
    else 0
  }
  ...
  fun bankConsentEnvelope(allowed: Boolean, manifestHash: ByteArray): ByteArray {
    ...
    require(
        (!allowed && manifestHash.all { it == 0.toByte() }) ||
            (allowed && manifestHash.any { it != 0.toByte() })
    )
