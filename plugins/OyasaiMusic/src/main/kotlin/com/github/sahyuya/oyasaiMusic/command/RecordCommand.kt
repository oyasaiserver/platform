 ...
 *
 * /record we grid <BPM> グリッド型録音 /record we default 回路型(レッドストーン)録音 /record start <1-4> 動的録音の開始
...
class RecordCommand(
  ...
  /**
   * 動的録音は `/record start` を実行してから最初の音を鳴らすまでに間が空くことが多いため、 最初に記録された音符のタイミングを0msとして全体をシフトし、先頭の無音を取り除く。
