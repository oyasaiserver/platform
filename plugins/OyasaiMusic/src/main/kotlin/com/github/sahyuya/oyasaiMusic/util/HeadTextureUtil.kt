...
/**
 * UI/UX設計書全体で使われる「モブヘッドのプレースホルダー」を 実際のプレイヤースキンヘッドに置換するためのユーティリティ。
 *
...
object HeadTextureUtil {
  ...
  /**
   * テクスチャ（スキン）を確実に反映させたい場合に使う非同期版。 まずキャッシュ状態のヘッドを即時 [onReady] へ渡し、Mojang APIから完全なプロフィールが
   * 取得でき次第、テクスチャ入りのヘッドで再度 [onReady] を呼び出す（呼び出し側は複数回 呼ばれても安全なようにInventoryへの再setItemだけを行うこと）。
   *
