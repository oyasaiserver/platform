...
/**
 * Never link Cumulus types through Velocity's cross-plugin class lookup. Geyser (and potentially
 * Floodgate) may each expose their own Cumulus copy.
 */
internal object GeyserConsentForm {
  ...
  internal fun build(loader: ClassLoader, content: String, finish: (Boolean?) -> Unit): Any {
    ...
    builderType
        .getMethod("validResultHandler", Consumer::class.java)
        .invoke(
            builder,
            Consumer<Any> { response ->
              finish(!(responseType.getMethod("clickedFirst").invoke(response) as Boolean))
            },
        )
    builderType
        .getMethod("closedOrInvalidResultHandler", Runnable::class.java)
