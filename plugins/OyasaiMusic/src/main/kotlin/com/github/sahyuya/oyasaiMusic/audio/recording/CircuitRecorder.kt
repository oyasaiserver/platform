 ...
 *
 * 旧実装は走査中のブロックの近傍だけをその場で辿っていたため、リピーターの出力先が 次のリピーターの入力点と一致しない構造、段差配線、コピー範囲外にある電源で経路を失っていた。
 * この実装は先に全ブロックを収集し、各リピーターを「背面入力 → 正面出力（delay付き）」という 有向エッジとして索引化してから、到達時刻の早い順に信号を伝播する。
 */
object CircuitRecorder {
  ...
  fun record(clipboard: Clipboard, world: World): List<NoteEvent> {
    ...
    fun recordNote(pos: BlockVector3, data: BukkitNoteBlock, timeMs: Int) {
      ...
      val signDelay =
          SignOverrideProcessor.extractDelayFromWorldPos(world, pos, QUARTER_NOTE_MS) ?: 0
    ...
    while (queue.isNotEmpty()) {
      ...
      for ((dx, dy, dz) in WIRE_OFFSETS) {
        val neighborPos = signal.pos.add(dx, dy, dz)
        if (blocks[neighborPos]?.material == Material.REDSTONE_WIRE)
            enqueue(neighborPos, signal.timeMs)
      }

      // この座標がいずれかのリピーター背面入力なら、正面へ遅延付きで出力する。
      repeaterInputs[signal.pos]?.forEach { edge ->
        enqueue(edge.output, signal.timeMs + edge.delayMs)
      }
    }
  ...
  /**
   * リピーターの facing 情報だけでは連結できなかったschematic向けの救済解析。 回路部品（ノートブロック/リピーター/ダスト）だけを6方向で接続し、リピーターを
   * 通過する度にそのdelayを加算する。石などの土台は伝播対象にしないため、長い床を 一斉通電させることはない。
   */
  private fun recordByPhysicalTopology(
    ...
    fun isComponent(data: BlockData?): Boolean =
        data is BukkitNoteBlock || data is Repeater || data?.material == Material.REDSTONE_WIRE
    fun enqueue(pos: BlockVector3, timeMs: Int) {
    ...
    while (queue.isNotEmpty()) {
      ...
      if (data is BukkitNoteBlock && firedNotes.add(signal.pos)) {
        var volume = 100
        var pan = 0
        val (overrideVolume, overridePan) =
            SignOverrideProcessor.extractFromWorldPos(world, signal.pos)
        overrideVolume?.let { volume = it }
        overridePan?.let { pan = it }
        val delay =
            SignOverrideProcessor.extractDelayFromWorldPos(world, signal.pos, QUARTER_NOTE_MS) ?: 0
