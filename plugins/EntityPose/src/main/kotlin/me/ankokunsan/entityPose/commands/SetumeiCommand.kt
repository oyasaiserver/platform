...
class SetumeiCommand : CommandExecutor {
  override fun onCommand(
    ...
    player.sendMessage("/epscale (0.3~3.0) エンティティの大きさ変更(/epinfo setのGUIからも可能)")
    player.sendMessage("/epinfo setと/epscale エンティティを範囲選択(Entity Copy Wandで)をすると同時に設定できる")
    ...
    player.sendMessage("/epwand -c 範囲選択全解除 pos1,pos2も消える")
    player.sendMessage("/epwand -cはオフハンド持ち替えでも")
    player.sendMessage("Entity Copy Wand WorldEditのエンティティを選択するver(当たり判定難しめ)")
    player.sendMessage("/epcopy 範囲選択したエンティティをコピー")
    player.sendMessage("/eppaste コピーしたエンティティをペースト")
    player.sendMessage("/epcut 範囲選択したエンティティを削除")
