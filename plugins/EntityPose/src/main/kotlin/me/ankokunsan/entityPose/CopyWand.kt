...
object CopyWand {
  fun create1(): ItemStack {
    ...
    // Lore
    meta.lore = listOf("${ChatColor.GRAY}エンティティを左クリック→pos1", "${ChatColor.GRAY}エンティティを右クリック→pos2")

    meta.addEnchant(Enchantment.DENSITY, 1, true)
    meta.addItemFlags(ItemFlag.HIDE_ENCHANTS)

    meta.persistentDataContainer.set(COPY_STICK_KEY, PersistentDataType.BYTE, 1.toByte())
    item.itemMeta = meta
    return item
  }
}
