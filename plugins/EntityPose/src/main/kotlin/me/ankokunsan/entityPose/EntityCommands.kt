...
class EntityCommands : CommandExecutor , TabCompleter {
    override fun onCommand(
        ...
        if (args.isEmpty()) {
            ...
            player.sendMessage("/ep kakudoはｵﾌﾊﾝﾄ持ち替えﾞ、+しゃがみで/ep zahyoのGUIを開ける")
        ...
        when (args[0].lowercase()) {
            "stick", "st" -> boucommand(player)
            "wand", "wa" -> entityWand(player)
            "info", "in" -> entityinfo(player)
            "infoset","ins" -> entityinfoset(player)
            "scale", "sc" -> entityScale(player, args)
            "kakudo", "ka" -> kakudoCommand(player, args)
            "zahyo", "za" -> zahyoCommand(player, args)
            "copy", "co" -> return EntityCopy().onCommand(sender, command, label, args)
            "paste", "pa" -> return EntityPaste().onCommand(sender, command, label, args)
            "cut", "cu" -> entityCut(player)
            "clear", "cl" -> entityclear(player)
            "hojo", "ho" -> entityHojo(player, args)
