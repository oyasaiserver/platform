...
class GUIClick : Listener {
    ...
    fun onSettingClick(event: InventoryClickEvent) {
        ...
        val result = player.world.rayTraceEntities(player.eyeLocation, player.location.direction, 3.0, 0.5) { it != player }
    ...
    fun onALLSettingClick(event: InventoryClickEvent) {
        ...
        val result =
            player.world.rayTraceEntities(player.eyeLocation, player.location.direction, 3.0, 0.5) { it != player }
        ...
        if (selected != null && selected.contains(target)) {
            ...
            when (slot) {
                0 -> {
                  val allInvincible = targets.all { it.persistentDataContainer.has(EntityPose.INVINCIBLE,
                    PersistentDataType.BYTE) }
                  targets.forEach {
                    if (allInvincible) it.persistentDataContainer.remove(EntityPose.INVINCIBLE)
                    else it.persistentDataContainer.set(EntityPose.INVINCIBLE, PersistentDataType.BYTE, 1.toByte())
                  }
                }
                6 -> {
                    targets.forEach { entity ->
                        (entity as? LivingEntity)?.getAttribute(Attribute.SCALE)?.let {
                            attribute -> val newScale = (attribute.baseValue + 0.1).coerceAtMost(3.0)
                            val roundedScale = round(newScale * 10) / 10.0
                            attribute.baseValue = roundedScale
                        }
                    }
                ...
                7 -> {
                    targets.forEach { entity ->
                        (entity as? LivingEntity)?.getAttribute(Attribute.SCALE)?.let {
                                attribute -> val newScale = (attribute.baseValue - 0.1).coerceAtLeast(0.3)
                            val roundedScale = round(newScale * 10) / 10.0
                            attribute.baseValue = roundedScale
                        }
                    }
                ...
                8 -> {
                  val allarrange = targets.all { it.persistentDataContainer.has(EntityPose.ARRANGELOCK,
                    PersistentDataType.BYTE) }
                  targets.forEach {
                    if (allarrange) it.persistentDataContainer.remove(EntityPose.ARRANGELOCK)
                    else it.persistentDataContainer.set(EntityPose.ARRANGELOCK, PersistentDataType.BYTE, 1.toByte())
                    }
                }
            }
            player.playSound(player.location, Sound.UI_BUTTON_CLICK, 1.0f, 1.5f)
    ...
    fun onwolfClick(event: InventoryClickEvent) {
        ...
        FollowEntity.start<Wolf>(player, EntityType.WOLF) { wolf ->
            ...
            Registry.WOLF_VARIANT.get(variantKey)?.let {
                wolf.variant = it
            }
            if (isMini) {
                wolf.setBaby()
    ...
    fun oncatClick(event: InventoryClickEvent) {
        ...
        FollowEntity.start<Cat>(player, EntityType.CAT) { cat ->
            val variantKey = NamespacedKey.minecraft(variantName)
            Registry.CAT_VARIANT.get(variantKey)?.let {
                cat.catType = it
            }
            if (isMini) {
                cat.setBaby()
    ...
    fun onparrotClick(event: InventoryClickEvent) {
        ...
        FollowEntity.start<Parrot>(player, EntityType.PARROT) { parrot->
                parrot.variant = parrotType
        }
        player.closeInventory()
    }
    @EventHandler
