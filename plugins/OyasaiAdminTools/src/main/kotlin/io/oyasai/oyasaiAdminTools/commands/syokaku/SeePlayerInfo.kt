...
object SeePlayerInfo : CommandExecutor {
    override fun onCommand(
        ...
        if (args.size == 1) {
            ...
            val lastPromoDate =
                if (lastPromo != null) DateTimeUtils.parseToJST(lastPromo.date) else DateTimeUtils.getCurrentJST()
            ...
            if (args[1].toIntOrNull() != null){
                ...
                if (history.records.isEmpty()) {
                    ...
                    val message = Component.text("===============${page+1}/${history.records.size}=============").appendNewline()
                        .append(Component.text(" - 日付: ${record.date}")).appendNewline()
                        .append(Component.text(" - 種類: ${record.type}")).appendNewline()
                        .append(Component.text(" - ランク: ${record.previousRank} -> ${record.newRank}")).appendNewline()
                        .append(Component.text(" - 備考: ${record.note}")).appendNewline()
                        .append(Component.text(" - 昇格者: ${Bukkit.getOfflinePlayer(UUID.fromString(record.promotedBy)).name}")).appendNewline()
                        .append(Component.text(" - 特例昇格: ${if (record.isForced) "はい" else "いいえ"}")).appendNewline()
                        .append(Component.text(" - 最後の建築ID: ${record.lastBuildID}").clickEvent(ClickEvent.runCommand("sltp ${record.lastBuildID}"))).appendNewline()
                        .append(Component.text(" - 建築数: ${record.builds}")).appendNewline()
                        .append(Component.text(" - 昇格時点のレベル: ${record.lastLv}")).appendNewline()
                        .append(Component.text(" - 昇格時点の経験値: ${record.lastExp}")).appendNewline()
                        .append(Component.text("<<================").hoverEvent(HoverEvent.showText(Component.text("クリックして前のページ"))).clickEvent(ClickEvent.runCommand("syokaku seeplayerinfo ${playerName} ${page}")))
//                        .append(Component.text("<<<<<<<<<<<<<<<").clickEvent(ClickEvent.runCommand("syokaku ${playerName} ${if(page+1<history.records.size) page+1 else page}")))//ページを戻る
                        .append(Component.text("================>>").hoverEvent(HoverEvent.showText(Component.text("クリックして次のページ"))).clickEvent(ClickEvent.runCommand("syokaku seeplayerinfo ${playerName} ${page+2}")))
                ...
                history.records.sortedByDescending { it.date }
                    .forEach { (type, newRank, previousRank, promotedBy, isForced, date, note, lastBuildID, builds, lastLv, lastExp) ->
                        ...
                        lore.add(Component.text("§e昇格者: §f${Bukkit.getOfflinePlayer(UUID.fromString(promotedBy)).name}"))
