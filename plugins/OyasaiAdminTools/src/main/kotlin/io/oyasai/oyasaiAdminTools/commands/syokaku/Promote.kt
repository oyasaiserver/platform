...
object Promote: CommandExecutor {
    override fun onCommand(
        ...
        val senderID: String = if (sender is Player) {
                        ...
                        RankManager.isCandidate(player).thenAcceptAsync { isCandidateRank ->
                            Bukkit.getScheduler().runTask(plugin, Runnable{
                                ...
                                userManager.loadUser(player.uniqueId).thenAccept { user ->
                                    Bukkit.getScheduler().runTask(plugin, Runnable{
                                        val nextGroupNode = PermsUtils.getGroupNode(nextRank.groupName)
                                        ...
                                        temp.records.add(
                                            PromotionRecord(
                                                ...
                                                note = note,
                                                lastBuildID = Data.getSLDataAll().filter { it.owner == player.uniqueId }
                                                    .maxByOrNull { it.time }?.id ?: Integer.MIN_VALUE,
                                        ...
                                        if(plugin!=null && plugin.isEnabled){
                                            ...
                                            statsData.promotions.records.forEach { plugin.logger.info(it.date) }
                                            statsData.move += 100
                                        }
                                        JsonUtils.saveUserJson(player.uniqueId)
                                        sender.sendMessage("§a${player.name}さんを§e${nextRank.name}§aに昇格させました。")
                                    })
                                }
                            })
