...
object SurveyExporter {
  ...
  fun exportResultsToDiscord(player: Player, surveyId: String) {
    val survey =
        SurveyManager.surveys.find { it.id == surveyId }
            ?: run {
              ...
              try {
                ...
                val rows =
                    results.map { result ->
                      ...
                      row.addAll(
                          survey.questions.indices.map { i -> result.answers.getOrNull(i) ?: "" }
                      )
                      row
                    }
                ...
                plugin.logger.warning(
                    "Failed to export results for survey ${surveyId}: ${e.message}"
                )
                e.printStackTrace()
              }
            },
        ...
        .runTaskAsynchronously(
            ...
            Runnable {
              try {
                ...
                plugin.logger.warning(
                    "Failed to send discord notification for survey ${survey.id}: ${e.message}"
                )
              }
            },
        )
