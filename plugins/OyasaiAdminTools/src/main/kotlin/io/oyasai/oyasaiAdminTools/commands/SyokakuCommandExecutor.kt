...
object SyokakuCommandExecutor : CommandExecutor, TabCompleter {
    private val subCommands = listOf(
        "promote", "demote", "getrank", "iscandidate", "seerequirements", "seeplayerinfo"
    )
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        return when (args[0].lowercase()) {
            ...
            "seerequirements" -> SeeRequirements.onCommand(sender, command, label, args.drop(1).toTypedArray())
            "seeplayerinfo" -> SeePlayerInfo.onCommand(sender, command, label, args.drop(1).toTypedArray())
            else -> {
    ...
    override fun onTabComplete(
        ...
        return when {
            ...
            args.size == 2 -> when (args[0].lowercase()) {
                "promote", "demote", "getrank", "iscandidate", "seeplayerinfo" -> Bukkit.getOnlinePlayers().map { it.name }.filter { it.startsWith(args[1], ignoreCase = true) }
                "seerequirements" -> RankManager.ranks.map { it.name }.filter { it.startsWith(args[1], ignoreCase = true) }
                else -> emptyList()
            }
