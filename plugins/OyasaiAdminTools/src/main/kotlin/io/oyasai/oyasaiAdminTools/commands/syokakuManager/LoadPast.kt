                    ...
                    Bukkit.getScheduler().runTask(plugin, Runnable{
                        log.forEach { action ->
                            ...
                            if(_args[0] != "parent" || _args[1] != "add") return@forEach
                                ...
                                val buildsBeforePromote = Data.getSLDataAll().filter { it.owner == uuid && it.time.isBefore(time.toLocalDateTime())}
                                val lastBuild = buildsBeforePromote.maxByOrNull { it.time }
                                val buildCount = buildsBeforePromote.size

                                statsData.promotions.records.addFirst(PromotionRecord(
