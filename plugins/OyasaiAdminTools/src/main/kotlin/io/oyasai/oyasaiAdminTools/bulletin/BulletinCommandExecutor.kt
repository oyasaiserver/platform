...
object BulletinCommandExecutor : CommandExecutor, TabCompleter {
  ...
  override fun onCommand(
    ...
    when (subCommand) {
      ...
      "announcement",
      "notice",
      "a" -> AnnouncementSubCommand.handle(sender, subArgs)
      "survey",
      "s",
      "anke" -> SurveySubCommand.handle(sender, subArgs)
      "reload" -> {
    ...
    if (args.size >= 2) {
      ...
      when (sub) {
        ...
        "anke" -> {
          ...
          if (
              args.size == 3 && args[1].equals("send", true) && sender.hasPermission("oyasai.admin")
          ) {
