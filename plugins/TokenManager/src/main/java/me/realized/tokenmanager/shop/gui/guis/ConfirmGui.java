...
public class ConfirmGui extends BaseGui {
  ...
  public ConfirmGui(final TokenManagerPlugin plugin, final Shop shop, final int slot) {
    super(
        plugin,
        shop,
        InventoryUtil.deepCopyOf(
            plugin.getShopConfig().getConfirmGuiSample(),
            plugin.getConfiguration().getConfirmPurchaseTitle()));
  ...
  public void refresh(final Player player, final boolean firstLoad) {
    ...
    inventory.setItem(
        ...
        replace(player, inventory.getItem(CONFIRM_PURCHASE_SLOT), balance, cost));
    inventory.setItem(
        ITEM_SLOT, replace(player, shop.getSlot(this.slot).getDisplayed().clone(), balance, cost));
    inventory.setItem(
        CANCEL_PURCHASE_SLOT,
        replace(player, inventory.getItem(CANCEL_PURCHASE_SLOT), balance, cost));
  }

  private ItemStack replace(
      final Player player, final ItemStack item, final long balance, final int price) {
