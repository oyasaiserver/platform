  ...
  public VaultHook(final TokenManagerPlugin plugin) {
    ...
    if (plugin.getConfiguration().isRegisterEconomy()) {
      Bukkit.getServicesManager()
          .register(Economy.class, new TokenManagerEconomy(), plugin, ServicePriority.Highest);
    }
  }
  ...
  private class TokenManagerEconomy implements Economy {
    ...
    public double getBalance(OfflinePlayer player) {
      return (player != null && player.getPlayer() != null)
          ? plugin.getTokens(player.getPlayer()).orElse(0)
          : 0;
    }
    ...
    public EconomyResponse withdrawPlayer(OfflinePlayer player, double amount) {
      if (player == null) {
        return new EconomyResponse(
            0, 0, EconomyResponse.ResponseType.FAILURE, "Player is not online");
      }
      ...
      if (balance < amount) {
        return new EconomyResponse(
            amount, balance, EconomyResponse.ResponseType.FAILURE, "Not enough tokens");
      }
