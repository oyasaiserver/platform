...
public class Lang extends AbstractConfiguration<TokenManagerPlugin> implements Reloadable {
  ...
  protected void loadValues(FileConfiguration configuration) throws Exception {
    ...
    for (String key : configuration.getKeys(true)) {
      ...
      // Fixes a weird occurrence with FileConfiguration#getKeys that an extra separator char is
      // prepended when called after FileConfiguration#set
      if (key.startsWith(".")) {
        key = key.substring(1);
      }
      ...
      final String message =
          value instanceof List ? StringUtil.fromList((List<?>) value) : value.toString();
