...
public class ShopGui extends BaseGui {
  ...
  public boolean handle(final Player player, final int slot) {
    ...
    if (!player.isOp()
        && !player.hasPermission(Permissions.CMD_ADMIN)
        && player.hasPermission(Permissions.SHOP_SLOT_CANCEL + slotInfo)) {
      plugin.doSync(player::closeInventory);
      lang.sendMessage(player, true, "ERROR.on-slot-cancel");
      return false;
    }

    if (data.isUsePermission()
        && !(player.hasPermission(Permissions.SHOP + slotInfo)
            || player.hasPermission(Permissions.SHOP_SLOT_OLD + slotInfo))) {
      plugin.doSync(player::closeInventory);
      lang.sendMessage(
          player, true, "ERROR.no-permission", "permission", Permissions.SHOP + slotInfo);
      return false;
    }

    if (data.getEmptySlotsRequired() > 0
        && InventoryUtil.getEmptySlots(player.getInventory()) < data.getEmptySlotsRequired()) {
      plugin.doSync(player::closeInventory);
      lang.sendMessage(
          player, true, "ERROR.not-enough-space", "slots", data.getEmptySlotsRequired());
      return false;
    }
