...
public class Slot {
  ...
  @Getter private final TokenManagerPlugin plugin;
  @Getter private final Config config;
  @Getter private final Shop shop;
  @Getter private final int slot;
  @Getter private final int cost;
  @Getter private final int emptySlotsRequired;
  @Getter private final ItemStack displayed;
  ...
  @Getter private final boolean confirmPurchase;

  public Slot(
      final TokenManagerPlugin plugin,
      final Shop shop,
      final int slot,
      final int cost,
      final int emptySlotsRequired,
      final ItemStack displayed,
      final String message,
      final String subshop,
      final List<String> commands,
      final boolean usePermission,
      final boolean confirmPurchase) {
    this.plugin = plugin;
  ...
  public boolean purchase(final Player player, final boolean confirmPurchase, final boolean close) {
    ...
    if (balance - cost < 0) {
      ...
      plugin
          .getLang()
          .sendMessage(player, true, "ERROR.balance-not-enough", "needed", cost - balance);
      return false;
    }
    ...
    if (commands != null) {
      for (final String command : commands) {
        Bukkit.dispatchCommand(
            ...
            Placeholders.replace(command, balance, "balance", "tokens")
    ...
    if (message != null && !message.isEmpty()) {
      plugin
          ...
          .sendMessage(
              player,
              false,
              Placeholders.replace(message, balance, "balance", "tokens"),
              "player",
              player.getName());
    }
      ...
      if (target.isUsePermission() && !player.hasPermission(Permissions.SHOP + target.getName())) {
        plugin
            ...
            .sendMessage(
                ...
                "ERROR.no-permission",
                "permission",
                Permissions.SHOP + target.getName());
        return true;
      }
