...
public class SendCommand extends BaseCommand {
  ...
  private static final List<String> TAB_AMOUNTS =
      Arrays.asList("5", "10", "25", "50", "75", "100", "500", "1000");
  ...
  protected void execute(final CommandSender sender, final String label, final String[] args) {
    ...
    if (config.isAltPrevention()
        && target.getAddress().getHostName().equals(((Player) sender).getAddress().getHostName())) {
    ...
    if (amount <= 0
        || (config.getSendMin() > -1 && amount < config.getSendMin())
        || (config.getSendMax() > -1 && amount > config.getSendMax())) {
    ...
    final TokenReceiveEvent tokenReceiveEvent =
        new TokenReceiveEvent(target.getUniqueId(), (int) amount);
    ...
    sendMessage(
        target, true, "COMMAND.token.receive", "player", sender.getName(), "amount", amount);
  }
