...
public class FileDatabase extends AbstractDatabase {
  ...
  private static final String SERVER_MODE_MISMATCH =
      "Server is in %s mode, but data.yml does not contain %s! Please delete or rename data.yml to generate a new data.yml with valid format.";
  ...
  public void load(final Player player, final Function<Long, Long> modifyLoad) {
    plugin.doSync(
        () -> get(online ? player.getUniqueId().toString() : player.getName(), null, null, true));
  }
  ...
  public void ordered(final int limit, final Consumer<List<TopElement>> consumer) {
    ...
    plugin.doAsync(
        () -> {
          ...
          final List<TopElement> result =
              elements.size() > limit ? elements.subList(0, 10) : elements;
  ...
  public void transfer(final CommandSender sender, final Consumer<String> errorHandler) {
    ...
    final String query =
        String.format(
            "SELECT %s, tokens FROM %s;",
            online ? "uuid" : "name",
            StringUtils.replace(plugin.getConfiguration().getMysqlTable(), "'", "''"));
    ...
    hikariConfig.setDriverClassName(
        "com.mysql." + (CompatUtil.isPre1_17() ? "jdbc" : "cj") + ".Driver");
        ...
        () -> {
          sender.sendMessage(
              ChatColor.BLUE
                  ...
                  + ": Loading user data from MySQL database...");
          ...
          try (HikariDataSource dataSource = new HikariDataSource(hikariConfig);
            ...
            final File file =
                new File(plugin.getDataFolder(), "sqldump-" + System.currentTimeMillis() + ".yml");
            file.createNewFile();
