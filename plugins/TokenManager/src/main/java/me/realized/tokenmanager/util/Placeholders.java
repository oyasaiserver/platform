...
public final class Placeholders {
  ...
  public static void replace(
      final ItemStack item, final Object value, final String... placeholders) {
    if (!item.hasItemMeta()) {
      return;
    }
            ...
            for (final String placeholder : placeholders) {
              line =
                  value instanceof Number
                      ? replace(line, (Number) value, placeholder)
                      : line.replace("%" + placeholder + "%", String.valueOf(value));
            }
    ...
    if (meta.hasDisplayName()) {
      ...
      for (final String placeholder : placeholders) {
        displayName =
            value instanceof Number
                ? replace(displayName, (Number) value, placeholder)
                : displayName.replace("%" + placeholder + "%", String.valueOf(value));
      }
  ...
  public static String replace(String line, final Number value, final String... placeholders) {
    for (final String key : placeholders) {
      line =
          line.replace("%" + key + "%", String.valueOf(value))
