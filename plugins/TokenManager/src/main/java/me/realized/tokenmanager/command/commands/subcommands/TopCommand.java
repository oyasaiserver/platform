...
public class TopCommand extends BaseCommand {
  ...
  protected void execute(final CommandSender sender, final String label, final String[] args) {
    sendMessage(
        sender,
        true,
        "COMMAND.token.balance-top.next-update",
        "remaining",
        dataManager.getNextUpdate());

    final List<TopElement> top = dataManager.getTopCache();

    sendMessage(
        sender, true, "COMMAND.token.balance-top.header", "total", (top != null ? top.size() : 0));
    ...
    if (top == null || top.isEmpty()) {
      ...
      for (int i = 0; i < top.size(); i++) {
        ...
        sendMessage(
            sender,
            true,
            "COMMAND.token.balance-top.display-format",
            "rank",
            i + 1,
            "name",
            element.getKey(),
