...
public class OfflineCommand extends BaseCommand {
  ...
  public OfflineCommand(
      final TokenManagerPlugin plugin, final ModifyType type, final String... aliases) {
    super(
        plugin,
        type.name().toLowerCase(),
        type.name().toLowerCase() + " <username> <amount>",
        null,
        3,
        false,
        aliases);
      ...
      if (type == ModifyType.SET) {
        ...
        if (!balance.isPresent()) {
          ...
          sendMessage(
              ...
              "&c"
                  + target.getName()
                  + "'s data is currently loading! Command has been queued for future execution.");
          return;
        }
          ...
          dataManager.get(
              ...
              balance -> {
                ...
                dataManager.set(
                    ...
                    type,
                    amount,
                    type.apply(balance.getAsLong(), amount),
                    silent,
                    () ->
                        sendMessage(
                            sender,
              ...
              error ->
                  sender.sendMessage(
                      ChatColor.RED
                          + "Could not get token balance of "
                          + key.get()
                          + ": "
                          + error));
        });
  ...
  private void getTarget(
      final CommandSender sender, final String input, final Consumer<Optional<String>> consumer) {
    ...
    plugin.doAsync(
        () ->
            ProfileUtil.getUUID(
                input,
                uuid -> consumer.accept(Optional.ofNullable(uuid)),
                error ->
                    sender.sendMessage(
                        ChatColor.RED + "Failed to obtain UUID of " + input + ": " + error)));
  }
}
