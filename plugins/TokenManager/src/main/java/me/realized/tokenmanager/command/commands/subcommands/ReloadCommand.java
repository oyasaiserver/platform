...
public class ReloadCommand extends BaseCommand {
  ...
  protected void execute(final CommandSender sender, final String label, final String[] args) {
    if (args.length > getLength()) {
      ...
      if (plugin.reload(loadable)) {
        ...
        sendMessage(
            sender,
            false,
            "&cAn error occured while reloading "
                + name
    ...
    if (plugin.reload()) {
      sendMessage(
          sender, false, "&a[" + plugin.getDescription().getFullName() + "] Reload complete.");
    } else {
      sendMessage(
          sender,
          false,
          "&cAn error occured while reloading the plugin! The plugin will be disabled, please check the console for more information.");
    }
  }
