...
import io.oyasaiserver.OyasaiVehicles
...
private val compassYawByDirection = mapOf(
    "e" to -90.0,
    "n" to -180.0,
    "s" to 0.0,
    "w" to 90.0,
...
data class NetworkModeSetting(var mode: Int = 0) {
    ...
    fun currentModeLabel(): String = when (mode) {
        0 -> "Ultra"
        1 -> "High"
        2 -> "Medium"
        3 -> "Low"
...
fun sendDonationLink(player: Player) {
    ...
    val text2 = buildTextComponent(DONATION_URL, Color.AQUA)
...
fun copyTransformationValues(transformation: Transformation): Transformation = Transformation(
...
private fun <T : Entity> findNearbyEntitiesInWorld(
    ...
    return entities.filter { entity ->
        entity.world.uid == worldId && aabb.contains(entity.location.x, entity.location.y, entity.location.z)
    }
}
...
fun findNearbyEntities(searchEntityList: List<Entity>, loc: Location, x: Double, y: Double, z: Double): List<Entity> {
...
fun rayTraceEntityHit(
    ...
    return searchEntityList.any { entity ->
        entity.world.uid == worldId && entity.boundingBox.expand(raySize).rayTrace(startPos, direction, maxDistance) != null
    }
}
...
object Tools {
    ...
    fun getNearbyEntities(searchEntityList: List<Entity>, loc: Location, x: Double, y: Double, z: Double): List<Entity> =
        findNearbyEntities(searchEntityList, loc, x, y, z)

    fun rayTraceEntities(start: Location, direction: Vector, maxDistance: Double, raySize: Double, searchEntityList: List<Entity>): Boolean =
        rayTraceEntityHit(start, direction, maxDistance, raySize, searchEntityList)
