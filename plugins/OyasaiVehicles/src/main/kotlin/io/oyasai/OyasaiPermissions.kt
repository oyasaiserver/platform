...
fun CommandSender.hasOyasaiAdminPermission(): Boolean =
    isOp || hasPermission(OyasaiPermissions.ADMIN)
...
fun CommandSender.canHaveUnlimitedVehicles(): Boolean =
    hasAdminOrPermission(OyasaiPermissions.VEHICLE_UNLIMITED)

fun CommandSender.canUseCarBuilder2(): Boolean =
    hasAdminOrPermission(OyasaiPermissions.CAR_BUILDER2_USE)

fun CommandSender.canCreateCarBuilder2(): Boolean =
    canUseCarBuilder2() || hasPermission(OyasaiPermissions.CAR_BUILDER2_CREATE)
...
fun CommandSender.canCreateAircraftBuilder(): Boolean =
    canUseAircraftBuilder() || hasPermission(OyasaiPermissions.AIRCRAFT_BUILDER_CREATE)
