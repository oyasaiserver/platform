...
abstract class LandVehicle(
    ...
    open fun handling(wasd: ControlKey?, wheelZ: Float): Float {
        var handlingStrength: Float = HANDLING_MULTIPLIER * 0.1F.coerceAtLeast(5.0F - this.speed.z.toFloat() * 1.5F)
        if (handlingStrength >= HANDLING_MAX) handlingStrength = HANDLING_MAX

        return when (wasd) {
            ControlKey.A, ControlKey.WA, ControlKey.SD -> {
                this.oldHandlingFloat = -handlingStrength
                -handlingStrength
            }
            ControlKey.D, ControlKey.WD, ControlKey.SA -> {
                this.oldHandlingFloat = handlingStrength
                handlingStrength
            }
            else -> {
    ...
    private fun traceStepBlock(mainArmorStand: ArmorStand, forwardVec: Vector): org.bukkit.block.Block? {
    ...
    fun jump(mainArmorStand: ArmorStand): Double {
        ...
        val forwardVec = Vector(0.0, 0.0, 1.0).rotateAroundY(-Math.toRadians(mainArmorStand.location.yaw.toDouble()))
