...
abstract class CarVehicle(
  ...
  protected fun calculateAddSpeed(
    ...
    when (wasd) {
      ...
      ControlKey.WD -> {
        if (per > 1.0 && !slipstream) {
          if (speed.z > currentGearSpeed + 0.0085) {
            if (speed.z > speedLimit0) speed.multiply(0.99925)
            else speed.multiply(0.9875).multiply(ubcFriction)
          }
        } else if (nowGear > 0) {
          if (!cruiseMode || cruiseSpeed > speed.z) {
            if (gearShiftDelay <= 0) addSpeed = power0 * powerMultiplier * ratio / powerDivisor
          } else if (cruiseSpeed < speed.z) {
            speed.z = cruiseSpeed
          }
        } else if (nowGear == -1 && speed.z <= 0.0 && !cruiseMode && gearShiftDelay <= 0) {
          addSpeed = -(power0 * powerMultiplier * ratio / powerDivisor)
        }
      }
      ControlKey.S,
      ControlKey.SA,
      ControlKey.SD -> {
        addSpeed =
            when {
              speed.z > 0.0 -> -brake0 / brakeDivisor * Math.max(2.0 - ubc, 0.0)
              !manualTransmission && speed.z <= 0.01 ->
                  -brake0 / brakeDivisor * Math.max(2.0 - ubc, 0.0)
              speed.z < 0.0 -> brake0 / brakeDivisor * Math.max(2.0 - ubc, 0.0)
              else -> 0.0
            }
  ...
  protected fun applySpeedUpdate(
    ...
    val addSpeed = calculateAddSpeed(wasd, per, slipstream, powerDivisor, brakeDivisor, ubc)
