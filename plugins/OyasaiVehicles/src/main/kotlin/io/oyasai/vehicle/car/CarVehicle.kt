...
import io.oyasaiserver.vehicle.base.LandVehicle
...
abstract class CarVehicle(
    ...
    init {
        if (this.maximumGears > 0) {
            val finalGear = this.maximumGears * (0.5 + this.maximumGears * 0.5 / (this.maximumGears * this.maximumGears))
    ...
    fun autoTrans(per: Double) {
        ...
        } else if (nowGear > 1 && gearSpeedList.getOrNull(nowGear - 1)?.let { speed.z / it < 0.875 } == true) {
            gearShift(-1)
        }
    }
    ...
    protected fun calculateAddSpeed(wasd: ControlKey?, per: Double, slipstream: Boolean, ubc: Double): Double {
        var addSpeed = 0.0
        ...
        when (wasd) {
            ControlKey.W, ControlKey.WA, ControlKey.WD -> {
                if (per > 1.0 && !slipstream) {
                    if (speed.z > currentGearSpeed + 0.0085) {
                        if (speed.z > speedLimit0) speed.multiply(0.99925 * VehicleBalanceSettings.carOverspeedFriction)
                        else speed.multiply(0.9875 * VehicleBalanceSettings.carOverspeedFriction).multiply(ubcFriction)
                    }
                } else if (nowGear > 0) {
            ...
            ControlKey.S, ControlKey.SA, ControlKey.SD -> {
                ...
                if (cruiseMode) {
                    if (cruiseSpeed > speed.z && gearShiftDelay <= 0) addSpeed = power0 * powerMultiplier * ratio / POWER_DIVISOR
    ...
    protected fun applySpeedUpdate(wasd: ControlKey?, slot: Int, slipstream: Boolean, ubc: Double, slipMultiplier: Double = 1.0): Vector {
