package io.oyasaiserver.vehicle.car
...
data class CarNormalVehicle(
    ...
    override fun controlSpeed(wasd: ControlKey?, slot: Int, slipstream: Boolean, ubc: Double): Vector =
        applySpeedUpdate(wasd, slot, slipstream, ubc, 1.0)
    ...
    override fun slip(first: ControlKey?): Double {
        ...
        when (first) {
            ...
            ControlKey.WA, ControlKey.SA, ControlKey.A -> {
                slipAngle += speedFactor
                if (first == ControlKey.SA) slipAngle *= 0.97
                    speed.multiply(FRICTION_HEAVY * VehicleBalanceSettings.carTurnFriction)
            }
            ControlKey.WD, ControlKey.SD, ControlKey.D -> {
                slipAngle -= speedFactor
    ...
    override fun addSlip(first: ControlKey?): Float {
        ...
        return when (first) {
            ControlKey.WA, ControlKey.A -> d.toFloat()
            ControlKey.WD, ControlKey.D -> (-d).toFloat()
...
data class CarProVehicle(
...
) : CarVehicle(speedLimitVal, powerVal, brakeVal, maxSizeVal, underBlockCheckVal, gearMaxVal, isMTVal) {
    ...
    override fun controlSpeed(wasd: ControlKey?, slot: Int, slipstream: Boolean, ubc: Double): Vector =
        applySpeedUpdate(wasd, slot, slipstream, ubc, 1.0 - slipAngle / SLIP_ANGLE_MAX)
    ...
    override fun handling(wasd: ControlKey?, wheelZ: Float): Float {
        ...
        val result = if (plusMode) {
            when (wasd) {
                ...
                ControlKey.SA, ControlKey.A -> -handlingStrength - slipFloat / 10.0F
                ControlKey.SD, ControlKey.D -> handlingStrength - slipFloat / 10.0F
    ...
    override fun slip(first: ControlKey?): Double {
        ...
        when (first) {
            null -> slipAngle *= 0.5
            ControlKey.W -> slipAngle *= (if (plusMode) min(0.6F + speedAbs.toFloat(), gripAdjustedSlipFactor) else 0.9675F).toDouble()
            ControlKey.WA, ControlKey.SA, ControlKey.A -> {
        ...
        if (abs(slipAngle) <= 3.5 && first !in listOf(ControlKey.W, ControlKey.WA, ControlKey.WD, ControlKey.SA, ControlKey.SD, ControlKey.A, ControlKey.D)) {
            slipAngle = 0.0
        }
    ...
    private fun handleLeftTurnSlip(first: ControlKey, speedAbs: Float) {
        when (first) {
            ControlKey.SA -> if (plusMode) { slipAngle += 10.6 * (0.5 + speedAbs); slipAngle *= 1.05 } else slipAngle *= 0.945
            ControlKey.WA -> slipAngle *= (if (plusMode) min(0.6F + speedAbs, 0.99F) else 0.975F).toDouble()
    ...
    private fun handleRightTurnSlip(first: ControlKey, speedAbs: Float) {
        when (first) {
            ControlKey.SD -> if (plusMode) { slipAngle -= 10.6 * (0.5 + speedAbs); slipAngle *= 1.05 } else slipAngle *= 0.945
            ControlKey.WD -> slipAngle *= (if (plusMode) min(0.6F + speedAbs, 0.99F) else 0.975F).toDouble()
...
class CarRealVehicle(
    ...
    override fun handling(wasd: ControlKey?, wheelZ: Float): Float {
        val baseHandling = when (wasd) {
            null -> 0.0f
            ControlKey.WA, ControlKey.SA, ControlKey.A -> -35.0f
            ControlKey.WD, ControlKey.SD, ControlKey.D -> 35.0f
        ...
        if (gripAdjustedHandlingForce == 0.0F) {
            ...
            when (wasd) {
                ControlKey.WA, ControlKey.SA, ControlKey.A, ControlKey.WD, ControlKey.SD, ControlKey.D -> handleAngle += gripAdjustedHandlingForce * 0.12F
                else -> {
                    handleAngle *= 0.95F
                    if (handleAngle in -0.75F..0.75F) handleAngle = 0.0F
                }
        ...
        handleAngle = handleAngle.coerceIn(-(35.0 / (0.5 + abs(speed.z) * 1.5)).toFloat(), (35.0 / (0.5 + abs(speed.z) * 1.5)).toFloat())
