package io.oyasaiserver.vehicle.air

import io.oyasaiserver.toolbox.ControlKey
import io.oyasaiserver.vehicle.base.AirVehicle
    ...
    override fun controlSpeed(wasd: ControlKey?): Vector {
        ...
        when (wasd) {
            ...
            ControlKey.S, ControlKey.SA, ControlKey.SD -> {
                addSpeed = if (this.speed.z > 0.0)
                    -this.brake / 15000.0
                else if (this.speed.z <= 0.01)
                    -this.brake / 15000.0
                else if (this.speed.z < 0.0)
                    this.brake / 15000.0
                else 0.0
            }
            else -> {
    ...
    override fun slip(first: ControlKey?): Float {
        when (first) {
            ...
            ControlKey.WA, ControlKey.SA, ControlKey.A -> {
                this.slipAngle = (this.slipAngle + this.slip + 1.5F.coerceAtMost(
                    Math.abs(this.speed.z).toFloat() * 1.2F
                )) * 0.899F
