package io.oyasaiserver.vehicle.base
...
object VehicleBalanceSettings {
    ...
    @Volatile var carOverspeedFriction: Double = DEFAULT_CAR_OVERSPEED_FRICTION
        private set
    @Volatile var carCoastFriction: Double = DEFAULT_CAR_COAST_FRICTION
        private set
    @Volatile var carTurnFriction: Double = DEFAULT_CAR_TURN_FRICTION
        private set
    @Volatile var carBrakeMultiplier: Double = DEFAULT_CAR_BRAKE_MULTIPLIER
        private set
    @Volatile var aircraftCoastFriction: Double = DEFAULT_AIRCRAFT_COAST_FRICTION
        private set
    @Volatile var aircraftTurnFriction: Double = DEFAULT_AIRCRAFT_TURN_FRICTION
        private set
    @Volatile var aircraftBrakeMultiplier: Double = DEFAULT_AIRCRAFT_BRAKE_MULTIPLIER
        private set
    @Volatile var upgradeCostLimit: Int = DEFAULT_UPGRADE_COST_LIMIT
        private set
    @Volatile var sizeLimit: Double = DEFAULT_VEHICLE_SIZE_LIMIT
        private set
    fun reload(config: FileConfiguration?) {
        upgradeCostLimit = config.readInt(PATH_UPGRADE_COST_LIMIT, DEFAULT_UPGRADE_COST_LIMIT)
        carOverspeedFriction = config.readClampedDouble(PATH_CAR_OVERSPEED_FRICTION, DEFAULT_CAR_OVERSPEED_FRICTION, 0.0, 1.0)
        carCoastFriction = config.readClampedDouble(PATH_CAR_COAST_FRICTION, DEFAULT_CAR_COAST_FRICTION, 0.0, 1.0)
        carTurnFriction = config.readClampedDouble(PATH_CAR_TURN_FRICTION, DEFAULT_CAR_TURN_FRICTION, 0.0, 1.0)
        carBrakeMultiplier = config.readMinimumDouble(PATH_CAR_BRAKE_MULTIPLIER, DEFAULT_CAR_BRAKE_MULTIPLIER, 0.0)
        aircraftCoastFriction = config.readClampedDouble(PATH_AIRCRAFT_COAST_FRICTION, DEFAULT_AIRCRAFT_COAST_FRICTION, 0.0, 1.0)
        aircraftTurnFriction = config.readClampedDouble(PATH_AIRCRAFT_TURN_FRICTION, DEFAULT_AIRCRAFT_TURN_FRICTION, 0.0, 1.0)
        aircraftBrakeMultiplier = config.readMinimumDouble(PATH_AIRCRAFT_BRAKE_MULTIPLIER, DEFAULT_AIRCRAFT_BRAKE_MULTIPLIER, 0.0)
        sizeLimit = config.readMinimumDouble(PATH_VEHICLE_SIZE_LIMIT, DEFAULT_VEHICLE_SIZE_LIMIT.toDouble(), 0.0).toInt()
    }
