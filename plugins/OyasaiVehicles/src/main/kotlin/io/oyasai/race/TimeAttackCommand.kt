...
object YamlData {
    ...
    val cpList = listOf(
        "CP:50", "CP:100", "CP:200", "CP:300", "CP:400", "CP:500", "CP:600", "CP:700", "CP:800", "CP:900", "CP:1000",
        "CP:1100", "CP:1200", "CP:1300", "CP:1400", "CP:1500", "CP:1600", "CP:1700", "CP:1800", "CP:1900", "CP:2000",
        "CP:2100", "CP:2200", "CP:2300", "CP:2400", "CP:2500"
    )
    ...
    fun getUserYaml(circuit: String, uuid: UUID): CustomYaml {
        return yamlList.getOrPut(Pair(uuid, circuit)) {
            CustomYaml("$dataFolder$circuit/$uuid.yml")
        }
    }
    ...
    fun sendBestTimes(player: Player, circuit: String) {
        ...
        for (cp in cpList) {
            ...
            if (bestTime.isNotEmpty()) {
                ...
                player.sendMessage(color("[TA] &9$circuit&7:&r$cp ベストタイム &a${LocalTime.ofNanoOfDay(totalTime).format(Tools.timeFormat)}"))
            }
        }
    }
}
                ...
                for (key in activeKeys) {
                    ...
                    if (timeData != null) {
                        ...
                        if (sectorTime != null && key.first.isOnline) {
                            key.first.sendMessage(color("[TA] &9$circuit&7:&rS$sectorInt &a${sectorTime.format(Tools.timeFormat)}"))
                        }
                    }
                }
            }
            "lap", "goal" -> {
                for (key in activeKeys) {
                    ...
                    if (lastSector != null && totalTime != null && key.first.isOnline) {
                        ...
                        player.sendMessage(color("[TA] &9$circuit&7:&rLS &a${lastSector.format(Tools.timeFormat)}"))
                        player.sendMessage(color("[TA] &9$circuit&7:&rTime &d${totalTime.format(Tools.timeFormat)}"))
                        
                        if (YamlData.setTimeData(circuit, player.uniqueId, timeData)) {
                            player.sendMessage(color("[TA] &9$circuit&7:&b${costRateConvert(timeData.cost)}の自己ベスト更新!"))
                        }
                    }
