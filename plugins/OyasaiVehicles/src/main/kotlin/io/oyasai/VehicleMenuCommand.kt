...
import io.oyasaiserver.anybuilder.carbuilder2.CarBuilder2GUI
...
import io.oyasaiserver.toolbox.NetworkModeSetting
import io.oyasaiserver.toolbox.OyasaiMenu
import io.oyasaiserver.toolbox.UnderBlockPattern
import io.oyasaiserver.toolbox.Tools
import io.oyasaiserver.toolbox.Tools.addText
import io.oyasaiserver.toolbox.Tools.allHide
...
object VehicleMenuCommand : CommandExecutor, TabCompleter {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        return when (args[0]) {
            ...
            SUB_NETWORK, "network" -> handleNetworkCommand(sender)
    ...
    override fun onTabComplete(sender: CommandSender, command: Command, label: String, args: Array<out String>): MutableList<String>? {
        ...
        return when (args.size) {
            ...
            2 -> when (args[0]) {
                SUB_ADMIN -> if (sender.hasOyasaiAdminPermission()) mutableListOf(SUB_EVENT_MODE, SUB_UNDER_BLOCK, SUB_RELOAD) else mutableListOf()
    ...
    private fun mainMenu(): OyasaiMenu {
        ...
        val glass = ItemStack(Material.BLACK_STAINED_GLASS_PANE).allHide().addText("&e統合版は非対応", emptyList())
        ...
        gui.setItem(SLOT_CAR, ItemStack(Material.ENDER_PEARL).allHide().addText(
            "&aCarBuilder2",
            listOf("&f車の試乗・購入などはこちらから", "&e統合版は非対応")
        )) {
            openBuilderMenu(it) { player -> CarBuilder2GUI.menu().open(player) }
        }

        gui.setItem(SLOT_AIRCRAFT, ItemStack(Material.FEATHER).allHide().addText(
            "&aAircraftBuilder",
            listOf("&f飛行機の試乗・購入などはこちらから", "&e統合版は非対応")
        )) {
            openBuilderMenu(it) { player -> AircraftBuilderGUI.menu().open(player) }
        }

        val cameraIcon = (CustomHead.getHead("camera") ?: ItemStack(Material.ENDER_EYE)).allHide().addText(
            "&f視点追尾",
            listOf("&f乗車時の視点追尾ON/OFFを切り替え")
        )
        gui.setItem(SLOT_CAMERA, cameraIcon) {
            openCommand(it.whoClicked as Player, "$COMMAND_ALIAS $SUB_CAMERA")
        }

        gui.setItem(SLOT_GARAGE, ItemStack(Material.CHEST).allHide().addText(
            "&fガレージ",
            listOf("&fガレージを開く")
        )) {
    ...
    private fun handleCameraCommand(sender: CommandSender): Boolean {
        ...
        player.sendMessage(Tools.color("[OyasaiVehicles] 視点追尾(カメラ自動回転)を${if (newStatus) "&a有効" else "&c無効"}&rに切り替えました"))
        return true
    }
    ...
    private fun handleNetworkCommand(sender: CommandSender): Boolean {
        ...
        setting.nextMode()
        player.sendMessage(Tools.color("[OyasaiVehicles] ネットワーク設定を&a${setting.currentModeLabel()}&rに切り替え"))
        return true
    }
    ...
    private fun handleAdminCommand(sender: CommandSender, args: Array<out String>): Boolean {
        ...
        when (args.getOrNull(1)?.lowercase(Locale.ROOT)) {
            SUB_EVENT_MODE -> {
                eventMode = !eventMode
                sender.sendMessage(Tools.color("[OyasaiVehicles] イベントモードを${if (eventMode) "&a有効化" else "&c無効化"}&rしました"))
            }
            SUB_UNDER_BLOCK -> sender.sendMessage("UnderBlockIsSpeed.yml reload! load Material=> ${UnderBlockPattern.loadConfig()}")
            SUB_RELOAD -> when (args.getOrNull(2)?.lowercase(Locale.ROOT)) {
                null, "", "all" -> sender.sendMessage(VehicleReloadService.reloadAll())
                "config" -> sender.sendMessage(VehicleReloadService.reloadSettings())
                "vehicle", "vehicles" -> sender.sendMessage(VehicleReloadService.reloadVehicles())
                else -> sender.sendMessage(Tools.color("[OyasaiVehicles] Unknown reload target: ${args[2]}"))
            }
            null -> Unit
            else -> sender.sendMessage(Tools.color("[OyasaiVehicles] Unknown admin subcommand: ${args[1]}"))
        }
        return true
    }
    ...
    private fun openBuilderMenu(event: org.bukkit.event.inventory.InventoryClickEvent, openMenu: (Player) -> Unit) {
