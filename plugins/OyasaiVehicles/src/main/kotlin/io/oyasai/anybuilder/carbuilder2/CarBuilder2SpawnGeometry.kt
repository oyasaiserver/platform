        ...
        val blockDisplays = if (vec.x >= 0.0) {
            ...
            for ((wVec, pairData) in blockData) {
                ...
                if (flippedData is Directional) {
                    flippedData.facing = when (flippedData.facing) {
                ...
                if (flippedData is Rotatable) {
                    flippedData.rotation = when (flippedData.rotation) {
                        BlockFace.EAST -> BlockFace.WEST
                        BlockFace.WEST -> BlockFace.EAST
...
fun CarBuilder2Spawn.getCarSize(cbd: CarBuilder2BaseData): Vector {
    ...
    val fixWidth = cbd.config.getSize(CarBuilder2BaseDataType.Body) / (cbd.boxSize[CarBuilder2BaseDataType.Body]?.maxX ?: 1.0)
    ...
    fun applyWheelRange(wheelData: Map<Vector, Pair<BlockData, Vector>>, type: CarBuilder2BaseDataType) {
        var maxXW = 0.0
        var minXW = 0.0
        var maxYW = 0.0
        var minYW = 0.0
