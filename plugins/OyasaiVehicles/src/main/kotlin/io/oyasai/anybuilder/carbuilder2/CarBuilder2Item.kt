...
import io.oyasaiserver.vehicle.car.CarNormalVehicle
import io.oyasaiserver.vehicle.car.CarProVehicle
import io.oyasaiserver.vehicle.car.CarRealVehicle
...
object CarBuilder2Item {
    ...
    fun buyCarItem(carName: String, playerName: String?): ItemStack? {
        ...
        return BuilderItemSupport.buildMinecartItem(
            ...
            10831,
            listOf(
                "Car: ${data.name}", "Cost: 0", "最高速: 68", "パワー: 64", "ブレーキ: 128", "ギア数: 4", "MT車: false", "タイヤ: 100", "Mode: Normal"
            )
        )
    }
    ...
    fun changeCarMode(item: ItemStack): String? {
        var mode = ""
        val updated = BuilderItemSupport.rewriteLore(item) { lore ->
    ...
    fun getCarProVehicle(meta: ItemMeta?, maxSize: Double, ubc: Boolean, plus: Boolean): CarProVehicle {
        val stats = readStats(meta).clamped()
