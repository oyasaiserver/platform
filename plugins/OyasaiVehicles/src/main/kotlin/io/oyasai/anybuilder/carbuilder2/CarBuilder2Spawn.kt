...
import io.oyasaiserver.anybuilder.carbuilder2.data.*
import io.oyasaiserver.anybuilder.common.spawn.BuilderSpawnSupport
import io.oyasaiserver.anybuilder.runtime.ArmorStandSpawner
...
object CarBuilder2Spawn {
    ...
    fun spawn(loc: Location, item: ItemStack, owner: Player?, spawnPlayer: Player?, data: CarBuilder2BaseData, yaw: Float = loc.yaw): Boolean {
        ...
        val seatMap = BuilderSpawnSupport.spawnSeat(
            loc,
        ...
        val bodyArmorStandList = BuilderSpawnSupport.spawnBodyArmorStands(
            loc,
            bodyArmorStandInt,
            yaw,
        ...
        val displayMap = mutableMapOf<CarBuilder2EntityType, MutableMap<Pair<Int, Vector>, MutableSet<BlockDisplay>>>()
        
        displayMap[CarBuilder2EntityType.Body] = mutableMapOf(
            (0 to Vector()) to BuilderSpawnSupport.spawnBlockDisplay(
                loc,
                bodyArmorStandList,
                data.body,
        ...
        displayMap[CarBuilder2EntityType.Wheel] = spawnAutoFlip(
            loc, bodyArmorStandList, data.wheel, 
        ...
        displayMap[CarBuilder2EntityType.Wheel2] = spawnAutoFlip(
            loc, bodyArmorStandList, data.wheel2, 
        ...
        displayMap[CarBuilder2EntityType.HeadLight] = spawnAutoFlip(
            loc, bodyArmorStandList, data.headLight, 
        ...
        val vehicle = when (carMode) {
            "Normal", "Normal_Race" -> CarBuilder2Item.getCarNormalVehicle(meta, maxSize, carMode == "Normal_Race")
            "Pro", "Pro+" -> CarBuilder2Item.getCarProVehicle(meta, maxSize, true, carMode == "Pro+")
            "Real" -> CarBuilder2Item.getCarRealVehicle(meta, maxSize)
            else -> return false
        }

        val cEntity = CarBuilder2EntityData(
            seatMap,
            bodyArmorStandList,
            displayMap,
            vehicle,
