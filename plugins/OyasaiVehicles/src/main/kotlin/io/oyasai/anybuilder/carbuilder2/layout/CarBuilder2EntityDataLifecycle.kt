package io.oyasaiserver.anybuilder.carbuilder2.layout
...
fun CarBuilder2EntityData.reOffsetDisplayMapImpl(type: CarBuilder2EntityType, newOffset: Map<Int, Vector>) {
 ...
 fun CarBuilder2EntityData.saveDefImpl(type: CarBuilder2EntityType) {
    ...
    this.display[type]?.values?.forEach { set ->
        set.forEach { blockDisplay ->
            map[blockDisplay] = blockDisplay.transformation
        }
    }
    this.displayDef[type] = map
}

 fun CarBuilder2EntityData.reOffsetDisplayImpl(type: CarBuilder2EntityType, newOffset: MutableMap<Int, Vector>) {
                    ...
                    for (displaySet in allTypeMap.values) {
                        for (blockDisplay in displaySet) {
                            ...
                            blockDisplay.transformation = Transformation(
                                Vector3f(trans.translation).add(diff.toVector3f()).rotateY(Math.toRadians(this.setYaw.toDouble()).toFloat()),
                                Quaternionf(trans.leftRotation).rotationX(Math.toRadians(this.pitch.toDouble()).toFloat()).rotateY(Math.toRadians(this.setYaw.toDouble()).toFloat()),
                                trans.scale,
                                trans.rightRotation
                            )
                        }
