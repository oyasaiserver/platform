package io.oyasaiserver.anybuilder.aircraftbuilder.presentation
...
import org.joml.Vector3f
 ...
 fun AircraftBuilderEntityData.soundTaskImpl(wasd: ControlKey?) {
    ...
    world.playSound(loc, Sound.ENTITY_COW_STEP, SoundCategory.PLAYERS, this.gasDouble.toFloat() * 0.45f, 0.5f + this.gasDouble.toFloat())
            ...
            if (def != null) {
                blockDisplay.setTransformation(
                    Transformation(
                        ...
                        Quaternionf().rotationY(Math.toRadians(this.setYaw.toDouble()).toFloat())
                            .rotateX(Math.toRadians(this.pitch.toDouble()).toFloat()),
                        def.scale,
                        def.rightRotation
                    )
