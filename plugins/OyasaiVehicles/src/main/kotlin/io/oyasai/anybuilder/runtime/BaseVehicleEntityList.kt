...
abstract class BaseVehicleEntityList<T : VehicleEntityData> {
    ...
    private fun matchesEntityUuid(data: T, uuid: UUID): Boolean {
        return data.seatArmorStands.keys.any { it.second.uniqueId == uuid } || data.bodyArmorStands.any { it.uniqueId == uuid }
    }
    ...
    fun checkDrivingSeat(uuid: UUID): T? = cache.firstOrNull { data ->
        data.seatArmorStands.keys.any { it.first == 0 && it.second.uniqueId == uuid }
    }

    fun checkSeatArmorStand(uuid: UUID): Boolean = cache.any { data ->
        data.seatArmorStands.keys.any { it.second.uniqueId == uuid }
    }

    fun checkSeatPlayerUUID(uuid: UUID): Boolean = cache.any { data ->
        data.getCarPlayers().any { it.uniqueId == uuid }
    }

    fun getData(uuid: UUID): T? = cache.firstOrNull { matchesEntityUuid(it, uuid) }

    fun allExit() {
        cache.filter { !it.exit }.forEach { data ->
            data.exit = true
            data.exitTask()
        }
    }
    ...
    fun breakUserVehicles(userUuid: UUID): Int {
        var count = 0
        cache.filter { it.summoner?.uniqueId == userUuid && !it.exit }.forEach { data ->
            data.exit = true
            data.exitTask()
            count++
        }
