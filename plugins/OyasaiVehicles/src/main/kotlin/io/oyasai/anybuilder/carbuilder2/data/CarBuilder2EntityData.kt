...
data class CarBuilder2EntityData(
    ...
    val display:
        MutableMap<CarBuilder2EntityType, MutableMap<Pair<Int, Vector>, MutableSet<BlockDisplay>>>,
  ...
  val displayTypeList: MutableMap<CarBuilder2DisplayType, MutableSet<BlockDisplay>> =
      LinkedHashMap()
  val displayDef: MutableMap<CarBuilder2EntityType, MutableMap<BlockDisplay, Transformation>> =
      LinkedHashMap()
  ...
  override fun displayGroups(): Iterable<Iterable<org.bukkit.entity.Display>> =
      this.display.values.flatMap { it.values }

  fun reOffsetDisplayMap(type: CarBuilder2EntityType, newOffset: Map<Int, Vector>) =
      reOffsetDisplayMapImpl(type, newOffset)

  fun saveDef(type: CarBuilder2EntityType) = saveDefImpl(type)

  fun reOffsetDisplay(type: CarBuilder2EntityType, newOffset: MutableMap<Int, Vector>) =
      reOffsetDisplayImpl(type, newOffset)
  ...
  fun smokeParticle(speed: Double, slipAngle: Float, yaw: Float, player: Player?) =
      smokeParticleImpl(speed, slipAngle, yaw, player)
