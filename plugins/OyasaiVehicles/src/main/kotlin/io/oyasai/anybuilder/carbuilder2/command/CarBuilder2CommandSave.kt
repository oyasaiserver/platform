...
fun CarBuilder2Command.handleCarBuilder2Save(
  ...
  executeSaveFlow(
      ...
      invalidNameMessage =
          "[CarBuilder2] Vehicle name can only contain alphanumeric characters and underscores!",
      resolveData = { vehicleName ->
        existingData
            ?: CarBuilder2BaseData(vehicleName).also { CarBuilder2BaseCache.addBaseData(it) }
      },
      ...
      onReady = { player, vehicleData ->
        if (args.size < 3) {
          ...
          if (baseDataType == null) {
            ...
            val size =
                if (args.size >= 4)
                    args[3].toDoubleOrNull() ?: vehicleData.config.getSize(baseDataType)
                else vehicleData.config.getSize(baseDataType)
...
private fun scaleSeatOffsets(config: CarBuilder2Config, scaleRatio: Double) {
  ...
  scaledSeatOffsets.forEach { (index, vector) -> config.setSeatOffset(index, vector) }
}
...
private fun scalePartOffsets(config: CarBuilder2Config, scaleRatio: Double) {
  OFFSET_AUTO_SCALE_TYPES.forEach { baseDataType ->
    ...
    scaledOffsets.forEach { (index, vector) -> config.setPartOffset(baseDataType, index, vector) }
  }
}
