package io.oyasaiserver.anybuilder.carbuilder2.physics
...
fun CarBuilder2EntityData.computeVelocity(
    ...
    return this.vehicle.controlSpeed(
        wasd,
        slot,
        slipstreamActive,
...
fun slipstream(data: CarBuilder2EntityData, slipAngle: Float, wasd: ControlKey?): Boolean {
    ...
    return when (wasd) {
        ControlKey.W, ControlKey.WA, ControlKey.WD -> {
            ...
            val dir = Vector(0.0, 0.0, 1.0).rotateAroundY(Math.toRadians((data.bodyArmorStands[0].location.yaw + slipAngle).toDouble()))
            val excludeList = CarBuilder2EntityList.getBodyArmorStandList(data.bodyArmorStands.first().uniqueId)
            if (Tools.rayTraceEntities(loc, dir, (data.vehicle.speed.z + 0.01) * 30.0, 5.0, excludeList)) {
