package io.oyasaiserver.anybuilder.carbuilder2.lifecycle
...
fun CarBuilder2EntityData.autoSitStartImpl() {
    ...
    object : BukkitRunnable() {
        override fun run() {
            ...
            if (firstSeat == null) {
                Bukkit.getLogger().warning("[CarBuilder2] Despawning car ${this@autoSitStartImpl.baseData.name}: No seats found!")
            ...
            if (firstSeat.isDead) {
                Bukkit.getLogger().warning("[CarBuilder2] Despawning car ${this@autoSitStartImpl.baseData.name}: Driver seat ArmorStand is dead!")
                this@autoSitStartImpl.exit = true
                this@autoSitStartImpl.exitTask()
                this.cancel()
                return
            ...
            if (speed in -0.05..0.05) {
                for (entry in this@autoSitStartImpl.seatArmorStands.entries) {
                    ...
                    if (bodyArmorStands.passengers.isEmpty()) {
                        val player = Tools.getNearbyPlayers(bodyArmorStands.eyeLocation, 0.3, 1.5, 0.3)
                            .firstOrNull { !it.isSneaking && (it as Entity).isOnGround }
                        ...
                        if (player != null && delayList[player.uniqueId] == null) {
                            ...
                            if (pair.first == 0) {
                                ...
                                val mtStr = if (this@autoSitStartImpl.vehicle.manualTransmission) "４=シフトダウン, ６=シフトアップ | " else ""
                                player.sendMessage("[CarBuilder2] １=左ウインカー, ２=ハザード, ３=右ウインカー | ${mtStr} ８=ライトON/OFF, ９=クルーズモード切替")
                            }
                        }
            ...
            val driverSeat = this@autoSitStartImpl.seatArmorStands.keys.firstOrNull { it.first == 0 }?.second
            if (driverSeat != null && driverSeat.passengers.isNotEmpty() && !this@autoSitStartImpl.driveStartSwitch) {
