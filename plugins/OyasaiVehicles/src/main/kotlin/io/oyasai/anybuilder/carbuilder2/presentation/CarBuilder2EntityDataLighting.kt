...
import kotlin.math.roundToInt
 ...
 fun CarBuilder2EntityData.updateBarImpl(slipstream: Boolean) {
    ...
    if (this.vehicle.nowGear == 0) {
        ...
        val barColorVal = if (this.vehicle.speed.z > 0.0) (if (slipstream) BarColor.YELLOW else BarColor.GREEN) else BarColor.RED
        ...
        var progress = this.vehicle.speed.z / (if (this.vehicle.nowGear > 0) this.vehicle.gearSpeedList[this.vehicle.nowGear] else this.vehicle.speedLimit0 * 0.33)
 ...
 fun CarBuilder2EntityData.bodyLightTaskImpl(slot: Int, wasd: ControlKey?) {
            ...
            if (firstDisplay.brightness != brightness) {
                for (displaySet in map.values) {
                    for (display in displaySet) {
                        if (displayTypeList[CarBuilder2DisplayType.LightBlock]?.contains(display) == true) continue
                        if (displayTypeList[CarBuilder2DisplayType.BrakeBlock]?.contains(display) == true) continue
                        if (displayTypeList[CarBuilder2DisplayType.WinkerLeft]?.contains(display) == true) continue
                        if (displayTypeList[CarBuilder2DisplayType.WinkerRight]?.contains(display) == true) continue
    ...
    val brakeBrightness = if (isBraking) Brightness(15, 15) else {
        if (this.lightSwitch) Brightness(brakeBaseLight, 6) else Brightness(brakeBaseLight, brakeSkyLight)
    }
    ...
    when (slot) {
        ...
        -3 -> this.winkerSwitch = if (this.winkerSwitch.first && this.winkerSwitch.second) false to false else true to true
    ...
    displayTypeList[CarBuilder2DisplayType.WinkerLeft]?.forEach { it.brightness = if (leftOn) winkerOnBrightness else winkerOffBrightness }
    displayTypeList[CarBuilder2DisplayType.WinkerRight]?.forEach { it.brightness = if (rightOn) winkerOnBrightness else winkerOffBrightness }
 ...
 fun CarBuilder2EntityData.retractableTaskImpl(switch: Boolean) {
    ...
    val rotation = Quaternionf().rotateY(Math.toRadians(this.setYaw.toDouble()).toFloat()).rotateX(angleRad.toFloat())
            ...
            for (x in -1..1) {
                ...
                while (current.add(forward).block.isPassable) {
                    ...
                    if (count == 15 && current.block.type == Material.AIR) {
                        ...
                        block.setBlockData(Material.LIGHT.createBlockData { (it as org.bukkit.block.data.type.Light).level = 15 }, false)
                ...
                if (count > 1) {
                    ...
                    if (current.block.type == Material.AIR) {
                        ...
                        current.block.setBlockData(Material.LIGHT.createBlockData { (it as org.bukkit.block.data.type.Light).level = level }, false)
                        this.lightBlockList.add(current.clone())
                    }
                }
            }
