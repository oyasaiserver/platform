...
import io.oyasaiserver.toolbox.ControlKey
...
import kotlin.math.min
 ...
 fun CarBuilder2EntityData.wheelTaskImpl(speed: Double, handleAngle: Float?) {
    ...
    val wheelRotation = Quaternionf(AxisAngle4f(haRad.toFloat(), 0f, 1f, 0f)).rotateY(Math.toRadians(this.setYaw.toDouble()).toFloat()).rotateX(rpmRad.toFloat())
    val wheelRotationSimple = Quaternionf().rotateY(Math.toRadians(this.setYaw.toDouble()).toFloat()).rotateX(rpmRad.toFloat())

    fun updateWheelGroup(type: CarBuilder2EntityType, rotation: Quaternionf, rotationWithHa: Quaternionf) {
        ...
        for ((offset, displays) in typeMap) {
            ...
            for (display in displays) {
                ...
                val translation = Vector(defTrans.translation.x.toDouble(), defTrans.translation.y.toDouble(), defTrans.translation.z.toDouble())

                val finalTranslation = if (isFront) {
                ...
                display.transformation = Transformation(
                    finalTranslation.add(offset.second).add(bodyOffset).toVector3f().rotateY(Math.toRadians(this.setYaw.toDouble()).toFloat()),
                    currentRotation,
    ...
    for (displaySet in bodyMap.values) {
        for (blockDisplay in displaySet) {
            ...
            blockDisplay.transformation = Transformation(
                ...
                Quaternionf().rotationY(Math.toRadians(this.setYaw.toDouble()).toFloat()).rotateX(Math.toRadians(this.pitch.toDouble()).toFloat()),
                defTrans.scale,
 ...
 fun CarBuilder2EntityData.soundTaskImpl(wasd: ControlKey?) {
    ...
    world.playSound(loc, Sound.ENTITY_WARDEN_AMBIENT, SoundCategory.PLAYERS, roadNoise * 0.25f, 0.5f + roadNoise * 0.18f)

    val egSound = this.baseData.config.getEGSound()
    when (egSound) {
        "EV" -> {
            world.playSound(loc, Sound.BLOCK_AMETHYST_BLOCK_CHIME, SoundCategory.PLAYERS, this.bossBar.progress.toFloat() * 1.5f, 0.5f + this.bossBar.progress.toFloat() * 1.5f)
            return
        }
        "Turbo" -> {
            world.playSound(loc, Sound.ENTITY_BLAZE_SHOOT, SoundCategory.PLAYERS, this.bossBar.progress.toFloat() * 0.45f + 0.1f, 0.5f + this.bossBar.progress.toFloat() * 0.7f)
            world.playSound(loc, Sound.BLOCK_BARREL_CLOSE, SoundCategory.PLAYERS, this.bossBar.progress.toFloat() * 0.75f + 0.2f, 0.5f + this.bossBar.progress.toFloat() * 0.4f)
            ...
            if (wasd == ControlKey.W || wasd == ControlKey.WA || wasd == ControlKey.WD) {
                ...
                world.playSound(loc, Sound.BLOCK_FIRE_EXTINGUISH, SoundCategory.PLAYERS, this.turboDouble.toFloat() * 0.125f, 0.5f + this.turboDouble.toFloat() * 0.35f)

                if (this.vehicle.nowGear != this.turboSlotChange) {
                    world.playSound(loc, Sound.ENTITY_ALLAY_DEATH, SoundCategory.PLAYERS, this.turboDouble.toFloat(), 1.75f)
        ...
        "Default3" -> {
            world.playSound(loc, Sound.ENTITY_HORSE_ANGRY, SoundCategory.PLAYERS, this.bossBar.progress.toFloat() * 0.75f + 0.2f, 0.8f + this.bossBar.progress.toFloat() * 1.2f)
            world.playSound(loc, Sound.BLOCK_BARREL_CLOSE, SoundCategory.PLAYERS, this.bossBar.progress.toFloat() * 0.45f + 0.2f, 0.5f + this.bossBar.progress.toFloat() * 0.4f)
            return
        }
    }

    world.playSound(loc, Sound.ENTITY_COW_STEP, SoundCategory.PLAYERS, this.bossBar.progress.toFloat() * 1.3f + 0.2f, 0.5f + this.bossBar.progress.toFloat() * 1.5f)
    ...
    world.playSound(loc, Sound.ENTITY_COW_STEP, SoundCategory.PLAYERS, 0.15f + speedVolumeBase, 0.5f + speedPitchBase)
    world.playSound(loc, Sound.ENTITY_HORSE_STEP_WOOD, SoundCategory.PLAYERS, 0.375f + speedVolumeBase, 0.5f + speedPitchRpm)
}
