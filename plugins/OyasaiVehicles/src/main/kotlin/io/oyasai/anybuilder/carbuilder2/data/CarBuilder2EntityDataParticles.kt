...
fun CarBuilder2EntityData.smokeParticleImpl(
    speed: Double,
    slipAngle: Float,
    yaw: Float,
    player: Player?
  ...
  fun spawnWheelSmoke(type: CarBuilder2EntityType) {
    this.display[type]?.forEach { (offset, _) ->
      if (offset.second.z <= 0.0) {
        val loc =
            this.bodyArmorStands[0]
                .location
                .clone()
                .add(Vector(offset.second.x, 0.0, offset.second.z).rotateAroundY(yawRad))
        world.spawnParticle(
            Particle.DUST,
            loc,
            totalCount,
      ...
      for (display in displaySet) {
        ...
        if (type == CarBuilder2EntityType.Body) {
          ...
          if (winkerMaterials.contains(material)) {
            val localX =
                this.displayDef[type]?.get(display)?.translation?.x
