...
fun CarBuilder2Command.handleCarBuilder2Settings(
  ...
  val needsManage =
      subCommand in setOf("delete", "size", "offset", "headlight", "sound", "spring") ||
  ...
  when (subCommand) {
    ...
    "size" -> {
      ...
      val size = args[3].toDoubleOrNull() ?: oldSize
...
private fun applyCarBuilder2Offset(
  ...
  when (partTypeName) {
    ...
    "headlight" -> {
      ...
      if (offset == null) config.clearPartOffset(partType, index)
      else config.setPartOffset(partType, index, offset)
    }
    else -> return false
  }
  return true
