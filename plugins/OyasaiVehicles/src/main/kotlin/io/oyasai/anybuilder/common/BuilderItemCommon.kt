...
import io.oyasai.toolbox.legacyLore
import org.bukkit.Material
import org.bukkit.inventory.ItemStack
import org.bukkit.inventory.meta.ItemMeta

object BuilderItemSupport {
  data class VehicleIntRule(val valueRegex: Regex, val max: Int, val min: Int = 1)

  data class LoreLineReplacement(val regex: Regex, val replacement: String)
  ...
  fun checkCustomModelMinecart(item: ItemStack?, customModelData: Int): Boolean {
    ...
    return meta.hasCustomModelDataComponent() &&
        meta.customModelDataComponent.floats.singleOrNull() == customModelData.toFloat()
  }
  ...
  fun readLoreValue(
    ...
    val line =
        if (fromEnd) lore.lastOrNull { regex.matches(it) }
        else lore.firstOrNull { regex.matches(it) }
  ...
  fun changeVehicleInt(
    ...
    for (line in lore) {
      ...
      } else if (costRegex.matches(line)) {} else {
