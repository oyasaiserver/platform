package io.oyasaiserver.anybuilder.aircraftbuilder.event
...
import io.oyasaiserver.anybuilder.common.event.VehicleEventListenerBase
...
object AircraftBuilderEvents : VehicleEventListenerBase() {
    ...
    override fun checkSeatPlayerUUID(uuid: UUID): Boolean = AircraftBuilderEntityList.checkSeatPlayerUUID(uuid)
    ...
    fun bodyArmorStandsDamageEvent(event: EntityDamageEvent) {
        handleArmorStandDamage(
            ...
            onEmptyDriverSeat = { data ->
                Bukkit.getLogger().info("[AircraftBuilder] Aircraft ${data.baseData.name} despawned due to ENTITY_ATTACK on empty driver seat.")
            }
        )
    }
    ...
    fun playerRightClickEvent(event: PlayerInteractEvent) {
        ...
        handleVehicleItemRightClickWithPermission(
            ...
            consumeItem = { clickEvent, _ ->
                clickEvent.player.inventory.itemInMainHand.amount = 0
            }
        )
    }
