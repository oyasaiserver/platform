...
import io.oyasaiserver.canRideCarBuilder2OrAdmin
import io.oyasaiserver.canUseCarBuilder2
...
// carbuilder2|cbmenu <name> eventcar <costLimit>
 ...
 val COMMAND_BLOCK_MATERIALS = setOf(
    Material.COMMAND_BLOCK,
    Material.CHAIN_COMMAND_BLOCK,
    Material.REPEATING_COMMAND_BLOCK
)

 val OFFSET_AUTO_SCALE_TYPES = listOf(
 ...
 val CarBuilder2_MANAGER_COMMANDS = setOf("delete", "save", "size", "offset", "headlight", "sound", "spring")
...
private fun handleCarBuilder2SingleArgumentCommand(sender: CommandSender, name: String, data: CarBuilder2BaseData?): Boolean {
    return CarBuilder2Command.spawnSingleArgumentTrial(
        ...
        spawnTrial = { player, baseData ->
            CarBuilder2Spawn.spawn(player.location, ItemStack(Material.HOPPER_MINECART), null, player, baseData)
        },
...
private fun handleCarBuilder2Spawn(sender: CommandSender, name: String, data: CarBuilder2BaseData?, args: Array<out String>) {
    if (data == null) return
    if (sender is Player) {
        ...
        val yaw = if (args.size > 2) Tools.getYawFromCardinalDirections(args[2]) else sender.location.yaw.toDouble()
        CarBuilder2Spawn.spawn(sender.location, ItemStack(Material.HOPPER_MINECART), null, sender, data, yaw.toFloat())
...
private fun handleCarBuilder2Buy(sender: CommandSender, name: String, data: CarBuilder2BaseData?) {
    CarBuilder2Command.handlePlayerPurchase(
        ...
        onPaymentFailure = {
            it.sendMessage("[CarBuilder2] Not enough points!")
        }
    )
}
