...
object CarBuilder2EntityList : BaseVehicleEntityList<CarBuilder2EntityData>() {
    ...
    fun getCarDataByMainArmorStand(uuid: UUID): CarBuilder2EntityData? = cache.firstOrNull { it.bodyArmorStands.firstOrNull()?.uniqueId == uuid }
    fun getBodyArmorStandList(uuid: UUID?): List<ArmorStand> = cache.mapNotNull { data -> data.bodyArmorStands.firstOrNull()?.takeIf { it.uniqueId != uuid } }
