...
import io.oyasai.anybuilder.common.VehicleEventListenerBase
...
object CarBuilder2Events : VehicleEventListenerBase() {
  ...
  fun bodyArmorStandsDamageEvent(event: EntityDamageEvent) {
    handleArmorStandDamage(
        event = event,
        isVehicleArmorStand = CarBuilder2EntityList::checkCarArmorStandUUID,
        getData = CarBuilder2EntityList::getCarData,
  ...
  fun unloadEntity(event: EntitiesUnloadEvent) {
    handleEntityUnload(event) { uuid -> CarBuilder2EntityList.getCarData(uuid) }
  }
  ...
  fun playerArmorStandRightClick(event: PlayerInteractAtEntityEvent) {
    handleSeatArmorStandRightClickWithPermission(
        ...
        canRide = { it.canRideCarBuilder2OrAdmin() },
        deniedMessage = "[CarBuilder2] 実行権限がありません。",
        isSeatArmorStand = CarBuilder2EntityList::checkCarSeatArmorStand,
  ...
  fun playerQuitEvent(event: PlayerQuitEvent) {
    handleVehicleQuit(event, { uuid -> CarBuilder2EntityList.getCarData(uuid) }) { data, playerUUID
      ->
