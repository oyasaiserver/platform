package io.oyasaiserver.anybuilder.carbuilder2.event
...
object CarBuilder2Events : VehicleEventListenerBase() {
    ...
    override fun checkSeatPlayerUUID(uuid: UUID): Boolean = CarBuilder2EntityList.checkSeatPlayerUUID(uuid)
    ...
    fun bodyArmorStandsDamageEvent(event: EntityDamageEvent) {
        handleArmorStandDamage(
            ...
            onEmptyDriverSeat = { data ->
                Bukkit.getLogger().info("[CarBuilder2] Car ${data.baseData.name} despawned due to ENTITY_ATTACK on empty driver seat.")
            }
        )
    }
