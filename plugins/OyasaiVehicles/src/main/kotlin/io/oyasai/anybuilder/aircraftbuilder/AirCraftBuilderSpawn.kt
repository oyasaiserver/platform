...
object AircraftBuilderSpawn {
  private val oldSpawnCarData: MutableMap<UUID, AircraftBuilderEntityData> = LinkedHashMap()
    ...
    fun spawn(loc: Location, item: ItemStack, owner: Player?, spawnPlayer: Player?, data: AircraftBuilderBaseData, yaw: Float = loc.yaw): Boolean {
        ...
        if (data.totalEntity() > entityLimit && !(spawnPlayer?.isOp ?: false)) {
            limitSender.sendMessage("[AircraftBuilder] エンティティ数が${entityLimit}を超えています (${data.totalEntity()})")
      return false
    } else {
      ...
      val blockDisplayMap = LinkedHashMap<Pair<Int, Vector>, MutableSet<BlockDisplay>>()
        blockDisplayMap[Pair(0, Vector().zero())] =
            BuilderSpawnSupport.spawnBlockDisplay(loc, bodyArmorStandList, data.body, data.config.getBodySize(), data.boxSize)

        val cEntity = AircraftBuilderEntityData(
        seatMap,
        bodyArmorStandList,
        blockDisplayMap,
