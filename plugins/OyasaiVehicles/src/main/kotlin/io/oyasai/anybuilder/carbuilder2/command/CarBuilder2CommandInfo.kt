...
fun handleCarBuilder2Info(sender: CommandSender, name: String, data: CarBuilder2BaseData?) {
    ...
    listOf(CarBuilder2BaseDataType.Wheel, CarBuilder2BaseDataType.Wheel2, CarBuilder2BaseDataType.HeadLight).forEach { type ->
        ...
        if (type == CarBuilder2BaseDataType.HeadLight) {
            ...
            info.add("  RotateOffset: ${BuilderInfoCommon.formatVector(config.getHeadLightRotateOffset())}")
        }
    }

    info.add("[Seats]")
    config.getSeatList().toList().sortedBy { it.first }.forEach { (index, vector) ->
        info.add("  #$index: ${BuilderInfoCommon.formatVector(vector)}")
    }
    info.add("[Vehicle]")
