...
class CarBuilder2BaseData(name: String) : BaseVehicleBaseDataCore<CarBuilder2Config>(
    name,
  CarBuilder2Config("CarBuilder2/$name/$name.yml"),
    ...
    fun totalWheelEntity(): Int {
        return getEntityCount(CarBuilder2BaseDataType.Wheel) * config.getOffset(CarBuilder2BaseDataType.Wheel).size +
                getEntityCount(CarBuilder2BaseDataType.Wheel2) * config.getOffset(CarBuilder2BaseDataType.Wheel2).size
    }

    fun totalHeadLightEntity(): Int {
        return getEntityCount(CarBuilder2BaseDataType.HeadLight) * config.getOffset(CarBuilder2BaseDataType.HeadLight).size
    }
    ...
    fun reloadData(type: CarBuilder2BaseDataType, we: WorldEdit?): Boolean {
        ...
        val raw: MutableMap<Vector, BlockData> = SchematicToData.fileToRawBlockData(vehicleFile("${name}_${type.configName.lowercase(Locale.ROOT)}.schem")) ?: return true
    ...
    private fun isHeadBlock(block: BlockData): Boolean {
        return when (block.getMaterial()) {
            ...
            Material.CREEPER_HEAD, Material.CREEPER_WALL_HEAD -> true
            else -> false
        }
    }
}
