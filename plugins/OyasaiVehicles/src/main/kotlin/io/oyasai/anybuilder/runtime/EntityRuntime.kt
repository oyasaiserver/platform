...
object EntityRuntime {
    ...
    private fun getHandleMethod(clazz: Class<*>): Method? = getHandleCache.getOrPut(clazz) {
        runCatching { clazz.getMethod("getHandle") }.getOrNull()
    }

    private fun getMoveMethod(clazz: Class<*>): Method? = methodCache.getOrPut(clazz) {
    ...
    private fun movePos(entity: Entity, loc: Location) {
        val handle = getHandleMethod(entity.javaClass)?.let {
            runCatching { it.invoke(entity) }.getOrNull()
        } ?: return

        val target = getMoveMethod(handle.javaClass) ?: run {
    ...
    fun setYawAndVelocity(
        ...
        for ((pair, seatOffset) in seatArmorStand) {
            ...
            val targetLocation = leadLocation.clone().add(
                seatOffset.clone().rotateAroundY(-Math.toRadians(setYaw.toDouble() + (addYaw.toDouble() * distance)))
            )
    ...
    fun getWASD(player: Player?): Pair<ControlKey, Vector>? {
        ...
        val horizontalAxis = when {
            movementInput.isLeft -> 1
            movementInput.isRight -> -1
            else -> 0
        }

        val controlKey = when (verticalAxis to horizontalAxis) {
