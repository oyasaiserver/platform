 ...
 * @param requiredPermission 指定した場合、このパーミッションを持つプレイヤーのみ表示・実行可 opOnly より優先される。
 * @param fallbackIcon 表示条件を満たさないプレイヤーに表示するアイコン。 null → ガラス (fillGlass) で埋める AIR → 強制的に空欄にする
 *   (ガラスも置かない) その他 → 指定マテリアルを表示
 * @param fallbackTexture fallbackIcon が PLAYER_HEAD (CUSTOM_HEAD) のときのテクスチャハッシュ
 * @param fallbackName fallbackIcon の表示名。デフォルト " " (空白)
 * @param fallbackLore fallbackIcon の説明文。権限なしプレイヤー向け説明などに使う。 空リストの場合は lore なし
 * @param fallbackActions 表示条件を満たさないプレイヤーがクリックした際に実行するアクション。 空リストの場合はクリック無効 (従来通り)
 */
