 ...
 *
 * config.yml: cooldown: click-ms: 200 # GUIクリックの最小間隔 (ms)。0 = 無効 command-ms: 300 # コマンド実行の最小間隔
 * (ms)。0 = 無効
 *
 * ■ 使用箇所 isClickOnCooldown → 各Engine の onInventoryClick で使用 isCommandOnCooldown → SellCommand
 * (/sell hand, /sell all) で使用 連続売却コマンドによるスパム防止
 */
object CooldownManager {
  ...
  /**
   * コマンド実行のクールダウンチェック。 現在の使用箇所: SellCommand (/sell hand, /sell all)
   *
   * @return true = クールダウン中 (無視すべき), false = 実行OK かつ時刻を記録
   */
