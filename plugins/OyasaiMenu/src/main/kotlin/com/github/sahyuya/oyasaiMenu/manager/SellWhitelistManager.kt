 ...
 *
 * ■ カスタムアイテム判定の修正
 * - WRITTEN_BOOK (記入済みの本) は hasDisplayName() が false でも BookMeta の title/author
 *   で識別されるため「カスタムアイテム」として扱う
 * - より厳密なコンポーネント比較を実装: type / custom_name / enchantments / stored_enchantments / book_title+author
 *   / potion_type を比較
 *
 * ■ hasCustomContent() ヘルパー SellEngine から呼び出してカスタムアイテムかどうかを判定する
 */
class SellWhitelistManager(private val plugin: OyasaiMenu) {
  ...
  private val file: File
    get() = File(plugin.dataFolder, "sell-whitelist.yml")
...
""",
    ...
    (yaml.getList("entries") ?: emptyList<Any>()).filterIsInstance<Map<*, *>>().forEach { map ->
  ...
  /**
   * このアイテムがホワイトリスト照合が必要な「カスタムアイテム」かどうかを返す。 バニラ名かつショップ登録済みならショップ価格で売れるため不要。
   *
  ...
  /** アイテムがホワイトリストに一致する場合の売却価格を返す。 耐久値は無視して比較する。 */
  ...
  fun addEntry(item: ItemStack, sellPrice: Double): String? {
    ...
    val itemDataB64 =
        runCatching { Base64.getEncoder().encodeToString(normalized.serializeAsBytes()) }
            .getOrElse {
              return "アイテムのシリアライズに失敗しました: ${it.message}"
            }
  ...
  /** 比較用正規化: 耐久値0・個数1・loreクリア ただしカスタム名・エンチャント・本コンテンツは保持する */
  ...
  private fun matchesComponents(a: ItemStack, b: ItemStack): Boolean {
    ...
    if (aHasName && bHasName) {
      if (plain.serialize(ameta.displayName()!!) != plain.serialize(bmeta.displayName()!!))
          return false
    }
