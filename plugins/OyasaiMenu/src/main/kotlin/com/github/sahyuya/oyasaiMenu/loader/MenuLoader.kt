...
class MenuLoader(private val plugin: OyasaiMenu) {
  ...
  private fun parseItemDefinition(section: ConfigurationSection, key: String): MenuItemDefinition {
    val base: MenuItemDefinition =
        section.getString("extends")?.let { tid ->
          templates[tid]
              ?: MenuItemDefinition(slot = 0).also {
    ...
    val actions =
        if (section.contains("actions")) parseActions(section, "actions") else base.actions
