 ...
 * /menuedit
 *
 * /menuedit announce title|line|remove-line|book|show /menuedit shop <category> list|add|remove
...
class MenuAdminCommand(private val plugin: OyasaiMenu) : BasicCommand {
  ...
  override fun execute(source: CommandSourceStack, args: Array<out String>) {
    val player =
        source.sender as? Player
            ?: run {
              source.sender.sendMessage("§cこのコマンドはゲーム内から実行してください。")
              return
            }
  ...
  private fun handleShop(player: Player, args: Array<String>) {
    ...
    when (args.getOrNull(1)?.lowercase()) {
      ...
      "add" -> {
        if (args.size < 5) {
          player.sendMessage(c("&c使い方: /menuedit shop <category> add <material> <buy> <sell>"))
          return
        }
        val matName = args[2].uppercase()
        val buyPrice =
            args[3].toDoubleOrNull()
                ?: run {
                  player.sendMessage(c("&c買値が不正です。"))
                  return
                }
        val sellPrice =
            args[4].toDoubleOrNull()
                ?: run {
                  player.sendMessage(c("&c売値が不正です。"))
                  return
                }
        runCatching { org.bukkit.Material.valueOf(matName) }
            .getOrElse {
      ...
      "remove" -> {
        ...
        if (index == null || index < 0 || index >= category.items.size) {
          player.sendMessage(c("&c番号は 1〜${category.items.size} で指定してください。"))
          return
        }
        val removed = plugin.shopLoader.removeItem(categoryId, index)
        if (removed != null) {
          plugin.shopLoader.reload()
          player.sendMessage(c("&a${index+1}番目 ($removed) を削除しました。"))
  ...
  private fun handleWhitelist(player: Player, args: Array<String>) {
    when (args.getOrNull(0)?.lowercase()) {
      "list" -> {
        ...
        if (entries.isEmpty()) {
          player.sendMessage(c("&7売却ホワイトリストは空です。"))
          return
        }
      ...
      "add" -> {
        ...
        if (target != "hand" && target != "h") {
          player.sendMessage(c("&c使い方: /menuedit whitelist add hand/h <売値>"))
          return
        }
        val sellPrice = args.getOrNull(2)?.toDoubleOrNull()
        if (sellPrice == null || sellPrice <= 0) {
          player.sendMessage(c("&c売値に正の数値を指定してください。"))
          return
        }
        val item = player.inventory.itemInMainHand
        if (item.type.isAir) {
          player.sendMessage(c("&c手にアイテムを持ってください。"))
          return
        }
        ...
        if (err != null) {
          ...
          val dispName =
              if (hasCustomName && item.itemMeta?.hasDisplayName() == true)
      ...
      "remove" -> {
        ...
        if (index == null || index < 1) {
          player.sendMessage(c("&c使い方: /menuedit whitelist remove <番号>"))
          return
        }
      ...
      args.size == 3 ->
          when {
            ...
            args[0].equals("whitelist", ignoreCase = true) &&
                args[1].equals("add", ignoreCase = true) -> listOf("hand", "h")
            args[0].equals("whitelist", ignoreCase = true) &&
