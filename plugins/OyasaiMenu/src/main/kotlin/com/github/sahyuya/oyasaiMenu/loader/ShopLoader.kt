...
class ShopLoader(private val plugin: OyasaiMenu) {
  ...
  private val legacyEnchantNames: Map<String, String> =
      mapOf(
  ...
  fun removeItem(categoryId: String, index: Int): String? {
    ...
    val realIndices =
        rawList.indices.filter {
          rawList[it].trim().isNotEmpty() && !rawList[it].trim().startsWith("#")
        }
  ...
  private fun shopsFile(): File =
      File(plugin.dataFolder, "menus/shop/shops.yml").also {
  ...
  private fun loadCustomItems() {
    customItems.clear()
    val file =
        File(plugin.dataFolder, "menus/shop/custom_items.yml").also {
          it.parentFile.mkdirs()
          if (!it.exists()) {
    ...
    yaml.getKeys(false).forEach { key ->
      ...
      val mat =
          runCatching { Material.valueOf(matName) }
              .getOrElse {
  ...
  private fun resolveEnchantment(name: String): Enchantment? {
    ...
    return runCatching { Registry.ENCHANTMENT.get(NamespacedKey.minecraft(key)) }
        .getOrElse { @Suppress("DEPRECATION") Enchantment.getByName(name.uppercase()) }
  }
  ...
  private fun parseCustomItemLine(line: String, catId: String): ShopItem? {
    ...
    if (parts.size < 3) {
      plugin.logger.warning("カスタムアイテム フォーマットエラー ($catId): '$line'")
      return null
    }
  ...
  private fun parseItemLine(line: String, catId: String): ShopItem? {
    val parts = line.split("\\s+".toRegex())
    if (parts.size < 3) {
      plugin.logger.warning("フォーマットエラー ($catId): '$line'")
      return null
    }
    val buyPrice = parts[1].toDoubleOrNull() ?: return null
    val sellPrice = parts[2].toDoubleOrNull() ?: return null
    val material =
        runCatching { Material.valueOf(parts[0].uppercase()) }
            .getOrElse {
              plugin.logger.warning("不明なマテリアル: ${parts[0]} ($catId)")
              null
            }
    return ShopItem(
        material = material,
