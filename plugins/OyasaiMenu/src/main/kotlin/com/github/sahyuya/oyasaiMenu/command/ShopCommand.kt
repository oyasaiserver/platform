...
class ShopCommand(private val plugin: OyasaiMenu) : CommandExecutor, TabCompleter {
  ...
  override fun onCommand(
    ...
    val category =
        plugin.shopLoader.getAllCategories().values.find {
          it.id.equals(resolved, ignoreCase = true) ||
              it.command?.equals(resolved, ignoreCase = true) == true
        }
    if (category == null) {
      player.sendMessage(c("&c'$input' が見つかりません。利用可能: &f${getSuggestions().joinToString(", ")}"))
      return false
    }
  ...
  private fun getSuggestions(): List<String> {
    ...
    names +=
        plugin.shopLoader.getAllCategories().values.flatMap { listOfNotNull(it.id, it.command) }
    names += "pointshop"
