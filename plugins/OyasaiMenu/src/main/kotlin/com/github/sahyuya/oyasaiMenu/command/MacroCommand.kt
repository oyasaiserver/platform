 ...
 * /macro
 *
 * /macro → マクロ一覧GUI /macro <id> → 指定IDのマクロを実行 /macro run <id> → 同上 (明示的) /macro <id> <番号> <コマンド> →
 * 指定インデックスにコマンドをセット /macro <id> <番号> remove → 指定インデックスのコマンドを削除 /macro share <id> → 共有IDを発行
 * (既存IDがあれば再表示も) /macro shares → 自分が発行した共有IDを一覧表示 /macro import <shareId> → 共有IDからマクロを取り込む
 */
    ...
    when {
      ...
      args.size >= 3 && args[1].toIntOrNull() != null -> {
        ...
        if (plugin.macroManager.getMacro(player.uniqueId, macroId) == null) {
          player.sendMessage("§cマクロ '§e$macroId§c' が見つかりません。")
          return
        }
      ...
      args.size == 2 -> player.sendMessage("§7使い方: §f/macro §e<id> §b<番号> §f<コマンド>")
