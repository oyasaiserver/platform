...
class MacroCommand(private val plugin: OyasaiMenu) : CommandExecutor, TabCompleter {
  ...
  override fun onCommand(
    ...
    when {
      ...
      args.size >= 1 && args[0].equals("shares", ignoreCase = true) -> handleShares(player)

      args.size >= 2 && args[0].equals("import", ignoreCase = true) -> handleImport(player, args[1])
      ...
      args.size >= 3 && args[1].toIntOrNull() != null -> {
        ...
        if (plugin.macroManager.getMacro(player.uniqueId, macroId) == null) {
          player.sendMessage(c("&cマクロ '&e$macroId&c' が見つかりません。"))
          return true
        }
        if (value.equals("remove", ignoreCase = true)) {
          val err = plugin.macroManager.removeCommandAtIndex(player.uniqueId, macroId, index)
          if (err != null) player.sendMessage(c("&c$err"))
          else player.sendMessage(c("&a'$macroId' の ${index+1} 番目を削除しました。"))
        } else {
          val err =
              plugin.macroManager.setCommandAtIndex(player.uniqueId, macroId, index, value, player)
          if (err != null) player.sendMessage(c("&c$err"))
          else player.sendMessage(c("&a'$macroId' の ${index+1} 番目を設定: &f$value"))
        }
      }

      args.size == 2 -> player.sendMessage(c("&7使い方: &f/macro &e<id> &b<番号> &f<コマンド>"))
      ...
      args.isEmpty() -> plugin.macroEngine.openMacroList(player)
        ...
        if (err != null) {
          ...
          player.sendMessage(c("&7GUIから管理: §e/macro"))
        }
      }
    }
    return true
  ...
  private fun handleShare(player: Player, macroId: String) {
    ...
    if (macro == null) {
      player.sendMessage(c("&cマクロ '$macroId' が見つかりません。"))
      return
    }
    ...
    if (shareId == null) {
      player.sendMessage(c("&c共有に失敗しました。"))
      return
    }
  ...
  private fun handleImport(player: Player, shareId: String) {
    ...
    if (shared == null) {
      player.sendMessage(c("&c共有ID '${shareId}' が見つかりません。"))
      return
    }
  ...
  override fun onTabComplete(
    ...
    return when (args.size) {
      0,
      1 ->
          (listOf("run", "share", "shares", "import") +
      ...
      3 ->
          when {
            args[1].toIntOrNull() != null ->
                listOf("wait 1s", "wait 0.5s", "remove").filter {
