 ...
 * - fallback_icon: AIR を指定すると強制的に空欄にする (null = ガラス埋め、AIR = 完全に空、CUSTOM_HEAD = カスタムヘッド、その他 =
 *   そのマテリアル)
...
class PopupMenuLoader(private val plugin: OyasaiMenu) {
  ...
  private fun loadFile(file: File, id: String): PopupMenuDef {
    ...
    val glass =
        runCatching { Material.valueOf(glassName.uppercase()) }
            .getOrDefault(Material.GRAY_STAINED_GLASS_PANE)
    ...
    itemsSec?.getKeys(false)?.forEach { key ->
      ...
      val iconName =
          sec.getString("icon", itemSpec?.material?.name ?: "STONE")?.uppercase() ?: "STONE"
      ...
      (sec.getList("actions") ?: emptyList<Any>()).filterIsInstance<Map<String, Any>>().forEach {
        parseAction(it)?.let { a -> actions.add(a) }
      }
  ...
  /**
   * ConfigurationSection を再帰的に Map<String, Any> に変換する。 YamlConfiguration の getValues(deep=true)
   * はドット区切りでフラット化するため、 このメソッドでネスト構造を維持したまま変換する。
   */
  ...
  private fun parseItemSpec(
    ...
    val material =
        runCatching { Material.valueOf(materialName) }
            .getOrElse {
    ...
    componentsSec?.getKeys(false)?.forEach { compKey ->
      ...
      val compData: Any =
          if (compSec != null) sectionToMap(compSec)
          else (componentsSec.get(compKey) ?: return@forEach)
  ...
  private fun parseAction(map: Map<String, Any>): PopupAction? =
      when {
        map.containsKey("player_cmd") ->
            PopupAction(PopupActionType.PLAYER_CMD, map["player_cmd"].toString())
        map.containsKey("console_cmd") ->
            PopupAction(PopupActionType.CONSOLE_CMD, map["console_cmd"].toString())
        map.containsKey("op_player_cmd") ->
            PopupAction(PopupActionType.OP_PLAYER_CMD, map["op_player_cmd"].toString())
        map.containsKey("url") -> PopupAction(PopupActionType.URL, map["url"].toString())
        map.containsKey("chat_paste") ->
            PopupAction(PopupActionType.CHAT_PASTE, map["chat_paste"].toString())
