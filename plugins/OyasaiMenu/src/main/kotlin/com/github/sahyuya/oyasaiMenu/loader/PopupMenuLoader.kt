 ...
 *
 * YAML 例: staff_channel: slot: 1 icon: COMMAND_BLOCK op_only: true fallback_icon:
 * GRAY_STAINED_GLASS_PANE # 非OPに表示するアイコン fallback_name: " " # 非OPに表示する名前 (省略可)
 */
  ...
  private fun loadFile(file: File, id: String): PopupMenuDef {
    ...
    itemsSec?.getKeys(false)?.forEach { key ->
      ...
      val fallbackIcon: Material? =
          fallbackIconName?.let {
            runCatching { Material.valueOf(it) }
                .getOrElse {
                  plugin.logger.warning("Popup $id '$key': 不明な fallback_icon '$it'")
                  null
                }
          }
