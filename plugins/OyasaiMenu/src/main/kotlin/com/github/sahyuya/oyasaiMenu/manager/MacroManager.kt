 ...
 * MacroManager
 *
 * ■ スラッシュの扱い 保存時: ユーザーが入力した内容をそのまま保存 (スラッシュを除去しない) 実行時: performCommand() の前に先頭スラッシュを1つだけ除去する →
 * `warp home` → performCommand("warp home") → /warp home → `/warp home` → performCommand("warp
 * home") → /warp home → `//wand` → performCommand("/wand") → //wand (FAWE)
 *
 * ■ リソースファイル op-macro-templates.yml は JAR 内リソースから saveResource() で展開する。 コード内にデフォルト内容を書かない。
 *
 * ■ 権限別マクロ最大数 config.yml の macro.max-per-permission で権限ごとに設定可能。
 */
class MacroManager(private val plugin: OyasaiMenu) {
  ...
  /** プレイヤーの権限に応じたマクロ最大数を返す。 複数権限を持つ場合は最大値を採用する。 */
  fun getMaxMacros(player: Player): Int {
    val default = plugin.config.getInt("macro.max-per-player", 10)
    val permSection =
        plugin.config.getConfigurationSection("macro.max-per-permission") ?: return default
  ...
  fun addMacro(uuid: UUID, macro: PlayerMacro, maxOverride: Int? = null): String? {
    ...
    validateCommands(macro.commands)?.let {
      return it
    }
    list.add(macro)
    savePlayer(uuid)
  ...
  fun setCommandAtIndex(uuid: UUID, macroId: String, index: Int, value: String): String? {
    ...
    validateCommands(listOf(value))?.let {
      return it
    }
  ...
  /**
   * OPプレイヤーにテンプレートマクロを配布する。 op-macro-templates.yml は JAR リソースから saveResource() で展開する。
  ...
  fun distributeOpTemplates(player: Player) {
    ...
    if (!templateFile.exists()) {
      runCatching { plugin.saveResource("op-macro-templates.yml", false) }
          .onFailure {
            plugin.logger.warning("op-macro-templates.yml の展開失敗: ${it.message}")
            return
          }
    }
   ...
   * コマンドを順に実行する。
   *
   * ■ スラッシュの扱い (実行時) performCommand() に渡す前に先頭スラッシュを1つだけ除去する。 `warp home` → performCommand("warp
   * home") → /warp home `/warp home` → performCommand("warp home") → /warp home `//wand` →
   * performCommand("/wand") → //wand (FAWE)
   */
