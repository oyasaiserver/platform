 ...
 *
 * ■ wait バリデーションの改善 エラーメッセージに秒数とtick数を両方表示し、入力例も添える。 秒→tick / tick→秒 の変換ヘルパーを専用関数に切り出してコードを整理した。
 *
 * ■ distributeOpTemplates ファイルが存在しない場合はデフォルト内容をプログラムで生成。
 */
class MacroManager(private val plugin: OyasaiMenu) {
  ...
  fun getMaxCommands(player: Player): Int {
    ...
    val permSection =
        plugin.config.getConfigurationSection("macro.max-commands-per-permission") ?: return default
    return permSection
        .getKeys(false)
        .filter { player.hasPermission(it) }
        .mapNotNull { permSection.getInt(it, 0).takeIf { v -> v > 0 } }
  ...
  fun addMacro(
    ...
    validateCommands(macro.commands, player)?.let {
      return it
    }
    list.add(macro)
    savePlayer(uuid)
  ...
  fun updateMacro(uuid: UUID, updated: PlayerMacro, player: Player? = null): String? {
    ...
    validateCommands(updated.commands, player)?.let {
      return it
    }
    list[idx] = updated
    savePlayer(uuid)
  ...
  fun setCommandAtIndex(
    ...
    validateCommands(commands, player)?.let {
      return it
    }
  ...
  private fun validateCommands(commands: List<String>, player: Player? = null): String? {
    ...
    val blocked =
        commands
            .filter { !isWaitCommand(it) && it.trim().startsWith("/") }
            .filter { cmd ->
              val base =
                  cmd.trimStart().removePrefix("/").removePrefix("/").split(" ").first().lowercase()
              blacklist.any { entry ->
  ...
  private fun formatSeconds(seconds: Double): String {
    ...
    return if (seconds == long.toDouble()) long.toString()
    else seconds.toBigDecimal().stripTrailingZeros().toPlainString()
  }
