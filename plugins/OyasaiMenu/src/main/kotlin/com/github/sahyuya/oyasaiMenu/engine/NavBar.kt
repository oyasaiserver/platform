...
object NavBar {
  ...
  fun slotForPopup(popupId: String): Int = entries.find { it.popupId == popupId }?.slot ?: -1
  ...
  fun buildPlayerHead(player: Player, plugin: OyasaiMenu): ItemStack {
    ...
    meta.lore(
        listOf(
            ...
            comp(
                "&7所持金: &f${if (EconomyManager.isAvailable) EconomyManager.format(EconomyManager.getBalance(player)) else "---"}"),
            comp(
                "&7ポイント: &f${if (TokenCurrencyManager.isAvailable) "${TokenCurrencyManager.format(TokenCurrencyManager.getTokens(player))}P" else "---"}"),
            comp(""),
            comp("&eクリックで更新")))
    skull.itemMeta = meta
    return skull
