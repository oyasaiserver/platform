 ...
 *
 * TokenManager のメインクラスは JavaPlugin を直接継承しており、 Bukkit.getPluginManager().getPlugin("TokenManager")
 * で取得できるが、 getTokens() の引数は OfflinePlayer ではなく Player を受け取るバージョンと OfflinePlayer を受け取るバージョンが混在する。
 *
 * さらに TokenManager は内部でデータキャッシュを持っており、 オンラインプレイヤーの場合は PlayerData 経由でデータを取得する。 getPlugin() が返す
 * Plugin インスタンスに対して直接 getTokens() を呼ぶのが正しい。
 *
 * 試行順:
...
object TokenCurrencyManager {
   ...
   * プレイヤーのトークン所持数を返す。
   *
   * TokenManager 未導入 / 取得失敗の場合は 0L。 fine ログに詳細を出力するのでデバッグ時は確認すること。
   */
  ...
  fun removeTokens(player: Player, amount: Long): String? {
    ...
    val tm =
        Bukkit.getPluginManager().getPlugin("TokenManager") ?: return "&cTokenManager が見つかりません。"
    ...
    runCatching {
      ...
      val m =
          tm.javaClass.getMethod("setTokens", Player::class.java, Long::class.javaPrimitiveType!!)
