 ...
 * - findSharesForMacro() → 特定マクロ名に紐づく自分の共有IDを返す これにより「/macro shares」や詳細画面の「共有コード確認」ボタンが実現できる
 *
 * ■ ファイル構造 shared-macros/<shareId>.yml share_id: "abc12345" macro_name: "ホーム設定" description: "作者:
 * sahyuya" author: "sahyuya" author_uuid: "<uuid>" created_at: 1234567890 commands:
 * - "warp home"
 */
class SharedMacroManager(private val plugin: OyasaiMenu) {
  ...
  /** 指定プレイヤーが共有したマクロを全件返す。 新しい順 (createdAt 降順) にソート。 */
    ...
    return shareDir
        ...
        ?.mapNotNull { file ->
          runCatching {
                ...
                SharedMacro(
                    shareId =
                        yaml.getString("share_id", file.nameWithoutExtension)
  ...
  /** 指定プレイヤーが共有した中で、マクロ名が一致するものを返す。 同一マクロを複数回共有している場合は全て返す (新しい順)。 */
