...
public class ShutdownService {
  ...
  public static void safeShutdown(Plugin plugin) {
    try {
      ...
      Chat.console(
          Phrase.build(
              Phrase.DISABLE_SUCCESS, "CoreProtect v" + plugin.getDescription().getVersion()));
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
   ...
   */
  private static void waitForPendingOperations(long shutdownTime, long nextAlertTime)
    ...
    while ((Consumer.isRunning() || ConfigHandler.converterRunning)
      ...
      if (currentTime >= nextAlertTime) {
        if (!ConfigHandler.converterRunning) {
          ...
          int consumerCount =
              Consumer.getConsumerSize(consumerId) + Process.getCurrentConsumerSize();
        ...
        nextAlertTime = currentTime + ALERT_INTERVAL_MS;
          ...
          && (currentTime - shutdownTime) >= DB_UNREACHABLE_TIMEOUT_MS) {
