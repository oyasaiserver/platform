 ...
 *
 * <p>This class provides methods for interacting with the CoreProtect database, including lookups,
 * rollbacks, and logging operations.
 */
public class CoreProtectAPI extends Queue {
  ...
  public boolean hasPlaced(String user, Block block, int time, int offset) {
    ...
    for (String[] value : check) {
      ...
      if (user.equalsIgnoreCase(result.getPlayer())
          && result.getActionId() == 1
  ...
  public boolean logInteraction(String user, Location location) {
    ...
    Queue.queuePlayerInteraction(
        user, location.getBlock().getState(), location.getBlock().getType());
    return true;
  }
  ...
  public boolean logRemoval(String user, BlockState blockState) {
    ...
    Queue.queueBlockBreak(
        user, blockState, blockState.getType(), blockState.getBlockData().getAsString(), 0);
    return true;
  }
  ...
  public boolean logRemoval(String user, Location location, Material type, byte data) {
    ...
    Queue.queueBlockBreak(
        user, location.getBlock().getState(), type, type.createBlockData().getAsString(), data);
    return true;
  }
  ...
  public List<String[]> performLookup(
    ...
    return processData(
        ...
        radius,
        radiusLocation,
        parseList(restrictBlocks),
        parseList(excludeBlocks),
        restrictUsers,
        excludeUsers,
        actionList,
        0,
   ...
   * @return List of results or null if the API is disabled
   * @deprecated Use {@link #performLookup(int, List, List, List, List, List, int, Location)}
   *     instead
   */
  @Deprecated
  public List<String[]> performLookup(
    ...
    return processData(
        user, time, radius, location, parseList(restrict), parseList(exclude), 0, 1, -1, -1, false);
  }
   ...
   * @param limitCount Maximum number of results to return
   * @return List of results or null if the API is disabled
   * @deprecated Use {@link #performPartialLookup(int, List, List, List, List, List, int, Location,
   *     int, int)} instead
   */
  @Deprecated
  public List<String[]> performPartialLookup(
   ...
   * @return List of results or null if the API is disabled
   * @deprecated Use {@link #performRestore(int, List, List, List, List, List, int, Location)}
   *     instead
   */
  @Deprecated
  public List<String[]> performRestore(
    ...
    return processData(
        user, time, radius, location, parseList(restrict), parseList(exclude), 1, 2, -1, -1, false);
  }
   ...
   * @return List of results or null if the API is disabled
   * @deprecated Use {@link #performRollback(int, List, List, List, List, List, int, Location)}
   *     instead
   */
  @Deprecated
  public List<String[]> performRollback(
    ...
    return processData(
        user, time, radius, location, parseList(restrict), parseList(exclude), 0, 2, -1, -1, false);
  }
  ...
  private List<String[]> processData(
      ...
      int radius,
      Location location,
      Map<Object, Boolean> restrictBlocksMap,
      Map<Object, Boolean> excludeBlocks,
      List<String> restrictUsers,
      List<String> excludeUsers,
      List<Integer> actionList,
      int action,
      ...
      if (connection != null) {
        ...
        if (lookup == 1) {
          ...
          if (!Bukkit.isPrimaryThread()) {
            ...
            result =
                Rollback.performRollbackRestore(
                    statement,
                    null,
                    uuids,
                    restrictUsers,
   ...
   * @param useLimit Whether to use pagination limits
   * @return List of results
   * @deprecated Use {@link #processData(int, int, Location, Map, Map, List, List, List, int, int,
  ...
  private List<String[]> processData(
      ...
      int radius,
      Location location,
      Map<Object, Boolean> restrictBlocks,
      Map<Object, Boolean> excludeBlocks,
      int action,
      int lookup,
      int offset,
