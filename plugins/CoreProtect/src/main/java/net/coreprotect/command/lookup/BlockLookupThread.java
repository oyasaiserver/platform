  ...
  public void run() {
    try (Connection connection = Database.getConnection(true)) {
      ...
      if (connection != null) {
        ...
        if (type == 8) {
          java.util.List<String> signData =
              SignMessageLookup.performLookup(
                  command.getName(), statement, blockState.getLocation(), player, page, limit);
          ...
          if (type == 7) {
            blockdata =
                InteractionLookup.performLookup(
                    command.getName(), statement, block, player, 0, page, limit);
          } else {
            blockdata =
                BlockLookup.performLookup(
                    command.getName(), statement, blockState, player, 0, page, limit);
          }
