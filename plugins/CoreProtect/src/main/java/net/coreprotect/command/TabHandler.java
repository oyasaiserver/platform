...
public class TabHandler implements TabCompleter {
  ...
  // private static String[] COMMANDS = new String[] { "help", "inspect", "rollback", "restore",
  // "lookup", "purge", "reload", "status", "near", "undo" }; // max 10!
  ...
  private static final String[] PARAMS =
      new String[] {"user:", "time:", "radius:", "action:", "include:", "exclude:", "#container"};
  ...
  public List<String> onTabComplete(
    ...
    if (isActionParam(lastArg, currentArg) && hasLookupPermission(sender)) {
      ...
      return handleSecondArgCompletions(sender, argument0, args[1], paramState);
        ...
        && argument0.equals("purge")
        && sender.hasPermission("coreprotect.purge")) {
      return handlePurgeThirdArgCompletions(args[1], args[2]);
    } else if (hasLookupCommand(argument0, sender)
        && (!argument0.equals("l") && !argument0.equals("lookup") || !paramState.hasPage)) {
  ...
  private void addCompletionIfPermitted(
      CommandSender sender, String permission, String completion, List<String> completions) {
  ...
  private boolean hasLookupPermission(CommandSender sender) {
    return sender.hasPermission("coreprotect.lookup")
        || sender.hasPermission("coreprotect.rollback")
        || sender.hasPermission("coreprotect.restore");
  }
  ...
  private boolean hasPagePermission(CommandSender sender) {
    return sender.hasPermission("coreprotect.lookup")
        || sender.hasPermission("coreprotect.lookup.near")
        || sender.hasPermission("coreprotect.inspect");
  }

  private boolean hasLookupCommand(String cmd, CommandSender sender) {
    return (sender.hasPermission("coreprotect.lookup") && (cmd.equals("l") || cmd.equals("lookup")))
        || (sender.hasPermission("coreprotect.rollback")
            && (cmd.equals("rollback") || cmd.equals("rb") || cmd.equals("ro")))
        || (sender.hasPermission("coreprotect.restore")
            && (cmd.equals("restore") || cmd.equals("rs") || cmd.equals("re")));
  }

  private boolean isActionParam(String lastArg, String currentArg) {
    return lastArg.equals("a:")
        || lastArg.equals("action:")
        || currentArg.startsWith("a:")
        || currentArg.startsWith("action:");
  }

  private boolean isUserParam(String lastArg, String currentArg) {
    return lastArg.equals("u:")
        || lastArg.equals("user:")
        || lastArg.equals("users:")
        || lastArg.equals("p:")
        || currentArg.startsWith("u:")
        || currentArg.startsWith("user:")
        || currentArg.startsWith("users:")
        || currentArg.startsWith("p:");
  }

  private boolean isTimeParam(String lastArg, String currentArg) {
    return lastArg.equals("t:")
        || lastArg.equals("time:")
        || currentArg.startsWith("t:")
  ...
  private boolean isRadiusParam(String lastArg, String currentArg) {
    return lastArg.equals("r:")
        || lastArg.equals("radius:")
        || currentArg.startsWith("r:")
        || currentArg.startsWith("radius:");
  }

  private boolean isMaterialParam(String lastArg, String currentArg) {
    return lastArg.equals("i:")
        || lastArg.equals("include:")
        || lastArg.equals("item:")
        || lastArg.equals("items:")
        || lastArg.equals("b:")
        || lastArg.equals("block:")
        || lastArg.equals("blocks:")
        || currentArg.startsWith("i:")
        || currentArg.startsWith("include:")
        || currentArg.startsWith("item:")
        || currentArg.startsWith("items:")
        || currentArg.startsWith("b:")
        || currentArg.startsWith("block:")
        || currentArg.startsWith("blocks:")
        || lastArg.equals("e:")
  ...
  private ParamState getParamState(String[] args) {
    ...
    if (ConfigHandler.lookupType.get(args[0]) != null
        && ConfigHandler.lookupPage.get(args[0]) != null) {
  ...
  private List<String> handleActionParamCompletions(String currentArg, String lastArg) {
    ...
    return StringUtil.copyPartialMatches(
        filter + arg, completions, new ArrayList<>(completions.size()));
  }
  ...
  private List<String> handleRadiusParamCompletions(
      String currentArg, String lastArg, String baseCommand) {
    String filter = lastArg;
    String arg = "";
    if (currentArg.contains(":")) {
      String[] split = currentArg.split(":", 2);
  ...
  private List<String> handleSecondArgCompletions(
      CommandSender sender, String cmd0, String cmd1, ParamState paramState) {
    ...
    if (argument0.equals("help") && sender.hasPermission("coreprotect.help")) {
      ...
      return StringUtil.copyPartialMatches(
          argument1, completions, new ArrayList<>(completions.size()));
      ...
      List<String> completions =
          new ArrayList<>(filterParams(true, argument0, argument1, paramState));
      completions.addAll(
  ...
  private List<String> handlePurgeThirdArgCompletions(String arg1, String arg2) {
    ...
    if (argument1.startsWith("t:")) {
      ...
      return StringUtil.copyPartialMatches(
          argument2, completions, new ArrayList<>(completions.size()));
  ...
  private List<String> handleGenericLookupCompletions(
      String cmd, String currentArg, ParamState paramState) {
    ArrayList<String> params = filterParams(false, cmd, currentArg, paramState);
    List<String> completions = new ArrayList<>(params);
    return StringUtil.copyPartialMatches(
        currentArg, completions, new ArrayList<>(completions.size()));
  }

  private ArrayList<String> filterParams(
      boolean firstParam, String lastArgument, String argument, ParamState state) {
    ArrayList<String> params = new ArrayList<>();
    for (String param : PARAMS) {
      if (param.equals("user:") && !state.hasUser) {
        ...
        params.add(param);
      } else if (param.equals("#container")
          && !state.hasContainer
          && !state.hasRadius
    ...
    if (firstParam
        && state.pageLookup
        && (lastArgument.equals("l") || lastArgument.equals("lookup"))) {
      params.add("page:");
