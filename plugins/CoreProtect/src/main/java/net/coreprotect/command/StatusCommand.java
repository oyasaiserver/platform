        ...
        try {
          ...
          Chat.sendMessage(
              ...
              Color.WHITE
                  ...
                  + Color.DARK_AQUA
                  + "CoreProtect"
                  + (VersionUtils.isCommunityEdition() ? " " + ConfigHandler.COMMUNITY_EDITION : "")
                  + Color.WHITE
                  + " -----");
          Chat.sendMessage(
              player,
          ...
          if (donationKey != null) {
            Chat.sendMessage(
                ...
                Color.DARK_AQUA
                    + Phrase.build(
                        Phrase.STATUS_LICENSE, Color.WHITE, Phrase.build(Phrase.VALID_DONATION_KEY))
                    + " ("
                    + donationKey
                    + ")");
          } else {
          ...
          */
          ...
          try {
            ...
            String usedRAM =
                String.format(
                    "%.2f",
                    Double.valueOf((runtime.totalMemory() - runtime.freeMemory()) / mb) / 1000.0);
            String totalRAM =
                String.format("%.2f", Double.valueOf(runtime.maxMemory() / mb) / 1000.0);
            ...
            Chat.sendMessage(
                player,
                Color.DARK_AQUA
                    + Phrase.build(Phrase.STATUS_SYSTEM, Color.WHITE, systemInformation));
          } catch (Exception e) {
            e.printStackTrace();
          }
          ...
          // Functions.sendMessage(player, Color.DARK_AQUA + Phrase.build(Phrase.LINK_DISCORD,
          // Color.WHITE + "www.coreprotect.net/discord/").replaceFirst(":", ":" + Color.WHITE));
          Chat.sendMessage(
              player,
              Color.DARK_AQUA
