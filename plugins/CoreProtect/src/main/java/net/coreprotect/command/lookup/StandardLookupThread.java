...
public class StandardLookupThread implements Runnable {
  ...
  public StandardLookupThread(
      CommandSender player,
      Command command,
      List<String> rollbackUsers,
      List<Object> blockList,
      Map<Object, Boolean> excludedBlocks,
  ...
  public void run() {
    try (Connection connection = Database.getConnection(true)) {
      ConfigHandler.lookupThrottle.put(
          player.getName(), new Object[] {true, System.currentTimeMillis()});
      ...
      if (connection != null) {
        ...
        if (exists) {
          ...
          if (argWorldId > 0) {
            ...
            finalLocation =
                new Location(
                    Bukkit.getServer().getWorld(WorldUtils.getWorldName(argWorldId)), x, y, z);
          } else if (finalLocation != null) {
            finalLocation =
                new Location(
                    Bukkit.getServer().getWorld(WorldUtils.getWorldName(worldId)), x, y, z);
          }
          ...
          if (count) {
            ...
            Chat.sendMessage(
                ...
                Color.WHITE
                    ...
                    + Color.DARK_AQUA
                    + Phrase.build(
                        Phrase.LOOKUP_HEADER, "CoreProtect" + Color.WHITE + " | " + Color.DARK_AQUA)
                    + Color.WHITE
                    + " -----");
            if (actions.contains(6) || actions.contains(7)) { // Chat/command
              for (String[] data : lookupList) {
                ...
                String timeago =
                    ChatUtils.getTimeSince(Integer.parseInt(time), unixtimestamp, true);
                Chat.sendComponent(
                    player,
                    timeago
                        + " "
              ...
              for (String[] data : lookupList) {
                ...
                int timeLength =
                    50
                        + (ChatUtils.getTimeSince(Integer.parseInt(time), unixtimestamp, false)
                                .replaceAll("[^0-9]", "")
                                .length()
                            * 6);
                ...
                if (timeLength % 4 == 0) {
                  ...
                  leftPadding =
                      leftPadding + Color.WHITE + Strings.padStart("", (timeLength - 50) / 4, ' ');
                }
                ...
                Chat.sendComponent(
                    ...
                    Color.WHITE
                        ...
                        + Color.GREY
                        + "^ "
                        + ChatUtils.getCoordinates(
                            command.getName(), wid, dataX, dataY, dataZ, true, true));
                PluginChannelListener.getInstance()
                    .sendInfoData(
                        player,
                        Integer.parseInt(time),
              ...
              for (String[] data : lookupList) {
                ...
                Chat.sendComponent(
                    ...
                    timeago
                        ...
                        + "- "
                        + Phrase.build(
                            Phrase.LOOKUP_USERNAME,
                            Color.DARK_AQUA + user + Color.WHITE,
                            Color.DARK_AQUA + username + Color.WHITE));
                PluginChannelListener.getInstance()
                    .sendUsernameData(player, Integer.parseInt(time), user, username);
              }
              ...
              for (String[] data : lookupList) {
                ...
                String rbd =
                    ((Integer.parseInt(data[8]) == 2 || Integer.parseInt(data[8]) == 3)
                        ? Color.STRIKETHROUGH
                        : "");
                String timeago =
                    ChatUtils.getTimeSince(Integer.parseInt(time), unixtimestamp, true);
                Material blockType =
                    ItemUtils.itemFilter(
                        MaterialUtils.getType(dtype), (Integer.parseInt(data[13]) == 0));
                String dname =
                    StringUtils.nameFilter(blockType.name().toLowerCase(Locale.ROOT), ddata);
                byte[] metadata =
                    data[11] == null ? null : data[11].getBytes(StandardCharsets.ISO_8859_1);
                ...
                if (daction == 2 || daction == 3) { // LOOKUP_ITEM
                  ...
                  tag = Color.RED + "-";
                } else if (daction == ItemLogger.ITEM_BREAK
                    || daction == ItemLogger.ITEM_DESTROY
                    || daction == ItemLogger.ITEM_CREATE) {
                ...
                } else if (daction == ItemLogger.ITEM_SELL
                    || daction == ItemLogger.ITEM_BUY) { // LOOKUP_TRADE
                ...
                Chat.sendComponent(
                    ...
                    timeago
                        ...
                        + Phrase.build(
                            ...
                            Color.DARK_AQUA + rbd + dplayer + Color.WHITE + rbd,
                            "x" + amount,
                            ChatUtils.createTooltip(Color.DARK_AQUA + rbd + dname, tooltip)
                                + Color.WHITE,
                            selector));
                PluginChannelListener.getInstance()
                    .sendData(
              ...
              for (String[] data : lookupList) {
                ...
                // ChatColors.DARK_AQUA+rbd+""+dplayer+" " + ChatColors.WHITE+rbd+""+a+" " +
                // ChatColors.DARK_AQUA+rbd+"#"+dtype+ChatColors.WHITE + ". " + ChatColors.GREY +
                // "(x"+x+"/y"+y+"/z"+z+")");
            ...
            if (rows > displayResults) {
              ...
              if (actions.contains(6)
                  || actions.contains(7)
                  || actions.contains(9)
                  || (actions.contains(4) && actions.contains(11))) {
                Chat.sendMessage(player, "-----");
              }
              Chat.sendComponent(
                  player, ChatUtils.getPageNavigation(command.getName(), page, total_pages));
            }
          } else if (rows > 0) {
            Chat.sendMessage(
                player,
    ...
    ConfigHandler.lookupThrottle.put(
        player.getName(), new Object[] {false, System.currentTimeMillis()});
  }
}
