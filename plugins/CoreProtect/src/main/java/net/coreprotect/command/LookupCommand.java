...
public class LookupCommand {
  public static void runCommand(
      CommandSender player, Command command, boolean permission, String[] args) {
    ...
    if (args[0].toLowerCase(Locale.ROOT).equals("page")
        && (args.length != 2 || !args[1].equals(args[1].replaceAll("[^0-9]", "")))) {
      Chat.sendMessage(
          player,
          Color.DARK_AQUA
              + "CoreProtect "
              + Color.WHITE
              + "- "
              + Phrase.build(Phrase.MISSING_PARAMETERS, Color.WHITE, "/co page <page>"));
      return;
    }
    ...
    if (argWid == -1) {
      ...
      Chat.sendMessage(
          player, new ChatMessage(Phrase.build(Phrase.WORLD_NOT_FOUND, worldName)).build());
      return;
    }
    ...
    if (argAction.contains(6)
        || argAction.contains(7)
        || argAction.contains(8)
        || argAction.contains(9)
    ...
    if (ConfigHandler.lookupThrottle.get(player.getName()) != null) {
      ...
      if ((boolean) lookupThrottle[0]
          || ((System.currentTimeMillis() - (long) lookupThrottle[1])) < 50) {
        Chat.sendMessage(
            player,
            Color.DARK_AQUA
                + "CoreProtect "
    ...
    boolean allPermission =
        args[0].equals("near") && player.hasPermission("coreprotect.lookup.near");
    if (!allPermission) {
      if (!pageLookup
          && (argAction.size() == 0
      ...
      if (argAction.contains(4)
          ...
          && !player.hasPermission("coreprotect.lookup.container")) {
        Chat.sendMessage(
            player,
            Color.DARK_AQUA
                + "CoreProtect "
      ...
      if (argAction.contains(4)
          && argAction.contains(11)
          && !player.hasPermission("coreprotect.lookup.inventory")) {
        Chat.sendMessage(
            player,
            Color.DARK_AQUA
                + "CoreProtect "
    ...
    if (startTime <= 0
        && !pageLookup
        && type == 4
        && (argBlocks.size() > 0 || argUsers.size() > 0)) {
      Chat.sendMessage(
          player,
          Color.DARK_AQUA
              + "CoreProtect "
    ...
    if (type == 1) {
      ...
      if (argAction.contains(6)
          || argAction.contains(7)
          || argAction.contains(9)
          || (argAction.contains(4) && argAction.contains(11))) {
        re = 7;
      }
      if (parseRows > 0) {
        re = parseRows;
      ...
      if (g == 1
            ...
            if (ruser.startsWith("#")) {
              Chat.sendMessage(
                  ...
                  Color.DARK_AQUA
                      ...
                      + Color.WHITE
                      + "- "
                      + Phrase.build(Phrase.INVALID_USERNAME, ruser));
              return;
            }
          }
        }
        ...
        if (type == 5) {
          ...
          if (rollbackusers.size() == 1
              && rollbackusers.contains("#global")
              && argAction.contains(9)) {
            Chat.sendMessage(
                player,
                Color.DARK_AQUA
