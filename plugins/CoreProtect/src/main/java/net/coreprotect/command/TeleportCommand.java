  ...
  protected static void runCommand(CommandSender player, boolean permission, String[] args) {
    ...
    if (!(player instanceof Player)) {
      Chat.sendMessage(
          ...
          Color.DARK_AQUA
              ...
              + Color.WHITE
              + "- "
              + Phrase.build(Phrase.TELEPORT_PLAYERS));
      return;
    }
    ...
    if (ConfigHandler.teleportThrottle.get(player.getName()) != null) {
      ...
      if ((boolean) lookupThrottle[0]
          || ((System.currentTimeMillis() - (long) lookupThrottle[1])) < 500) {
        Chat.sendMessage(
            player,
            Color.DARK_AQUA
                + "CoreProtect "
                + Color.WHITE
                + "- "
                + Phrase.build(Phrase.COMMAND_THROTTLED));
        return;
      }
    }
    ...
    ConfigHandler.teleportThrottle.put(
