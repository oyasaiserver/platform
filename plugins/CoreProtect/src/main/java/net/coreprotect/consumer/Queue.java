...
public class Queue {
  ...
  private static synchronized void queueStandardData(
      int consumerId, int currentConsumer, String[] user, Object object) {
    ...
    Consumer.consumer_id.put(
        currentConsumer, new Integer[] {Consumer.consumer_id.get(currentConsumer)[0], 0});
  }

  protected static void queueAdvancedBreak(
      String user,
      BlockState block,
      Material type,
      String blockData,
    ...
    addConsumer(
        currentConsumer,
        new Object[] {
          consumerId, Process.BLOCK_BREAK, type, data, breakType, 0, blockNumber, blockData
        });
  ...
  protected static void queueArtInsert(int id, String name) {
    ...
    int consumerId = Consumer.newConsumerId(currentConsumer);
    addConsumer(
        currentConsumer, new Object[] {consumerId, Process.ART_INSERT, null, 0, null, 0, id, null});
    queueStandardData(consumerId, currentConsumer, new String[] {null, null}, name);
  }

  protected static void queueBlockBreak(
      String user, BlockState block, Material type, String blockData, int extraData) {
    queueBlockBreak(user, block, type, blockData, null, extraData, 0);
  }

  protected static void queueBlockBreakValidate(
      final String user,
      final Block block,
  ...
  protected static void queueBlockBreak(
    ...
    if (type == Material.SPAWNER && block instanceof CreatureSpawner mobSpawner) { // Mob spawner
      extraData = EntityUtils.getSpawnerType(mobSpawner.getSpawnedType());
        ...
        || BlockGroup.DOORS.contains(type)
        ...
        || type.equals(Material.PEONY)) { // Double plant
        ...
        if (((Bisected) block.getBlockData()).getHalf().equals(Half.TOP)) {
          ...
          if (block.getY() > BukkitAdapter.ADAPTER.getMinHeight(block.getWorld())) {
            block =
                block
                    .getWorld()
                    .getBlockAt(block.getX(), block.getY() - 1, block.getZ())
                    .getState();
    ...
    addConsumer(
        currentConsumer,
        new Object[] {
          consumerId, Process.BLOCK_BREAK, type, extraData, breakType, 0, blockNumber, blockData
        });
    queueStandardData(consumerId, currentConsumer, new String[] {user, null}, block);
  }

  protected static void queueBlockPlace(
      String user,
      BlockState blockLocation,
      Material blockType,
      BlockState blockReplaced,
      Material forceType,
      int forceD,
      int forceData,
      String blockData) {
    ...
    if (type == Material.SPAWNER
        && blockLocation instanceof CreatureSpawner mobSpawner) { // Mob spawner
    ...
    if (blockReplaced != null) {
      ...
      if ((replaceType == Material.IRON_DOOR
              || BlockGroup.DOORS.contains(replaceType)
              || replaceType.equals(Material.SUNFLOWER)
              || replaceType.equals(Material.LILAC)
              || replaceType.equals(Material.TALL_GRASS)
              || replaceType.equals(Material.LARGE_FERN)
              || replaceType.equals(Material.ROSE_BUSH)
              || replaceType.equals(Material.PEONY))
          && replaceData >= 8) { // Double plant top half
        BlockState blockBelow =
            blockReplaced
                .getWorld()
                .getBlockAt(blockReplaced.getX(), blockReplaced.getY() - 1, blockReplaced.getZ())
                .getState();
        Material belowType = blockBelow.getType();
        Queue.queueBlockBreak(
            user, blockBelow, belowType, blockBelow.getBlockData().getAsString(), 0);
      }
    }
  ...
  protected static void queueBlockPlaceDelayed(
      final String user,
      final Location placed,
      final Material type,
      final String blockData,
    ...
    Scheduler.scheduleSyncDelayedTask(
        ...
        () -> {
          try {
            queueBlockPlace(
                user, placed.getBlock().getState(), type, replaced, null, -1, 0, blockData);
          } catch (Exception e) {
            e.printStackTrace();
          }
        },
  ...
  protected static void queueBlockPlaceValidate(
      final String user,
      final BlockState blockLocation,
      final Block block,
      final BlockState blockReplaced,
      final Material forceT,
      final int forceD,
      final int forceData,
      final String blockData,
      int ticks) {
          ...
          try {
            ...
            if (blockType.equals(forceT)) {
              ...
              if (Config.getConfig(blockLocation.getWorld()).BLOCK_MOVEMENT) {
                blockStateLocation =
                    BlockUtil.gravityScan(blockLocation.getLocation(), blockType, user).getState();
              }
  ...
  protected static void queueBlockGravityValidate(
      final String user,
        ...
        () -> {
          try {
            ...
            if (!block.equals(placementBlock)) {
              queueBlockPlace(
                  user, placementBlock.getState(), blockType, null, blockType, -1, 0, null);
            }
          } catch (Exception e) {
            e.printStackTrace();
          }
  ...
  protected static void queueContainerBreak(
      String user, Location location, Material type, ItemStack[] oldInventory) {
    int currentConsumer = Consumer.currentConsumer;
    int consumerId = Consumer.newConsumerId(currentConsumer);
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.CONTAINER_BREAK, type, 0, null, 0, 0, null});
  ...
  protected static synchronized void queueContainerTransaction(
      String user, Location location, Material type, Object inventory, int chestId) {
    int currentConsumer = Consumer.currentConsumer;
    int consumerId = Consumer.newConsumerId(currentConsumer);
    addConsumer(
        currentConsumer,
  ...
  protected static void queueItemTransaction(
      String user, Location location, int time, int offset, int itemId) {
    int currentConsumer = Consumer.currentConsumer;
    int consumerId = Consumer.newConsumerId(currentConsumer);
    addConsumer(
        currentConsumer,
        new Object[] {
          consumerId, Process.ITEM_TRANSACTION, null, offset, null, time, itemId, null
        });
    queueStandardData(consumerId, currentConsumer, new String[] {user, null}, location);
  }
  ...
  protected static void queueEntityInsert(int id, String name) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.ENTITY_INSERT, null, 0, null, 0, id, null});
    queueStandardData(consumerId, currentConsumer, new String[] {null, null}, name);
  }

  protected static void queueEntityKill(
      String user, Location location, List<Object> data, EntityType type) {
    int currentConsumer = Consumer.currentConsumer;
    int consumerId = Consumer.newConsumerId(currentConsumer);
    addConsumer(
        currentConsumer, new Object[] {consumerId, Process.ENTITY_KILL, null, 0, null, 0, 0});
  ...
  protected static void queueEntitySpawn(String user, BlockState block, EntityType type, int data) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.ENTITY_SPAWN, null, 0, null, 0, data, null});
    queueStandardData(
        consumerId, currentConsumer, new String[] {user, null}, new Object[] {block, type});
  }
  ...
  protected static void queueMaterialInsert(int id, String name) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.MATERIAL_INSERT, null, 0, null, 0, id, null});
    queueStandardData(consumerId, currentConsumer, new String[] {null, null}, name);
  }
  ...
  protected static void queueBlockDataInsert(int id, String data) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.BLOCKDATA_INSERT, null, 0, null, 0, id, null});
    queueStandardData(consumerId, currentConsumer, new String[] {null, null}, data);
  }

  protected static void queueNaturalBlockBreak(
      String user, BlockState block, Block relative, Material type, String blockData, int data) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.NATURAL_BLOCK_BREAK, type, data, null, 0, 0, blockData});
  ...
  protected static void queuePlayerChat(Player player, String message, long timestamp) {
    ...
    int consumerId = Consumer.newConsumerId(currentConsumer);
    addConsumer(
        currentConsumer, new Object[] {consumerId, Process.PLAYER_CHAT, null, 0, null, 0, 0, null});
  ...
  protected static void queuePlayerCommand(Player player, String message, long timestamp) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.PLAYER_COMMAND, null, 0, null, 0, 0, null});
    Consumer.consumerStrings.get(currentConsumer).put(consumerId, message);
    queueStandardData(
        consumerId,
        currentConsumer,
  ...
  protected static void queuePlayerInteraction(String user, BlockState block, Material type) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.PLAYER_INTERACTION, type, 0, null, 0, 0, null});
    queueStandardData(consumerId, currentConsumer, new String[] {user, null}, block);
  }
  ...
  protected static void queuePlayerKill(String user, Location location, String player) {
    ...
    int consumerId = Consumer.newConsumerId(currentConsumer);
    addConsumer(
        currentConsumer, new Object[] {consumerId, Process.PLAYER_KILL, null, 0, null, 0, 0, null});
    queueStandardData(
        consumerId,
        currentConsumer,
        new String[] {user, null},
  ...
  protected static void queuePlayerLogin(
      Player player, int time, int configSessions, int configUsernames) {
    int currentConsumer = Consumer.currentConsumer;
    int consumerId = Consumer.newConsumerId(currentConsumer);
    ...
    addConsumer(
        currentConsumer,
        new Object[] {
          consumerId, Process.PLAYER_LOGIN, null, configSessions, null, configUsernames, time, null
        });
    Consumer.consumerStrings.get(currentConsumer).put(consumerId, uuid);
    queueStandardData(
        consumerId,
        currentConsumer,
        new String[] {player.getName(), uuid},
  ...
  protected static void queuePlayerQuit(Player player, int time) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.PLAYER_LOGOUT, null, 0, null, 0, time, null});
    queueStandardData(
        consumerId,
        currentConsumer,
        new String[] {player.getName(), null},
  ...
  protected static void queueRollbackUpdate(
      String user, Location location, List<Object[]> list, int table, int action) {
    if (location == null) {
  ...
  protected static void queueSignText(
      String user,
      Location location,
      int action,
      int color,
    ...
    */
    ...
    Consumer.consumerSigns
        .get(currentConsumer)
        .put(
            consumerId,
            new Object[] {
              colorSecondary,
              BlockUtils.getSignData(frontGlowing, backGlowing),
              isWaxed,
              isFront,
              line1,
              line2,
  ...
  protected static void queueSkullUpdate(String user, BlockState block, int rowId) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.SKULL_UPDATE, null, 0, null, 0, rowId, null});
    queueStandardData(consumerId, currentConsumer, new String[] {user, null}, block);
  }

  protected static void queueStructureGrow(
      String user, BlockState block, List<BlockState> blockList, int replacedListSize) {
    int currentConsumer = Consumer.currentConsumer;
    int consumerId = Consumer.newConsumerId(currentConsumer);
    addConsumer(
        currentConsumer,
        new Object[] {
          consumerId, Process.STRUCTURE_GROWTH, null, 0, null, 0, replacedListSize, null
        });
  ...
  protected static void queueWorldInsert(int id, String world) {
    ...
    addConsumer(
        currentConsumer,
        new Object[] {consumerId, Process.WORLD_INSERT, null, 0, null, 0, id, null});
