...
/**
 * Bukkit adapter implementation for Minecraft 1.21. Provides version-specific implementations for
 * the BukkitInterface to handle features introduced in the 1.21 update, including: - New block
 * types (crafter) - Registry handling for named objects - Updated interaction blocks
 */
public class Bukkit_v1_21 extends Bukkit_v1_20 implements BukkitInterface {

  /**
   * Initializes the Bukkit_v1_21 adapter with 1.21-specific block groups and mappings. Sets up
   * collections of blocks with similar behavior for efficient handling.
   */
  public Bukkit_v1_21() {
  ...
  /**
   * Initializes the block groups specific to Minecraft 1.21. Updates container, interaction, and
   * update state blocks with new 1.21 blocks.
   */
  ...
  /**
   * Initializes trapdoor blocks for interaction handling. Excludes iron trapdoors as they cannot be
  ...
  /**
   * Gets the EntityType corresponding to a Material. Maps Material to its equivalent EntityType for
   * entity handling.
   *
  ...
  /**
   * Gets a registry key string from a keyed object. Used for serializing objects that implement
   * Keyed.
   *
  ...
  /**
   * Gets a registry value from a key string and class. Used for deserializing registry objects.
   *
  ...
  public Object getRegistryValue(String key, Object tClass) {
    ...
    // return
    // RegistryAccess.registryAccess().getRegistry(RegistryKey.CAT_VARIANT).get((NamespacedKey)value);
    return Bukkit.getRegistry((Class) tClass).get(namespacedKey);
  }

  /**
   * Gets the wolf variant and adds it to the info list. This functionality is specific to Minecraft
   * 1.21+.
   *
  ...
  /**
   * Sets the wolf variant from the provided value. This functionality is specific to Minecraft
   * 1.21+.
   *
   * @param wolf The wolf entity
