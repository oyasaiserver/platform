...
public class ItemMetaHandler {
  ...
  public static List<List<Map<String, Object>>> serialize(
      ItemStack item, Material type, String faceData, int slot) {
        ...
        if (subMeta.hasChargedProjectiles()) {
          ...
          for (ItemStack chargedProjectile : subMeta.getChargedProjectiles()) {
            Map<String, Object> itemMap =
                ItemUtils.serializeItemStack(chargedProjectile, null, slot);
            if (itemMap.size() > 0) {
              list.add(itemMap);
            }
          }
  ...
  private static void deserializeFireworkEffect(
      FireworkEffect effect, List<List<Map<String, Object>>> metadata) {
