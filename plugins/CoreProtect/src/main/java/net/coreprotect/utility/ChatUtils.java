...
public class ChatUtils {
  ...
  public static String getPageNavigation(String command, int page, int totalPages) {
    ...
    if (page > 1) {
      backArrow = "◀ ";
      backArrow =
          Chat.COMPONENT_TAG_OPEN
              + Chat.COMPONENT_COMMAND
              + "|/"
              + command
              + " l "
              + (page - 1)
  ...
  public static String getTimeSince(long resultTime, long currentTime, boolean component) {
    ...
    if (timeSince < 60.0) {
      message.append(
          Phrase.build(Phrase.LOOKUP_TIME, new DecimalFormat("0.00").format(timeSince) + "/m"));
    }
    ...
    if (message.length() == 0) {
      ...
      if (timeSince < 24.0) {
        message.append(
            Phrase.build(Phrase.LOOKUP_TIME, new DecimalFormat("0.00").format(timeSince) + "/h"));
      }
    }
    ...
    if (message.length() == 0) {
      ...
      message.append(
          Phrase.build(Phrase.LOOKUP_TIME, new DecimalFormat("0.00").format(timeSince) + "/d"));
    }
    ...
    if (component) {
      ...
      return Chat.COMPONENT_TAG_OPEN
          + Chat.COMPONENT_POPUP
          + "|"
          + Color.GREY
          + formattedTimestamp
          + "|"
  ...
  public static String createTooltip(String phrase, String tooltip) {
    ...
    String message =
        Chat.COMPONENT_TAG_OPEN
            + Chat.COMPONENT_POPUP
            + "|"
            + tooltip.replace("|", Chat.COMPONENT_PIPE)
            + "|"
            +
  ...
  public static void sendConsoleComponentStartup(
      ConsoleCommandSender consoleSender, String string) {
    Chat.sendComponent(
        consoleSender,
        Color.RESET
            + "[CoreProtect] "
