...
public class ItemUtils {
        ...
        if (o1 != null && o1.getAmount() > 0) {
          ...
          for (ItemStack o2 : items) {
            if (o2 != null
                && c2 > c1
                && o1.isSimilar(o2)
                && !BlockUtils.isAir(o1.getType())) { // Ignores amount
  ...
  public static ItemStack[] getContainerContents(
      Material type, Object container, Location location) {
    ItemStack[] contents = null;
    if (Config.getConfig(location.getWorld()).ITEM_TRANSACTIONS
        && BlockGroup.CONTAINERS.contains(type)) {
      try {
        ...
        if (type == Material.ARMOR_STAND) {
          ...
          org.bukkit.block.Jukebox blockState =
              (org.bukkit.block.Jukebox) ((Block) container).getState();
  ...
  public static ItemMeta deserializeItemMeta(
      Class<? extends ItemMeta> itemMetaClass, Map<String, Object> args) {
    try {
      org.bukkit.configuration.serialization.DelegateDeserialization delegate =
          itemMetaClass.getAnnotation(
  ...
  public static String getEnchantments(byte[] metadata, int type, int amount) {
    ...
    item =
        (ItemStack) net.coreprotect.database.rollback.Rollback.populateItemStack(item, metadata)[2];
    String displayName =
  ...
  public static Map<Integer, Object> serializeItemStackLegacy(
      ItemStack itemStack, String faceData, int slot) {
  ...
  public static Map<String, Object> serializeItemStack(
      ItemStack itemStack, String faceData, int slot) {
    Map<String, Object> itemMap = new HashMap<>();
    if (itemStack != null && !itemStack.getType().equals(Material.AIR)) {
      ItemStack item = itemStack.clone();
      List<List<Map<String, Object>>> metadata =
          ItemMetaHandler.serialize(item, null, faceData, slot);
  ...
  public static ItemStack unserializeItemStack(Object value) {
    ...
    if (value instanceof Map) {
      ...
      Object[] populatedStack =
          net.coreprotect.database.rollback.Rollback.populateItemStack(item, metadata);
