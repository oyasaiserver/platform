...
public class RollbackUtil extends Lookup {
        ...
        if (inventory != null) {
          ...
          if (action == 1) {
            ...
            ItemStack[] inventoryContents =
                (isPlayerInventory ? inventory.getContents() : inventory.getStorageContents())
                    .clone();
  ...
  public static void sortContainerItems(
      PlayerInventory inventory, List<Integer> modifiedArmorSlots) {
    try {
  ...
  private static void buildFireworkEffect(
      Builder effectBuilder, Material rowType, ItemStack itemstack) {
    try {
    ...
    try {
      ...
      */
      ...
      for (List<Map<String, Object>> map : (List<List<Map<String, Object>>>) list) {
        if (map.size() == 0) {
          if (itemCount == 3
              && (rowType == Material.FIREWORK_ROCKET || rowType == Material.FIREWORK_STAR)) {
        ...
        if (mapData.get("slot") != null) {
          ...
          ItemMeta meta =
              ItemUtils.deserializeItemMeta(itemstack.getItemMeta().getClass(), map.get(0));
          itemstack.setItemMeta(meta);
   ...
   *
   * @param metadata The byte array containing serialized metadata
   ...
   *
   * @param user The username of the player
   * @param block The block state to update
   * @param rowId The row ID for the skull data
   */
