...
public class RollbackProcessor {
   ...
   * @param chunkKey The chunk lookup key
   * @param blockList The list of block data to process
   * @param itemList The list of item data to process
   * @param rollbackType The rollback type (0=rollback, 1=restore)
   * @param preview Whether this is a preview (0=no, 1=yes-non-destructive, 2=yes-destructive)
   * @param finalUserString The username performing the rollback
   * @param finalUser The user performing the rollback
   * @param bukkitRollbackWorld The world to process
   * @return True if successful, false if there was an error
   */
  public static boolean processChunk(
      int finalChunkX,
      int finalChunkZ,
      long chunkKey,
    ...
    try {
      ...
      for (Object[] row : data) {
        ...
        if (preview > 0) {
          if (rowAction != 3) { // entity kill
            ...
            if (preview == 2) {
              ...
              if (!BukkitAdapter.ADAPTER.isItemFrame(blockType)
                  ...
                  && !blockType.equals(Material.END_CRYSTAL)) {
                BlockUtils.prepareTypeAndData(
                    chunkChanges, block, blockType, block.getBlockData(), true);
                blockCount++;
              }
            } else {
          ...
          entityCount +=
              RollbackEntityHandler.processEntity(
                  row,
                  rollbackType,
                  finalUserString,
                  oldTypeRaw,
                  ...
                  rowData,
                  rowAction,
                  MaterialUtils.rolledBack((Integer) row[9], false),
                  rowX,
                  rowY,
                  rowZ,
                  rowWorldId,
          ...
          if ((rowType == pendingChangeType)
            ...
            if (checkData != null) {
              if (checkData.getAsString().equals(pendingChangeData.getAsString())
                  || checkData instanceof org.bukkit.block.data.MultipleFacing
                  || checkData instanceof org.bukkit.block.data.type.Stairs
                  || checkData instanceof org.bukkit.block.data.type.RedstoneWire) {
                if (rowType != Material.CHEST
                    && rowType != Material.TRAPPED_CHEST) { // always update double chests
                  changeBlock = false;
                }
              }
            ...
            countBlock = false;
              ...
              && (pendingChangeType != Material.CAVE_AIR)) {
            countBlock = true;
          }

          if ((pendingChangeType == Material.WATER)
              && (rowType != Material.AIR)
              && (rowType != Material.CAVE_AIR)
              && blockData != null) {
            if (blockData instanceof org.bukkit.block.data.Waterlogged) {
              if (Material.WATER.createBlockData().equals(block.getBlockData())) {
                org.bukkit.block.data.Waterlogged waterlogged =
                    (org.bukkit.block.data.Waterlogged) blockData;
                waterlogged.setWaterlogged(true);
              }
            }
          }

          if (RollbackBlockHandler.processBlockChange(
                  bukkitWorld,
                  block,
                  row,
                  rollbackType,
                  ...
                  changeBlock,
                  changeBlockData,
                  meta != null ? new ArrayList<>(meta) : null,
                  blockData,
                  rowUser,
                  rowType,
                  rowX,
                  ...
                  rowAction,
                  rowWorldId,
                  BlockUtils.byteDataToString((byte[]) row[13], rowTypeRaw))
              && countBlock) {
            blockCount++;
          }
        }

        ConfigHandler.rollbackHash.put(
            finalUserString, new int[] {itemCount, blockCount, entityCount, 0, scannedWorlds});
      }
      data.clear();

      // Apply cached block changes
      RollbackBlockHandler.applyBlockChanges(
          chunkChanges, preview, finalUser instanceof Player ? (Player) finalUser : null);
      ...
      for (Object[] row : itemData) {
        ...
        if (rowType != null) {
          if (inventoryRollback
              && ((rollbackType == 0 && rolledBackInventory == 0)
                  || (rollbackType == 1 && rolledBackInventory == 1))) {
            ...
            if (rowAction == ItemLogger.ITEM_DROP
                || rowAction == ItemLogger.ITEM_PICKUP
                || rowAction == ItemLogger.ITEM_THROW
                || rowAction == ItemLogger.ITEM_SHOOT
                || rowAction == ItemLogger.ITEM_BREAK
                || rowAction == ItemLogger.ITEM_DESTROY
                || rowAction == ItemLogger.ITEM_CREATE
                || rowAction == ItemLogger.ITEM_SELL
                || rowAction == ItemLogger.ITEM_BUY) {
              inventoryAction =
                  ((rowAction == ItemLogger.ITEM_PICKUP
                          || rowAction == ItemLogger.ITEM_CREATE
                          || rowAction == ItemLogger.ITEM_BUY)
                      ? 1
                      : 0);
            } else if (rowAction == ItemLogger.ITEM_REMOVE_ENDER
                || rowAction == ItemLogger.ITEM_ADD_ENDER) {
            ...
            if (rowAction == ItemLogger.ITEM_REMOVE_ENDER
                || rowAction == ItemLogger.ITEM_ADD_ENDER) {
              RollbackUtil.modifyContainerItems(
                  containerType,
                  player.getEnderChest(),
                  (Integer) populatedStack[0],
                  ((ItemStack) populatedStack[2]).clone(),
                  action ^ 1);
            }
            int modifiedArmor =
                RollbackUtil.modifyContainerItems(
            ...
            ConfigHandler.rollbackHash.put(
                finalUserString,
                new int[] {itemCount1, blockCount1, entityCount1, 0, scannedWorlds});
            continue; // remove this for merged rollbacks in future? (be sure to re-enable chunk
            // sorting)
          }
            ...
            if (!containerInit
              ...
              if (BlockGroup.CONTAINERS.contains(block.getType())) {
                ...
                containerType = block.getType();
                  ...
                  || BlockGroup.CONTAINERS.contains(Material.ITEM_FRAME)) {
                  ...
                  if (entity.getLocation().getBlockX() == rowX
                    ...
                    if (entity instanceof ArmorStand) {
                      ...
                      if (faceData.length() > 0
                          && (BlockFace.valueOf(faceData) == ((ItemFrame) entity).getFacing())) {
                        break;
                      }
                    }
                  }
        ...
        ConfigHandler.rollbackHash.put(
            finalUserString, new int[] {itemCount1, blockCount1, entityCount1, 0, scannedWorlds});
      }
      itemData.clear();

      for (Entry<Player, List<Integer>> sortEntry : sortPlayers.entrySet()) {
        RollbackItemHandler.sortContainerItems(
            sortEntry.getKey().getInventory(), sortEntry.getValue());
      }
      sortPlayers.clear();
      ...
      int scannedWorlds = rollbackHashData[4];
      ConfigHandler.rollbackHash.put(
          finalUserString, new int[] {itemCount, blockCount, entityCount, 1, (scannedWorlds + 1)});
      ...
      if (preview == 0) {
        for (Player player : Bukkit.getOnlinePlayers()) {
          ...
          if (bukkitRollbackWorld.getName().equals(playerWorld)
              && chunkX == finalChunkX
              && chunkZ == finalChunkZ) {
      ...
      ConfigHandler.rollbackHash.put(
          finalUserString, new int[] {itemCount, blockCount, entityCount, 2, (scannedWorlds + 1)});
      return false;
    }
  }
}
