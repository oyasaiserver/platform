...
public class BlockLookup {
  ...
  public static String performLookup(
    ...
    try {
      ...
      query =
          "SELECT time,user,action,type,data,rolled_back FROM "
              + ConfigHandler.prefix
              + "block "
              + WorldUtils.getWidIndex("block")
              + "WHERE wid = '"
      ...
      while (results.next()) {
          ...
          resultTextBuilder =
              new StringBuilder(
                  Color.WHITE
                      ...
                      + Color.WHITE
                      + "----- "
                      + ChatUtils.getCoordinates(command, worldId, x, y, z, false, false)
                      + "\n");
        }
        found = true;
        ...
        if (resultAction == 3) {
          ...
          target =
              StringUtils.nameFilter(resultMaterial.name().toLowerCase(Locale.ROOT), resultData);
      ...
      if (found) {
        ...
        if (rowMax > count && count > 0) {
          ...
          // resultText = Color.DARK_AQUA + "CoreProtect " + Color.WHITE + "- " + Color.WHITE + "No
          // block data found at " + Color.ITALIC + "x" + x + "/y" + y + "/z" + z + ".";
          resultText =
              Color.DARK_AQUA
                  + "CoreProtect "
                  + Color.WHITE
