...
public class LookupRaw extends Queue {
  ...
  protected static List<Object[]> performLookupRaw(
      Statement statement,
      CommandSender user,
  ...
  static ResultSet rawLookupResultSet(
      Statement statement,
      CommandSender user,
      List<String> checkUuids,
      List<String> checkUsers,
    ...
    try {
      ...
      if ((lookup && actionList.size() == 0)
          || (actionList.contains(11) && actionList.size() == 1)) {
          ...
          if (validActions.contains(actionTarget)) {
            ...
            if (actionList.contains(11) && !actionList.contains(4)) {
              if (actionTarget == ItemLogger.ITEM_REMOVE
                  && !actionList.contains(ItemLogger.ITEM_DROP)) {
      ...
      if (validAction) {
        queryBlock = queryBlock + " action IN(" + action + ") AND";
          ...
          || actionExclude.length() > 0
          || includeBlock.length() > 0
          || includeEntity.length() > 0
          || excludeBlock.length() > 0
      ...
      if (includeBlock.length() > 0 || includeEntity.length() > 0) {
        queryBlock =
            queryBlock + " type IN(" + (includeBlock.length() > 0 ? includeBlock : "0") + ") AND";
      }
      ...
      if (actionList.contains(10)) {
        queryBlock =
            queryBlock
                + " action = '1' AND (LENGTH(line_1) > 0 OR LENGTH(line_2) > 0 OR LENGTH(line_3) > 0 OR LENGTH(line_4) > 0 OR LENGTH(line_5) > 0 OR LENGTH(line_6) > 0 OR LENGTH(line_7) > 0 OR LENGTH(line_8) > 0) AND";
      }
      ...
      if (includeBlock.length() > 0 || includeEntity.length() > 0) {
        queryEntity =
            queryEntity.replace(
                "type IN(" + (includeBlock.length() > 0 ? includeBlock : "0") + ")",
      ...
      String baseQuery =
          ((!includeEntity.isEmpty() || !excludeEntity.isEmpty()) ? queryEntity : queryBlock);
      ...
      if (actionList.contains(4) || actionList.contains(5)) {
        ...
        queryTable = "sign";
        rows =
            "rowid as id,time,user,wid,x,y,z,face,line_1,line_2,line_3,line_4,line_5,line_6,line_7,line_8";
      } else if (actionList.contains(11)) {
        queryTable = "item";
        rows =
            "rowid as id,time,user,wid,x,y,z,type,data as metadata,0 as data,amount,action,0 as rolled_back";
      }
      ...
      if (Config.getGlobal().MYSQL) {
        if (queryTable.equals("block")) {
          ...
          if ((restrictWorld
              && (users.length() > 0 || includeBlock.length() > 0 || includeEntity.length() > 0))) {
      ...
      if ((lookup && actionList.size() == 0) || (itemLookup && !actionList.contains(0))) {
        if (!count) {
          rows =
              "rowid as id,time,user,wid,x,y,z,type,meta as metadata,data,-1 as amount,action,rolled_back";
        }
        ...
        if (includeBlock.length() > 0 || excludeBlock.length() > 0) {
          baseQuery =
              baseQuery.replace(
                  "action NOT IN(-1)",
                  "action NOT IN(3)"); // if block specified for include/exclude, filter out entity
          // data
        }
      ...
      if (itemLookup) {
        ...
        if (!count) {
          rows =
              "rowid as id,time,user,wid,x,y,z,type,data as metadata,0 as data,amount,action,rolled_back";
          queryOrder = " ORDER BY time DESC, tbl DESC, id DESC";
        }

        if (actionExclude.length() > 0) {
          queryBlock =
              queryBlock.replace("action NOT IN(-1)", "action NOT IN(" + actionExclude + ")");
        }
      ...
      if (query.length() == 0) {
        if (actionExclude.length() > 0) {
          baseQuery =
              baseQuery.replace("action NOT IN(-1)", "action NOT IN(" + actionExclude + ")");
        }
