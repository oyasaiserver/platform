...
public class RollbackBlockHandler extends Queue {
   ...
   * @param row Block data from the database (used only for specific operations)
   * @param rollbackType The type of rollback (0=rollback, 1=restore)
   * @param clearInventories Whether to clear container inventories
   * @param chunkChanges Map of block changes to apply
   * @param countBlock Whether to count this block in stats
   * @param oldTypeMaterial The previous material type
   * @param pendingChangeType The pending change material type
   * @param pendingChangeData The pending change block data
   * @param finalUserString The username for this rollback
   * @param rawBlockData The raw block data
   * @param changeType The current block type
   * @param changeBlockData The current block data
   * @param meta Block metadata
   * @param blockData The processed block data
   * @param rowUser The username associated with this block change
   * @param rowType The material type for this block change
   ...
   * @param blockDataString The block data as a string
   * @return Updated count status
   */
  public static boolean processBlockChange(
      Block block,
      Object[] row,
      int rollbackType,
      boolean clearInventories,
      Map<Block, BlockData> chunkChanges,
      boolean countBlock,
      Material oldTypeMaterial,
      Material pendingChangeType,
      BlockData pendingChangeData,
      String finalUserString,
      BlockData rawBlockData,
      Material changeType,
      BlockData changeBlockData,
      ArrayList<Object> meta,
      BlockData blockData,
      String rowUser,
      Material rowType,
      int rowX,
    ...
    try {
      if (changeBlock) {
        ...
        } else if (rowType == Material.MOVING_PISTON
            && blockData instanceof TechnicalPiston technicalPiston
            && !(blockData instanceof PistonHead)) {
          rowType =
              (technicalPiston.getType() == org.bukkit.block.data.type.TechnicalPiston.Type.STICKY
        ...
        if ((rowType == Material.AIR)
          ...
          for (Entity entity : block.getChunk().getEntities()) {
            if (entity instanceof ArmorStand) {
              if (entity.getLocation().getBlockX() == location1.getBlockX()
                  ...
                  && entity.getLocation().getBlockZ() == location1.getBlockZ()) {
                exists = true;
              }
            }
          }
          ...
          if (!exists) {
            Entity entity =
                block.getLocation().getWorld().spawnEntity(location1, EntityType.ARMOR_STAND);
          ...
          if (!exists) {
            Entity entity =
                block
                    ...
                    .getWorld()
                    .spawnEntity(
                        location1, BukkitAdapter.ADAPTER.getEntityType(Material.END_CRYSTAL));
            ...
            if (pendingChangeData instanceof Waterlogged waterlogged) {
              if (waterlogged.isWaterlogged()) {
                BlockUtils.prepareTypeAndData(
                    chunkChanges, block, Material.WATER, Material.WATER.createBlockData(), true);
                remove = false;
              }
          ...
          if (remove) {
            ...
            if ((changeType == Material.NETHER_PORTAL)
              ...
              physics = true;
            } else if (changeBlockData instanceof Bisected bisected
          ...
          BlockUtils.prepareTypeAndData(chunkChanges, block, Material.FIRE, null, true);
            ...
            && rowData > 0
            && (rowType == Material.IRON_DOOR || BlockGroup.DOORS.contains(rowType))) {
          if (countBlock) {
            updateBlockCount(finalUserString, 1);
          }
          ...
          return false;
        } else if (rowType != changeType
            && (BlockGroup.CONTAINERS.contains(rowType)
          ...
          return countBlock;
            ...
            && rawBlockData instanceof Bisected bisected
            && !(rawBlockData instanceof Stairs || rawBlockData instanceof TrapDoor)) {
   ...
   * @param increment The amount to increment the block count by
   */
  protected static void updateBlockCount(String userString, int increment) {
    ...
    ConfigHandler.rollbackHash.put(
        userString, new int[] {itemCount, blockCount, entityCount, 0, scannedWorlds});
  }

  /**
   * Apply all pending block changes to the world
   *
   * @param chunkChanges Map of blocks to change
   * @param preview Whether this is a preview
   * @param user The user performing the rollback
   */
  public static void applyBlockChanges(
      Map<Block, BlockData> chunkChanges, int preview, Player user) {
