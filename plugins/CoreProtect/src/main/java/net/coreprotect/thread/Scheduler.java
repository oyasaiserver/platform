...
public class Scheduler {
  ...
  public static void scheduleSyncDelayedTask(
      CoreProtect plugin, Runnable task, Object regionData, int delay) {
    if (ConfigHandler.isFolia) {
      if (regionData instanceof Location) {
        ...
        if (delay == 0) {
          ...
          Bukkit.getServer()
              .getRegionScheduler()
              .runDelayed(plugin, location, value -> task.run(), delay);
        }
        ...
        if (delay == 0) {
          ...
          Bukkit.getServer()
              ...
              .runDelayed(plugin, value -> task.run(), delay);
        }
      }
    } else {
      if (delay == 0) {
  ...
  public static Object scheduleSyncRepeatingTask(
      CoreProtect plugin, Runnable task, Object regionData, int delay, int period) {
    if (ConfigHandler.isFolia) {
      if (regionData instanceof Location) {
        Location location = (Location) regionData;
        return Bukkit.getServer()
            .getRegionScheduler()
            .runAtFixedRate(plugin, location, value -> task.run(), delay, period);
      } else if (regionData instanceof Entity) {
        Entity entity = (Entity) regionData;
        return entity
            .getScheduler()
            .runAtFixedRate(plugin, value -> task.run(), task, delay, period);
      } else {
        return Bukkit.getServer()
            .getGlobalRegionScheduler()
            .runAtFixedRate(plugin, value -> task.run(), delay, period);
      }
    } else {
      return Bukkit.getServer()
          .getScheduler()
  ...
  public static void scheduleAsyncDelayedTask(CoreProtect plugin, Runnable task, int delay) {
    if (ConfigHandler.isFolia) {
      if (delay == 0) {
        ...
        Bukkit.getServer()
            .getAsyncScheduler()
            .runDelayed(plugin, value -> task.run(), (delay * 50L), TimeUnit.MILLISECONDS);
      }
    } else {
      if (delay == 0) {
