...
public final class BlockSpreadListener extends Queue implements Listener {
  ...
  private void onBlockSpread(BlockSpreadEvent event) {
    ...
    if (Config.getConfig(event.getBlock().getWorld()).VINE_GROWTH
        && (BlockGroup.VINES.contains(type)
            || BlockGroup.AMETHYST.contains(type)
      ...
      if (BlockGroup.VINES.contains(type)) {
        ...
        Queue.queueBlockPlaceDelayed(
            ...
            sourceBlock.getType(),
            null,
            sourceBlock.getState(),
            0);
        Queue.queueBlockPlaceDelayed(
            "#chorus", block.getLocation(), block.getType(), null, block.getState(), 0);
        ...
        Queue.queueBlockPlaceDelayed(
            "#bamboo", sourceBlock.getLocation(), type, null, sourceBlock.getState(), 0);
        Queue.queueBlockPlaceDelayed(
            "#bamboo", block.getLocation(), type, null, block.getState(), 0);
      }
    } else if (Config.getConfig(event.getBlock().getWorld()).SCULK_SPREAD
        && BlockGroup.SCULK.contains(type)) {
      Block block = event.getBlock();
      if (checkCacheData(block, type)) {
        return;
      }
  ...
  private boolean checkCacheData(Block block, Material type) {
    String cacheId =
        block.getX()
            ...
            + block.getZ()
            + "."
            + WorldUtils.getWorldId(block.getWorld().getName());
