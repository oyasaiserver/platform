...
public final class BlockExplodeListener extends Queue implements Listener {
        ...
        while (scanMin < scanMax) {
          ...
          if (blockMap.get(location) == null) {
            ...
            if (BlockGroup.TRACK_ANY.contains(scanType)
              ...
              while (!topFound) {
                Block topBlock =
                    world.getBlockAt(location.getBlockX(), scanY, location.getBlockZ());
                Material topMaterial = topBlock.getType();
                if (!topMaterial.hasGravity()) {
                  location =
                      new Location(world, location.getBlockX(), (scanY - 1), location.getBlockZ());
                  topFound = true;
  ...
  private void onBlockExplode(BlockExplodeEvent event) {
    ...
    String user = "";
    if (!eventBlock.getType().equals(Material.AIR)
        && !eventBlock.getType().equals(Material.CAVE_AIR)) {
