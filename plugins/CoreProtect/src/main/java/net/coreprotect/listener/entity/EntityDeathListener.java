...
public final class EntityDeathListener extends Queue implements Listener {
  ...
  public static void parseEntityKills(String message) {
    ...
    if (args.length < 2
        || !args[0].replaceFirst("/", "").equals("kill")
        || !args[1].startsWith("@e")) {
      return;
    }
  ...
  protected static void logEntityDeath(LivingEntity entity, String e) {
    ...
    boolean isCommand =
        (damage.getCause() == DamageCause.VOID
            && entity.getLocation().getBlockY()
                >= BukkitAdapter.ADAPTER.getMinHeight(entity.getWorld()));
    if (e == null) {
      e = isCommand ? "#command" : "";
    }
    ...
    List<DamageCause> validDamageCauses =
        Arrays.asList(
            DamageCause.SUICIDE,
            DamageCause.POISON,
            DamageCause.THORNS,
    ...
    if (!Config.getConfig(entity.getWorld()).SKIP_GENERIC_DATA
        ...
        || (validDamageCauses.contains(cause) || cause.name().equals("KILL"))) {
      skip = false;
    }
    ...
    if (entity instanceof ArmorStand) {
      ...
      if (!Config.getConfig(entityLocation.getWorld()).ITEM_TRANSACTIONS) {
        ...
        Queue.queueBlockBreak(
            e, block.getState(), Material.ARMOR_STAND, null, (int) entityLocation.getYaw());
      }
      /*
      else if (isCommand) {
          entityLocation.setY(entityLocation.getY() + 0.99);
      ...
      if (entity instanceof Attributable) {
        ...
        for (Attribute attribute : Lists.newArrayList(Registry.ATTRIBUTE)) {
          ...
          if (attributeInstance != null) {
            ...
            attributeData.add(
                BukkitAdapter.ADAPTER.getRegistryKey(attributeInstance.getAttribute()));
      ...
      if (entity instanceof Creeper) {
        ...
        for (MerchantRecipe merchantRecipe : abstractVillager.getRecipes()) {
          ...
          List<List<Map<String, Object>>> metadata =
              ItemMetaHandler.serialize(item, item.getType(), null, 0);
