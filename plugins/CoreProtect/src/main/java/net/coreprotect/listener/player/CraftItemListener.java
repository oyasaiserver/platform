...
public final class CraftItemListener extends Queue implements Listener {
  ...
  protected static void logCraftedItem(
      Location location, String user, ItemStack itemStack, int action) {
  ...
  protected static void playerCraftItem(InventoryClickEvent event, boolean isTrade) {
          ...
          if (isTrade) {
            ...
            for (ItemStack ingredient : merchantRecipe.getIngredients()) {
              if (ingredient.isSimilar(item)) {
                ItemStack adjusted =
                    BukkitAdapter.ADAPTER.adjustIngredient(merchantRecipe, ingredient);
                if (adjusted == null) {
                  return;
                }
    ...
    if (addItem.getAmount() > 0) {
      Location location =
          (isTrade || event.getInventory().getLocation() == null)
              ? player.getLocation()
      ...
      logCraftedItem(
          ...
          player.getName(),
          addItem,
          isTrade ? ItemLogger.ITEM_BUY : ItemLogger.ITEM_CREATE);
    }
  }
