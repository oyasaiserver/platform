  ...
  private void onBlockIgnite(BlockIgniteEvent event) {
    ...
    if (!event.isCancelled() && Config.getConfig(world).BLOCK_IGNITE) {
      ...
      if (BlockGroup.LIGHTABLES.contains(blockType)) {
        ...
        if (forceBlockData instanceof Lightable lightable) {
      ...
      if (event.getPlayer() == null) {
        ...
        if (event.getCause() == IgniteCause.FIREBALL
          ...
          // Fix bug where fire is recorded as being placed above a campfire (when lit via a
          // fireball from a dispenser)
