...
public final class HopperPushListener {
  ...
  static void processHopperPush(
    ...
    String loggingChestId =
        "#hopper-push."
            + destinationLocation.getBlockX()
            + "."
            + destinationLocation.getBlockY()
            + "."
    ...
    ItemStack[] destinationContainer =
        ItemUtils.getContainerState(destinationHolder.getInventory().getContents());
      ...
      Scheduler.runTaskAsynchronously(
          ...
          () -> {
            try {
              ...
              while (!hopperQueue.isEmpty()
                  && (ConfigHandler.serverRunning || ConfigHandler.converterRunning)) {
                processHopperBatch();
              }
            } catch (Exception e) {
              e.printStackTrace();
              ...
              if (!hopperQueue.isEmpty()
                  && (ConfigHandler.serverRunning || ConfigHandler.converterRunning)
                  && activeProcessors.get() == 0) {
                startHopperProcessor();
              }
            }
          });
  ...
  private static void processSingleHopperPush(
      Location location,
      String user,
      InventoryHolder sourceHolder,
      InventoryHolder destinationHolder,
    ...
    boolean abort = false;
    boolean addedInventory =
        ItemUtils.canAddContainer(
            destinationContainer, movedItem, destinationHolder.getInventory().getMaxStackSize());
    if (!addedInventory) {
      abort = true;
    }
    ...
    if (abort) {
      ...
      ConfigHandler.hopperAbort.put(
          loggingChestId,
          new Object[] {movedItems, ItemUtils.getContainerState(destinationContents)});
      return;
    } else {
      ConfigHandler.hopperSuccess.put(
          loggingChestId, new Object[] {destinationContainer, movedItem});
    }
    ...
    for (int i = 0; i < originalDestination.length; i++) {
      ...
      ItemStack itemStack =
          (originalDestination[i] != null ? originalDestination[i].clone() : null);
    ...
    InventoryChangeListener.onInventoryInteract(
        user,
        destinationInventory,
        originalDestination,
        null,
        destinationInventory.getLocation(),
        true);
  }
}
