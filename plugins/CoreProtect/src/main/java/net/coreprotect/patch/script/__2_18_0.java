...
public class __2_18_0 {
  ...
  protected static boolean patch(Statement statement) {
    try {
      ...
      try {
        if (Config.getGlobal().MYSQL) {
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "block ADD COLUMN blockdata BLOB");
        }
      } catch (Exception e) {
      ...
      String query =
          "SELECT rowid, id, material FROM "
              + ConfigHandler.prefix
              + "material_map WHERE material LIKE 'minecraft:legacy_%' LIMIT 0, 1";
      String preparedBlockQuery =
          "SELECT rowid as id, data, blockdata FROM "
              + ConfigHandler.prefix
      ...
      String preparedMaterialDeleteQuery =
          "DELETE FROM " + ConfigHandler.prefix + "material_map WHERE rowid = ?";
              ...
              if (blockBlockData == null && validatedMaterial.isBlock()) {
                ...
                if (newBlockData != null) {
                  if (validatedMaterial == Material.OAK_WALL_SIGN
                      && newBlockData instanceof Directional directional) {
                  ...
                  if (validatedMaterial == Material.SKELETON_SKULL
                      && newBlockData instanceof Rotatable rotatable) {
                    BlockFace newRotation = getLegacyRotation(blockData);
                    rotatable.setRotation(newRotation);
                  }
                  blockBlockData =
                      BlockUtils.stringToByteData(newBlockData.getAsString(), validatedID);
                }
              }
      ...
      if (createIndexes) {
        try {
          if (Config.getGlobal().MYSQL) {
            statement.executeUpdate(
                "ALTER TABLE " + ConfigHandler.prefix + "art_map ADD INDEX(id)");
            statement.executeUpdate(
                "ALTER TABLE " + ConfigHandler.prefix + "entity_map ADD INDEX(id)");
            statement.executeUpdate(
                "ALTER TABLE " + ConfigHandler.prefix + "material_map ADD INDEX(id)");
            statement.executeUpdate("ALTER TABLE " + ConfigHandler.prefix + "world ADD INDEX(id)");
            statement.executeUpdate(
                "ALTER TABLE " + ConfigHandler.prefix + "blockdata_map ADD INDEX(id)");
          } else {
            statement.executeUpdate(
                "CREATE INDEX IF NOT EXISTS art_map_id_index ON "
                    + ConfigHandler.prefix
                    + "art_map(id);");
            statement.executeUpdate(
                "CREATE INDEX IF NOT EXISTS blockdata_map_id_index ON "
                    + ConfigHandler.prefix
                    + "blockdata_map(id);");
            statement.executeUpdate(
                "CREATE INDEX IF NOT EXISTS entity_map_id_index ON "
                    + ConfigHandler.prefix
                    + "entity_map(id);");
            statement.executeUpdate(
                "CREATE INDEX IF NOT EXISTS material_map_id_index ON "
                    + ConfigHandler.prefix
                    + "material_map(id);");
            statement.executeUpdate(
                "CREATE INDEX IF NOT EXISTS world_id_index ON "
                    + ConfigHandler.prefix
                    + "world(id);");
          }
        } catch (Exception e) {
