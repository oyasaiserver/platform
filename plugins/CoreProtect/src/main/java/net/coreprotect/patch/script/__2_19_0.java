...
public class __2_19_0 {
  ...
  protected static boolean patch(Statement statement) {
    try {
      if (Config.getGlobal().MYSQL) {
        try {
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "sign ADD COLUMN action int");
        ...
        try {
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "sign ADD COLUMN action INTEGER;");
        } catch (Exception e) {
          Chat.console(
              Phrase.build(
                  Phrase.PATCH_SKIP_UPDATE,
        ...
        try {
          statement.executeUpdate(
              "CREATE INDEX IF NOT EXISTS sign_index ON "
                  + ConfigHandler.prefix
                  + "sign(wid,x,z,time);");
          statement.executeUpdate(
              "CREATE INDEX IF NOT EXISTS sign_user_index ON "
                  + ConfigHandler.prefix
                  + "sign(user,time);");
          statement.executeUpdate(
              "CREATE INDEX IF NOT EXISTS sign_time_index ON "
                  + ConfigHandler.prefix
                  + "sign(time);");
        } catch (Exception e) {
          Chat.console(
        ...
        try {
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "chat ADD COLUMN wid INTEGER;");
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "chat ADD COLUMN x INTEGER;");
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "chat ADD COLUMN y INTEGER;");
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "chat ADD COLUMN z INTEGER;");
        } catch (Exception e) {
          Chat.console(
              Phrase.build(
                  Phrase.PATCH_SKIP_UPDATE,
        ...
        try {
          statement.executeUpdate(
              "CREATE INDEX IF NOT EXISTS chat_wid_index ON "
                  + ConfigHandler.prefix
                  + "chat(wid,x,z,time);");
        } catch (Exception e) {
          Chat.console(
        ...
        try {
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "command ADD COLUMN wid INTEGER;");
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "command ADD COLUMN x INTEGER;");
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "command ADD COLUMN y INTEGER;");
          statement.executeUpdate(
              "ALTER TABLE " + ConfigHandler.prefix + "command ADD COLUMN z INTEGER;");
        } catch (Exception e) {
          Chat.console(
              Phrase.build(
                  Phrase.PATCH_SKIP_UPDATE,
        ...
        try {
          statement.executeUpdate(
              "CREATE INDEX IF NOT EXISTS command_wid_index ON "
                  + ConfigHandler.prefix
      ...
      String blockQuery =
          "SELECT time, user, wid, x, y, z FROM "
              + ConfigHandler.prefix
              + "block WHERE type IN("
              + signData.toString()
              + ") AND action='1' ORDER BY rowid ASC";
      String preparedSignQuery =
          "SELECT rowid as id FROM "
              + ConfigHandler.prefix
              + "sign WHERE user = ? AND wid = ? AND x = ? AND y = ? AND z = ? AND time >= ? ORDER BY rowid ASC LIMIT 0, 1";
      String preparedQueryUpdate =
          "UPDATE " + ConfigHandler.prefix + "sign SET action = 1 WHERE rowid = ?";
      PreparedStatement preparedSignStatement =
          statement.getConnection().prepareStatement(preparedSignQuery);
