...
object PublicityHistoryGUI {
    ...
    fun createGUI(): ChestGui {
        ...
        val thread = Thread({
            PublicityHistory.getData().toList().sortedByDescending { it.first }.toMap().values.forEach { pData ->
                try {
        ...
        pagePane.setOnClick {
            val id = it.currentItem?.itemMeta?.persistentDataContainer?.get(idKey, PersistentDataType.INTEGER)
                ?: return@setOnClick
