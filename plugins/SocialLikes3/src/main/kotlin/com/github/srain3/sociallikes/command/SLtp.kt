...
object SLtp: CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        val slData = if (id != null) {
            ...
            val getOwnerUUID = AllBuild.timeToUser(time) ?: run {
                sender.sendMessage(Tools.socialLikesLOGO+ "&c 情報が欠けているためユーザーの次の建築を返せませんでした".color())
                return true
            }
            val getID = UserBuild.timeToNextID(time,getOwnerUUID) ?: run {
                sender.sendMessage(Tools.socialLikesLOGO+ "&e 次の新着Likeは有りませんでした".color())
                return true
            }
        ...
        object : BukkitRunnable() {
            override fun run() {
                if (sender.teleport(slLoc, PlayerTeleportEvent.TeleportCause.COMMAND)) {
                    sender.sendMessage(Tools.socialLikesLOGO + " &r「${slData.title}」へテレポートしました(ID:${slData.id})".color())
                } else {
                    sender.sendMessage(Tools.socialLikesLOGO + " &c何らかの理由で「${slData.title}」へテレポート出来ませんでした(ID:${slData.id})".color())
                }
            }
    ...
    fun userLastSLTPTimeSave() {
        ...
        userLastTimeStamp.forEach { (uuid, time) ->
            yml.set(uuid.toString(), time.toString())
        }
        yml.save()
    }
    ...
    fun BlockFace.toYaw(): Float {
        return when (this) {
            ...
            SOUTH_EAST -> {135F}
            SOUTH_WEST -> {-135F}
            WEST_NORTH_WEST -> {67.5F}
            NORTH_NORTH_WEST -> {-22.5F}
            NORTH_NORTH_EAST -> {22.5F}
            EAST_NORTH_EAST -> {67.5F}
            EAST_SOUTH_EAST -> {112.5F}
            SOUTH_SOUTH_EAST -> {157.5F}
            SOUTH_SOUTH_WEST -> {-157.5F}
            WEST_SOUTH_WEST -> {-112.5F}
            else -> {0F}
        }
    }
}