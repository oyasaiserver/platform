...
object Data {
    ...
    fun save(data: SLData) {
        ...
        val dirName = if (data.id in -49..-1) {
            "-1--49"
        } else if (fPage < 0) {
    ...
    fun delID(slData: SLData, updateMode: Boolean = false) {
        ...
        val dirName = if (slData.id in -49..-1) {
            "-1--49"
        } else if (fPage < 0) {
            "${fPage*50}-${(fPage-1)*50+1}"
        } else {
        ...
        Thread(
            {
                ...
                dir.forEach dir@{
                    ...
                    files.forEach file@{ file ->
                        ...
                        //locationへ変換
                        val world = Bukkit.getServer().getWorld(worldStr) ?: run {
                        ...
                        // Cacheへ入れる
                        val slData = SLData(id, loc, time, owner, title, likes, check, comment, worldStr, textID)
     ...
     */
    private val slNearData = mutableMapOf<String,MutableMap<Int,MutableMap<Int,MutableList<SLData>>>>()
    ...
    private fun slNearLoadTask() {
        ...
        list.forEach { data ->
            if (data.loc.world != null) {
                ...
                val nChunkX = nWorld[x] ?: run {
                    val map = mutableMapOf<Int, MutableList<SLData>>()
                    nWorld[x] = map
                    map
                }
                val z = data.loc.blockZ.shr(4)
                val nChunkZ = nChunkX[z] ?: run {
                    val set = mutableListOf<SLData>()
                    nChunkX[z] = set
                    set
                }
    ...
    fun vacantTPTask(player: Player, radius: Int, maxCount: Int, biomeStr: String?){
        ...
        val bordarData = WorldBorder.plugin.getWorldBorder(loc.world.name) ?: run {
            ...
            while (switch) {
                ...
                if (count <= c) {
                    ...
                    object : BukkitRunnable() {
                        override fun run() {
                            var switchY = true
                            val sLoc = Location(loc.world, x.toDouble(), 321.0, z.toDouble(), loc.yaw, loc.pitch)
                            while (switchY) {
                                if (sLoc.block.isPassable) {
                                    ...
                                    if (sLoc.block.getRelative(BlockFace.DOWN).blockData.material == Material.WATER) {
                                        switchY = false
                                        continue
                                    }
                                }
                            ...
                            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "tp ${player.name} $x ${sLoc.y} $z")
                        }
                    }.runTask(Tools.plugin)
                    switch = false
                }
