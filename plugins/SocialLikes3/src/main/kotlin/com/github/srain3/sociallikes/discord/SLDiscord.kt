...
object SLDiscord {
    ...
    fun enable() {
        ...
        textChID = textIDStr.toLongOrNull() ?: kotlin.run {
        ...
        DiscordApiBuilder()
            ...
            .login().thenAccept {
                ...
                it.addReactionAddListener { event ->
                    ...
                    val uuid = DiscordSRV.getPlugin().accountLinkManager.linkedAccounts[event.userIdAsString]
                        ?: return@addReactionAddListener
                    ...
                    object : BukkitRunnable() {
                        override fun run() {
                            ...
                            if (ownerPlayer?.isOnline == true) {
                                ...
                                Events.offlineLikesPoint[data.owner] = (Events.offlineLikesPoint[data.owner] ?: 0) + 2
                            }
    ...
    fun sendSLEmbedMsg(slData: SLData): Long {
        ...
        val embed = EmbedBuilder()
            ...
            .addField("Author: " + Bukkit.getPlayer(slData.owner)?.name + " | ${slData.time.format(DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm"))}", "BlueMap: ${blueMapURL(slData.loc)}")
            .setColor(Color.PINK)
