...
object Events: Listener {
    ...
    fun signEditEvent(e: SignChangeEvent) {
        ...
        //Data.lastID++
        val id = if (Data.emptyIDList.isEmpty()) {
            Data.lastID += 1
            Data.lastID
        } else {
        ...
        //SLDataを作成&保存
        val data = SLData(id,e.block.location, LocalDateTime.now(),e.player.uniqueId,title, mutableListOf(), false, "No comment", e.block.world.name, 0)
        ...
        Bukkit.spigot().broadcast(TextComponent(Tools.socialLikesLOGO + "&f${e.player.name}さん&rが「&a${title}&r」を建てました！".color()).apply {
        ...
        Bukkit.spigot().broadcast(TextComponent("&l&n/sltp ${id}&rで見に行きましょう！".color()).apply {
            this.clickEvent = ClickEvent(ClickEvent.Action.RUN_COMMAND, "/sociallikes3:sltp $id")
            this.hoverEvent = HoverEvent(HoverEvent.Action.SHOW_TEXT, Text("&nクリックでテレポート&rします".color()))
        })
        Bukkit.spigot().broadcast(TextComponent(" "))
        ...
        if (slSignRegex.containsMatchIn(unColorFrontL0)) {
            ...
            if (data.loc != block.location) {
                if (SLUpdate.switch[e.player.uniqueId] == true) {
                    val newData = SLData(data.id,block.location,data.time,data.owner,data.title,data.likes,data.check,data.comment,block.world.name,data.discordTextID)
                    Data.delID(data, true)
                    Data.save(newData)
            ...
            if (e.player.isSneaking) {
                SLSignLikes.createGUI(block,data,(e.player.uniqueId == data.owner),e.player.isOp).show(e.player)
                return
            }
            ...
            if (data.likes.none { it == e.player.uniqueId }) {
                ...
                if (ownerPlayer?.isOnline == true) {
                    ...
                    Tools.advAPI.displayCustomToast(ownerPlayer, ItemStack(Material.OAK_SIGN), Tools.socialLikesLOGOShort + "&a${data.title}&7ID:${id}&r\n${e.player.name}&7<&rイイね!".color(), AdvancementFrameType.TASK)
                    ownerPlayer.playSound(ownerPlayer, Sound.ENTITY_PLAYER_LEVELUP, 1F, 1F)
                    if (e.player.uniqueId != data.owner) {
                        Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "tokenmanager:tm add ${ownerPlayer.name} 2")
                    }
                } else {
            ...
            var id = block.getSide(Side.FRONT).line(0).style().color()?.asHexString()?.substring(1)?.toIntOrNull(16) ?: return
            id = -id
            val data = Data.getSLData(id) ?: return
            ...
            if (data.loc != block.location) {
                if (SLUpdate.switch[e.player.uniqueId] == true) {
                    ...
                    block.setLine(3, "&7Likes&8: &6${data.likes.count()}${if(data.check){" &e✓"}else{""}}".color())
                    ...
                    e.player.sendMessage(Tools.socialLikesLOGO+"&f旧Like看板です。登録データの座標と違います。 運営に報告お願いします".color())
                    Bukkit.getLogger().warning("[SocialLikes3] 旧Like看板の座標違いがクリックされました")
                    Bukkit.getLogger().warning("[SocialLikes3] world:${block.location.world.name},x${block.location.x},y${block.location.y},z${block.location.z}")
                }
                return
            }
            //看板の装飾
    ...
    fun offlineLikePointSave() {
        ...
        offlineLikesPoint.forEach { (uuid, int) ->
            yml.set(uuid.toString(), int)
        }
        yml.save()
    }
    ...
    fun joinEvent(e: PlayerJoinEvent) {
        ...
        Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "tokenmanager:tm add ${e.player.name} $pointInt")
