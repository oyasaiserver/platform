...
object SLSignDel {
    fun createGUI(sign: Sign, slData: SLData): HopperGui {
        ...
        val buttonY = GuiItem(ItemStack(Material.GREEN_BED).apply {
            allFlag()
            addText("&cはい、消去します。", mutableListOf("&fこの操作は取り消せません"))
        })
        val buttonN = GuiItem(ItemStack(Material.RED_BED).apply {
            allFlag()
        ...
        buttonY.setAction {
            ...
            it.whoClicked.sendMessage(Tools.socialLikesLOGO+"&r ID:${slData.id}「&a${slData.title}&r」を&c消去しました".color())
        }
        buttonN.setAction {
            it.whoClicked.closeInventory()
        }
