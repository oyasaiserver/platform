...
object UserBuild {
    ...
    fun createGUI(user: OfflinePlayer, player: Player): ChestGui {
        ...
        pagePane.populateWithItemStacks(userBuildItem[user.uniqueId]?.toSortedMap(reverseOrder())?.values?.toList() ?: listOf())
        pagePane.setOnClick {
            val id = it.currentItem?.itemMeta?.persistentDataContainer?.get(idKey, PersistentDataType.INTEGER) ?: return@setOnClick
        ...
        if (FollowBuild.isFollow(user.uniqueId, player)) {
            navigation.addItem(GuiItem(
                ...
                event.whoClicked.sendMessage(Tools.socialLikesLOGO+"&r ${user.name}さんへのフォローを外しました".color())
            }, 6, 0
            )
        } else {
            navigation.addItem(GuiItem(
                ...
                event.whoClicked.sendMessage(Tools.socialLikesLOGO+"&r ${user.name}さんをフォローしました！".color())
                event.whoClicked.sendMessage(Tools.socialLikesLOGO+"&r \"/slmenu\"のフォロー建築一覧から建築を確認できます".color())
            }, 6, 0
            )
        }
    ...
    private fun createSignItem(slData: SLData) {
        ...
        item.allFlag()
        item.addText("&f>>&a${slData.title} &rID:${slData.id}", mutableListOf(
            "&3制作者:&f ${
                try {
    ...
    fun timeToNextID(localDateTime: LocalDateTime, uuid: UUID): Int? {
        ...
        return ite?.findLast { it.key > localDateTime }?.value?.itemMeta?.persistentDataContainer?.get(idKey, PersistentDataType.INTEGER)
    }
    ...
    fun openOnlinePlayerGUI(p: HumanEntity) {
        ...
        pagePane.setOnClick {
            ...
            val playerUUIDStr = meta.persistentDataContainer.get(key, PersistentDataType.STRING) ?: return@setOnClick
    ...
    private fun offlinePlayerSearch(player: Player) {
        AnvilGUI.Builder().apply {
            itemLeft(ItemStack(Material.PLAYER_HEAD).allFlag().addText(
