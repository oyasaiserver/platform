...
object SLSignLikes {
  fun createGUI(sign: Sign, slData: SLData, owner: Boolean, isOP: Boolean): ChestGui {
    ...
    if (owner) {
      ...
      val lore = slData.comment.split(',')
      navigation.addItem(
        GuiItem(ItemStack(Material.WRITTEN_BOOK).allFlag().addText("&7コメント", lore.toMutableList())),
        1,
        0,
      )
    }
    ...
    navigation.addItem(
      GuiItem(
        ownerHeadItem
          .allFlag()
          .addText("&f${offlineOwnerPlayer.name}さんの建築一覧を見る", mutableListOf("&7この方が建てた他の建築を見れます"))
      ) {
        it.whoClicked.closeInventory()
          ...
          .broadcast(
            TextComponent(
                Tools.socialLikesLOGO +
                  "&f${offlineOwnerPlayer.name}さん&rの「&a${slData.title}&r」を見に行きましょう！".color()
              )
              .apply {
                this.clickEvent =
    ...
    AnvilGUI.Builder().apply {
      itemLeft(
        ItemStack(Material.WRITABLE_BOOK)
          ...
          .addText(
            slData.comment,
            mutableListOf("&7出力先(右側)にあるこの本をクリックで確定します", "&7普通に閉じた場合はキャンセルです", "&7カンマ(,)で改行扱いします"),
          )
      )
      onClick { slot, e ->
