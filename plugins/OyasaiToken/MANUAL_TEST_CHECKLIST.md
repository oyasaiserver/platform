...
- [ ] `/tm add <自分> 25` → 125になる
- [ ] オンラインのテストプレイヤーが複数いる状態で `/tm addall 5` → 全員の残高がそれぞれ5増え、実行者に `N 人に <amount>P 付与しました` が返る
- [ ] オンライン0人の状態で `/tm addall 5` → `対象なし` で正常終了する
- [ ] `/tm addall 0`、`/tm addall -3`、`/tm addall x` → usageエラーになり、対象プレイヤーの残高が変わらない
