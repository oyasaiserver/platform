...
class RoadCurveCommand(private val plugin: Main) : CommandExecutor, TabCompleter {
    ...
    private fun tabCompleteSetValue(param: String, prefix: String): List<String> {
        return when (param) {
            "roadmat", "sidewalkmat", "linemat" ->
                Material.values()
                    .filter { it.isBlock && !it.isAir && it.name.lowercase().startsWith(prefix.lowercase()) }
                    .take(20)
            ...
            "centerline", "outerline" -> listOf("1", "2").filter { it.startsWith(prefix) }
    ...
    private fun handleBuild(player: Player, session: RoadSession) {
        ...
        val path = if (session.previewDirty || session.cachedPath.isEmpty()) {
        ...
        if (path.isEmpty()) { player.sendMessage("§c[RC] パスを計算できませんでした。"); return }
        ...
        plugin.server.scheduler.runTaskAsynchronously(plugin, Runnable {
            try {
                ...
                plugin.server.scheduler.runTask(plugin, Runnable {
                    player.sendMessage("§a[RC] 道路の設置が完了しました。")
                })
            } catch (e: Exception) {
                plugin.server.scheduler.runTask(plugin, Runnable {
    ...
    private fun handleUndo(player: Player, session: RoadSession) {
        val (editSession, isSmooth) = when {
            ...
            session.lastEditSession != null -> session.lastEditSession!! to false
                ...
                player.sendMessage("§c[RC] 取り消す操作がありません。"); return
            }
        }
        try {
    ...
    private fun handleSmoothLines(player: Player, session: RoadSession) {
        ...
        val path = session.cachedPath.ifEmpty {
            RoadGeometry.computePath(session.waypoints, session.settings)
        }
        if (path.isEmpty()) { player.sendMessage("§c[RC] パスを計算できませんでした。"); return }
        ...
        plugin.server.scheduler.runTaskAsynchronously(plugin, Runnable {
            try {
                ...
                plugin.server.scheduler.runTask(plugin, Runnable {
                    player.sendMessage("§c[RC] ${e.message}")
                })
            } catch (e: Exception) {
                plugin.server.scheduler.runTask(plugin, Runnable {
    ...
    private fun applyOneSetting(player: Player, s: RoadSettings, param: String, value: String): Boolean {
        try {
            when (param.lowercase()) {
                "radius"      -> { s.radius           = value.toDouble(); player.sendMessage("§a[RC] 半径: §f${s.radius} ブロック") }
                "transition"  -> { s.transitionLength = value.toDouble(); player.sendMessage("§a[RC] 緩和曲線長: §f${s.transitionLength} ブロック") }
                "lane"        -> { s.laneWidth         = value.toInt();   player.sendMessage("§a[RC] 車道幅: §f${s.laneWidth} ブロック") }
                "centerline"  -> { s.centerLineWidth   = value.toInt();   player.sendMessage("§a[RC] 中央白線幅: §f${s.centerLineWidth} ブロック") }
                "outerline"   -> { s.outerLineWidth    = value.toInt();   player.sendMessage("§a[RC] 外縁白線幅: §f${s.outerLineWidth} ブロック") }
                "sidewalk"    -> { s.sidewalkWidth     = value.toInt();   player.sendMessage("§a[RC] 歩道幅: §f${s.sidewalkWidth} ブロック") }
                "roadmat" -> {
                    val mat = Material.matchMaterial(value) ?: run { player.sendMessage("§c不明な素材: $value"); return false }
                    s.roadMaterial = mat; player.sendMessage("§a[RC] 車道素材: §f${mat.key}")
                }
                "sidewalkmat" -> {
                    val mat = Material.matchMaterial(value) ?: run { player.sendMessage("§c不明な素材: $value"); return false }
                    s.sidewalkMaterial = mat; player.sendMessage("§a[RC] 歩道素材: §f${mat.key}")
                }
                "linemat" -> {
                    val mat = Material.matchMaterial(value) ?: run { player.sendMessage("§c不明な素材: $value"); return false }
                    s.lineMaterial = mat; player.sendMessage("§a[RC] 白線素材: §f${mat.key}")
                }
                else -> { player.sendMessage("§c不明なパラメータ: $param"); return false }
            }
    ...
    private fun handleDebugLine(player: Player, session: RoadSession, args: Array<out String>) {
        ...
        val newValue = when {
            ...
            args[1].equals("on", true) || args[1].equals("enable", true) || args[1].equals("true", true) -> true
            args[1].equals("off", true) || args[1].equals("disable", true) || args[1].equals("false", true) -> false
    ...
    companion object {
        ...
        private val SET_PARAMS  = listOf("radius", "transition", "lane", "centerline", "outerline", "sidewalk",
                                         "roadmat", "sidewalkmat", "linemat")
    }
}
