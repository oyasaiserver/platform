...
/**
 * 道路中心線上のサンプル点。
 * heading: +X軸(East)から時計回りのラジアン（+Z = South 方向で増加）
 */
...
object RoadGeometry {
    ...
    fun computePath(waypoints: List<Location>, settings: RoadSettings): List<PathPoint> {
        ...
        for (i in 1 until waypoints.size - 1) {
            ...
            // 曲線区間（緩和曲線 + 円弧 + 緩和曲線）
            val curvePoints = sampleCurveElement(
                startX = csX, startY = csY, startZ = csZ,
                startHeading = hIn,
                alpha = alpha, R = R, Ls = Ls,
                waypoints = waypoints
            )
            ...
            if (curvePoints.isNotEmpty()) {
                ...
                prevX = last.x; prevY = last.y; prevZ = last.z
            } else {
                prevX = csX; prevY = csY; prevZ = csZ
            }
        }
    ...
    private fun sampleCurveElement(
        ...
        startHeading: Double,
        alpha: Double, R: Double, Ls: Double,
        ...
        var x = startX; var z = startZ; var h = startHeading
    ...
    private fun interpolateY(waypoints: List<Location>, px: Double, pz: Double): Double {
        ...
        for (i in 0 until waypoints.size - 1) {
            val w0 = waypoints[i]; val w1 = waypoints[i + 1]
    ...
    private fun projectOnSegment(
        px: Double, pz: Double,
        ax: Double, az: Double, bx: Double, bz: Double
    ): Double {
