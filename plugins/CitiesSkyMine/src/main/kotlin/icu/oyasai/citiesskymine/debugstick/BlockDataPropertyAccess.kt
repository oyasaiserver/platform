...
class BlockDataPropertyAccess {
    ...
    private fun propertiesFromNms(blockData: BlockData, parsed: ParsedBlockData): List<DebugBlockProperty>? {
        val state = invoke(blockData, "getState") ?: return null
        val rawProperties = invoke(state, "getProperties") as? Iterable<*> ?: return null
        val result = rawProperties.mapNotNull { rawProperty ->
            val property = rawProperty ?: return@mapNotNull null
            val name = invoke(property, "getName") as? String ?: return@mapNotNull null
            val possibleValues = invoke(property, "getPossibleValues") as? Iterable<*> ?: return@mapNotNull null
            val currentValue = invoke(state, "getValue", property)
            val values = possibleValues.mapNotNull { rawValue ->
                rawValue?.let { propertyValueName(property, it) }
            }
            val current = currentValue?.let { propertyValueName(property, it) }
    ...
    private fun propertiesFromString(blockData: BlockData, parsed: ParsedBlockData): List<DebugBlockProperty> =
    ...
    private fun commonValueCandidates(propertyName: String): List<String> {
        ...
        values += listOf(
            ...
            "left", "right", "single",
            "none", "side", "low", "tall",
            "floor", "wall", "ceiling", "single_wall", "double_wall",
            "head", "foot",
    ...
    companion object {
        private val primitiveWrappers = mapOf<Class<*>, Class<*>>(
