...
class HaussmannCommand(private val plugin: JavaPlugin) : CommandExecutor, TabCompleter {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (sender !is Player) { sender.sendMessage("§cプレイヤーのみ使用可能です"); return true }
    ...
    private fun doBuild(player: Player, args: Array<out String>) {
        ...
        if (es != null) { lastEdit[player.uniqueId] = es; player.sendMessage("§a生成完了！") }
    }
    ...
    private fun doSchem(player: Player, args: Array<out String>) {
        ...
        if (sub == "full") {
            ...
            if (es != null) { lastEdit[player.uniqueId] = es; player.sendMessage("§a貼り付け完了！") }
            return
        }
    ...
    private fun doUndo(player: Player) {
        val es = lastEdit.remove(player.uniqueId) ?: run {
            player.sendMessage("§cアンドゥする操作がありません"); return
        }
