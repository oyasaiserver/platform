...
object RoadBuilder {
     ...
     */
    fun build(player: Player, world: World, path: List<PathPoint>, settings: RoadSettings): EditSession {
    ...
    /**
     * 既存の EditSession に道路ブロックを追加する。
     * 交差点の腕など、複数パスを同一セッションで扱う場合に使用する。
     */
    internal fun buildInto(editSession: EditSession, path: List<PathPoint>, settings: RoadSettings) {
        ...
        for (slot in offsetSlots) {
            val trace = traceBlocksForOffset(path, slot.offset, avoidRightAngleCorners = slot.zone.isLine())
        ...
        // 3. 取り除いた角セルを埋める
        fillRemovedCornersWithNeighbors(pendingCornerFill, settings, hasLane, hasSidewalk, editSession, placed, occupancy)
    }
    ...
    private fun walkGrid(
        ...
        var tMaxX = if (stepX != 0) distanceToGridBoundary(startX, currentX, stepX) * invDx else Double.POSITIVE_INFINITY
        var tMaxZ = if (stepZ != 0) distanceToGridBoundary(startZ, currentZ, stepZ) * invDz else Double.POSITIVE_INFINITY
    ...
    private fun fillRemovedCornersWithNeighbors(
        ...
        for (request in pending) {
            for (corner in request.corners) {
                val zone = findNearestZoneForCorner(corner.pos, occupancy, request.slot.zone)
    ...
    private fun fallbackFillerZone(
        ...
        hasLane: Boolean,
        hasSidewalk: Boolean
    ): OffsetZone? = when (slotZone) {
        OffsetZone.CENTER_LINE -> when {
            hasLane -> OffsetZone.LANE
            hasSidewalk -> OffsetZone.SIDEWALK
            else -> null
        }
        OffsetZone.OUTER_LINE -> when {
            hasSidewalk -> OffsetZone.SIDEWALK
            hasLane -> OffsetZone.LANE
            else -> null
        }
    ...
    private fun OffsetZone.material(settings: RoadSettings) = when (this) {
        OffsetZone.CENTER_LINE, OffsetZone.OUTER_LINE -> settings.lineMaterial
    ...
    private fun OffsetZone.isLine() =
        this == OffsetZone.CENTER_LINE || this == OffsetZone.OUTER_LINE
