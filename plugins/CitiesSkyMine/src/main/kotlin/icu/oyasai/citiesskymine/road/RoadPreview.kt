...
object RoadPreview {
    ...
    /**
     * 現在フレームのパーティクルを一度だけ描画する（スケジューリングなし）。
     * Main の常時タスクから毎 10 tick 呼び出される。
     *
     * 白  = 道路中心線
     * 灰  = 車道端（外縁白線の外側）
     * 水色 = 歩道外縁
    ...
    fun showOnce(player: Player, path: List<PathPoint>, settings: RoadSettings) {
        ...
        val sidewalkEdge = (cHW + settings.laneWidth + settings.outerLineWidth + settings.sidewalkWidth).toDouble()

        // 3点に1点だけ描画してパーティクル数を抑える
        for ((i, pt) in path.withIndex()) {
            if (i % 3 != 0) continue
            val cx = pt.x; val cy = pt.y + 0.1; val cz = pt.z
            val perpX = -sin(pt.heading); val perpZ = cos(pt.heading)
