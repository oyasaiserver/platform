...
object WhiteLineSmoother {
    ...
    fun smooth(world: World, path: List<PathPoint>, settings: RoadSettings): SmoothResult {
        ...
        for (slot in slots) {
            ...
            for (i in traced.indices) {
                ...
                val orientation = zigzagOrientations[i]
                    ?: resolveOrientation(prevVec, nextVec, slot.offset, block.heading)
                val data = (settings.lineMaterial.createBlockData() as Stairs).apply {
    ...
    private fun orientCorner(prev: Vec, next: Vec, offset: Int): Orientation {
        ...
        val shape = if (prev.turnDirection(next) > 0) Stairs.Shape.INNER_LEFT else Stairs.Shape.INNER_RIGHT
    ...
    private enum class OffsetZone { CENTER_LINE, LANE, OUTER_LINE, SIDEWALK }
    ...
    private fun assignZigzagRun(
        ...
        var baseFace = enteringDiag?.let { enteringFace(it, dir) }
    ...
    private fun BlockFace.opposite(): BlockFace = when (this) {
