...
private object A {
    ...
    const val Y0 = 0; const val Y1 = 30
...
object SchematicFacadeGenerator {
    ...
    val patternIds: List<String> get() = A.PATTERNS.map { it.id }
    ...
    fun generate(player: Player, schemFile: File, bays: Int, patternId: String): EditSession? {
        ...
        for (i in 0 until bays) {
            ...
            pasteSlab(clip, es, origin, right, back,
                srcX0, srcX0 + pat.bayWidth - 1,
                A.Y0, A.Y1,
                A.DZ_FRONT, A.DZ_BACK,
    ...
    fun pasteFull(player: Player, schemFile: File): EditSession? {
        ...
        pasteSlab(clip, es, origin, right, back,
            0, 130, A.Y0, 38, A.DZ_FRONT, A.DZ_BACK, worldX = 0)
    ...
    fun undo(player: Player, lastEs: EditSession) {
        ...
        val undoEs = WorldEdit.getInstance().newEditSessionBuilder()
            .world(weWorld).actor(BukkitAdapter.adapt(player)).build()
        lastEs.undo(undoEs)
        undoEs.close()
    }
    ...
    private fun setup(player: Player): SetupResult? {
        val hit = player.rayTraceBlocks(20.0) ?: run {
            player.sendMessage("§cブロックを見てください"); return null
        }
        val block = hit.hitBlock ?: run { player.sendMessage("§cブロックなし"); return null }
    ...
    private fun pasteSlab(
        ...
        srcX0: Int, srcX1: Int,
        srcY0: Int, srcY1: Int,
        dzMin: Int, dzMax: Int,     // dzMin<0 = in front of wall, dzMax>0 = behind
    ...
    private fun load(file: File, player: Player): Clipboard? {
        ...
        val fmt = ClipboardFormats.findByFile(file) ?: run {
            player.sendMessage("§c対応フォーマット不明: ${file.name}"); return null
        }
        return try {
