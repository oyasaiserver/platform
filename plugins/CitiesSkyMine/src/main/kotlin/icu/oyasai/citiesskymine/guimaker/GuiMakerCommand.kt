...
class GuiMakerCommand(private val plugin: Main, private val engine: GuiEditorEngine) :
  ...
  override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
    if (sender !is Player) { sender.sendMessage("このコマンドはプレイヤーのみ使用できます。"); return true }
    if (!sender.isOp) { sender.sendMessage(comp("&cこのコマンドはOPのみ使用できます。")); return true }
  ...
  override fun onTabComplete(sender: CommandSender, command: Command, alias: String, args: Array<String>): List<String> {
    ...
    if (args[0].equals("edit", ignoreCase = true)) {
      if (args.size == 2) {
        return GuiMakerExporter.listMenuIds(plugin).filter { it.startsWith(args[1], ignoreCase = true) }
      }
      if (args.size == 3) {
    ...
    if (args[0].equals("ui", ignoreCase = true)) {
      if (args.size == 2) return listOf("edit", "list", "silent").filter { it.startsWith(args[1], ignoreCase = true) }
  ...
  private fun handleEdit(sender: Player, args: Array<String>) {
    ...
    val session = resolveSession(sender, menuId) ?: run {
    ...
    when (operation) {
      ...
      "name" -> {
        val slot = parseSlot(sender, session, args.getOrNull(3), "&c使用方法: /.gm edit $menuId name <slot> <text>") ?: return
      ...
      "lore" -> {
        val slot = parseSlot(sender, session, args.getOrNull(3), "&c使用方法: /.gm edit $menuId lore <slot> <text>") ?: return
        val text = args.drop(4).joinToString(" ")
        if (text.isBlank()) {
      ...
      "clearlore" -> {
        val slot = parseSlot(sender, session, args.getOrNull(3), "&c使用方法: /.gm edit $menuId clearlore <slot>") ?: return
        engine.cmdClearLore(sender, session, slot)
      }
      "perm" -> {
        val slot = parseSlot(sender, session, args.getOrNull(3), "&c使用方法: /.gm edit $menuId perm <slot> [permission]") ?: return
        engine.cmdSetPerm(sender, session, slot, args.getOrNull(4)?.takeIf { it.isNotBlank() })
      }
      "action" -> {
        val slot = parseSlot(sender, session, args.getOrNull(3), "&c使用方法: /.gm edit $menuId action <slot> <type> [value]") ?: return
        val type = args.getOrNull(4)?.uppercase() ?: run {
      ...
      "clearactions" -> {
        val slot = parseSlot(sender, session, args.getOrNull(3), "&c使用方法: /.gm edit $menuId clearactions <slot>") ?: return
        engine.cmdClearActions(sender, session, slot)
      }
      "clearslot" -> {
        val slot = parseSlot(sender, session, args.getOrNull(3), "&c使用方法: /.gm edit $menuId clearslot <slot>") ?: return
  ...
  private fun handleUi(sender: Player, args: Array<String>) {
    when (args.getOrNull(1)?.lowercase()) {
      "list" -> sender.sendMessage(comp("&e[GuiMaker] &f編集可能なUI画面: &a${UI_SCREENS.joinToString("&7, &a")}"))
      "edit" -> {
        val screen = args.getOrNull(2) ?: run {
          sender.sendMessage(comp("&c使用方法: /.gm ui edit <screen>  画面: ${UI_SCREENS.joinToString(", ")}"))
          return
        }
  ...
  private fun resolveSession(sender: Player, menuId: String): GuiEditorSession? {
    engine.getSession(sender)?.takeIf { it.menuId == menuId }?.let { return it }
  ...
  private fun printInfo(sender: Player, session: GuiEditorSession) {
    engine.rebuildSlotsFromInventory(session)
    val itemCount = session.canvasInventory?.contents?.count { it != null && it.type.name != "AIR" } ?: 0
  ...
  private fun validateMenuId(sender: Player, menuId: String): Boolean {
    val valid =
        menuId.matches(MENU_ID_PATTERN) &&
  ...
  private fun parseSlot(sender: Player, session: GuiEditorSession, raw: String?, usage: String): Int? {
