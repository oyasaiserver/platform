...
object FacadeGenerator {
  ...
  /**
   * Brisis (Y=27..31): steep slope, steps back 1 block every 2 rows. Lucarne (dormer): every other
   * bay, punched into brisis. Terrasson (Y=32..33): flat cap in lighter material.
   */
  ...
  /**
   * Lucarne (dormer window) centred at cx, starting at Y=27. 3 wide × 3 tall, protruding 1 block
   * from brisis face.
   */
   ...
   *
   * right = player's right when looking at the face (cross(viewDir, up)) back = into the wall
   * (opposite of face normal)
   */
