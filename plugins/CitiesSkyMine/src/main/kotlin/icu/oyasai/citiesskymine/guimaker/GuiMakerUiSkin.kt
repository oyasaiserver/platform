...
object GuiMakerUiSkinDefinitions {
  ...
  fun screenDefaultGlass(screenName: String): Material =
      when (screenName) {
        "context",
        "action_type" -> Material.BLUE_STAINED_GLASS_PANE
        "action_manage",
        "action_edit",
        "popup_pick" -> Material.PURPLE_STAINED_GLASS_PANE
        "lore_manage",
        "lore_edit",
        "favorites" -> Material.YELLOW_STAINED_GLASS_PANE
        "permission",
        "sound_category",
        "sound_pick",
        "color" -> Material.LIGHT_BLUE_STAINED_GLASS_PANE
...
class GuiMakerUiSkinStore(private val plugin: Main) {
  ...
  private fun file(screenName: String) = java.io.File(plugin.dataFolder, "ui-skin/$screenName.yml")
        ...
        runCatching {
              ...
              val icons =
                  yaml
                      ...
                      ?: yaml
                          ...
                          .toMap()
              val allLabels =
                  GuiMakerUiSkinDefinitions.screenDefs[screenName]
                      ?.values
                      ?.map { it.first }
                      ?.toSet()
                      .orEmpty()
              ...
              val enabledLabels =
                  if (loadedEnabledLabels != null &&
                      schema < GuiMakerUiSkinDefinitions.SCHEMA_VERSION) {
                    loadedEnabledLabels +
                        GuiMakerUiSkinDefinitions.legacyEnabledAdditions(screenName)
                  } else {
                    loadedEnabledLabels
                  }
