          ...
          export const composeSpec = z.object({ "version": z.string().describe("declared for backward compatibility, ignored. Please remove it.").optional(), "name": z.string().describe("define the Compose project name, until user defines one explicitly.").optional(), "include": z.array(z.any()).describe("compose sub-projects to be included.").optional(), "services": z.record(z.union([z.any(), z.never()])).superRefine((value, ctx) => {
for (const key in value) {
let evaluated = false
if (key.match(new RegExp("^[a-zA-Z0-9._-]+$"))) {
evaluated = true
...
}).describe("The services that will be used by your application.").optional(), "models": z.record(z.any()).superRefine((value, ctx) => {
for (const key in value) {
if (key.match(new RegExp("^[a-zA-Z0-9._-]+$"))) {
const result = z.any().safeParse(value[key])
if (!result.success) {
...
}).describe("Language models that will be used by your application.").optional(), "networks": z.record(z.any()).superRefine((value, ctx) => {
for (const key in value) {
if (key.match(new RegExp("^[a-zA-Z0-9._-]+$"))) {
const result = z.any().safeParse(value[key])
if (!result.success) {
...
}).describe("Networks that are shared among multiple services.").optional(), "volumes": z.record(z.union([z.any(), z.never()])).superRefine((value, ctx) => {
for (const key in value) {
let evaluated = false
if (key.match(new RegExp("^[a-zA-Z0-9._-]+$"))) {
evaluated = true
...
}).describe("Named volumes that are shared among multiple services.").optional(), "secrets": z.record(z.union([z.any(), z.never()])).superRefine((value, ctx) => {
for (const key in value) {
let evaluated = false
if (key.match(new RegExp("^[a-zA-Z0-9._-]+$"))) {
evaluated = true
...
}).describe("Secrets that are shared among multiple services.").optional(), "configs": z.record(z.union([z.any(), z.never()])).superRefine((value, ctx) => {
for (const key in value) {
let evaluated = false
if (key.match(new RegExp("^[a-zA-Z0-9._-]+$"))) {
evaluated = true
...
}).describe("Configurations that are shared among multiple services.").optional() }).catchall(z.union([z.any(), z.never()])).superRefine((value, ctx) => {
for (const key in value) {
let evaluated = ["version", "name", "include", "services", "models", "networks", "volumes", "secrets", "configs"].includes(key)
...
}).describe("The Compose file is a YAML file defining a multi-containers based application.") satisfies z.ZodType<ComposeSpec>
...
export interface ComposeSpec {
  ...
  /**
   * This interface was referenced by `ComposeSpec`'s JSON-Schema definition
   * via the `patternProperty` "^x-".
   */
  [k: string]: unknown;
}
