...
class Gradle2Nix :
  ...
  private val tasks: List<String> by
      option(
              "--task",
              "-t",
              metavar = "TASK",
          ...
          .split(",")
          .default(listOf(RESOLVE_ALL_TASK))

  private val artifacts: List<ArtifactType> by
      option(
              "--artifacts",
              "-a",
              metavar = "ARTIFACTS",
          ...
          .enum<ArtifactType>(key = { it.name.lowercase() })
          .split(",")
          .default(emptyList())

  private val projectDir: File by
      option(
              "--project",
              "-p",
          ...
          .convert { file ->
            getProjectRoot(file)
                ?: fail("Could not locate project root in \"$file\" or its parents.")
          }
          .defaultLazy("Current directory") { File(".") }

  private val outDir: File? by
      option(
              "--out-dir",
              "-o",
              metavar = "DIR",
  ...
  internal val lockFile: String by
      option(
              "--lock-file",
              "-l",
              metavar = "FILENAME",
  ...
  private val gradleSource: GradleSource by
      mutuallyExclusiveOptions(
              ...
              option(
                      ...
                      metavar = "DIR",
                      help =
                          "Gradle home path (e.g. \\`nix eval --raw nixpkgs#gradle.outPath\\`/lib/gradle)",
                  )
                  .file(mustExist = true, canBeFile = false)
                  .convert { GradleSource.Path(it) },
              option(
                      "--gradle-wrapper",
          ...
          .single()
          .default(GradleSource.Project)

  private val gradleJdk: File? by
      option(
              "--gradle-jdk",
              "-j",
              metavar = "DIR",
              help =
                  "JDK home to use for launching Gradle (e.g. \\`nix eval --raw nixpkgs#openjdk.home\\`)",
          )
          .file(canBeFile = false, canBeDir = true)

  private val logLevel: LogLevel by
      option(
              "--log",
  ...
  private val dumpEvents: Boolean by
      option(
              "--dump-events",
  ...
  private val stacktrace: Boolean by
      option(
              "--stacktrace",
  ...
  private val gradleArgs: List<String> by
      argument(
              name = "ARGS",
  ...
  override fun run() {
    ...
    val appHome =
        System.getProperty("org.nixos.gradle2nix.share")?.let(::File)
            ?: error(
                "Could not locate the /share directory in the gradle2nix installation: ${System.getenv("APP_HOME")}")
    ...
    outLockFile.outputStream().buffered().use { output -> JsonFormat.encodeToStream(env, output) }
  }
}
