  ...
  for (case in cases) {
    registerTestCase(
        NestedTest(
          ...
          if (!tempDir.resolve("settings.gradle").exists() &&
              !tempDir.resolve("settings.gradle.kts").exists()) {
          ...
          file.shouldBeAFile()
          val env: Env = file.inputStream().buffered().use { input -> Json.decodeFromStream(input) }
          test(tempDir, env)
        },
    )
  }
...
suspend fun TestScope.golden(
  ...
  fixture(project, *args) { dir, env ->
    ...
    if (updateGolden) {
      ...
      goldenFile.outputStream().buffered().use { output -> json.encodeToStream(env, output) }
    } else {
      ...
      val goldenData =
          try {
            ...
            fail(
                "Failed to load golden data from '$filename'. Run with --update-golden to regenerate.")
          }
...
object MavenRepo : MountableExtension<MavenRepo.Config, NettyApplicationEngine>, AfterSpecListener {
  ...
  init {
    require(config.repository.exists()) { "test repository doesn't exist: ${config.repository}" }
    val m2Url = Url(m2)
