...
val Project.reportableConfigurations: List<Configuration>
  get() =
      configurations.filter {
        (it as? DeprecatableConfiguration)?.canSafelyBeResolved() ?: it.isCanBeResolved
      }
...
abstract class AbstractResolveAllArtifactsApplier : ResolveAllArtifactsApplier {
  ...
  final override fun apply(gradle: Gradle) {
    ...
    val artifacts =
        System.getProperty(ARTIFACTS_PROPERTY, "")
            .split(",")
    ...
    gradle.allprojects { project ->
      val resolveProject =
          project.tasks.register(
              RESOLVE_PROJECT_TASK, ResolveProjectDependenciesTask::class.java) { task ->
                ...
                for (configuration in project.reportableConfigurations) {
                  ...
                  for (artifactType in artifacts) {
                    task.configurations.from(
                        project.addConfigurationArtifactResolver(configuration, artifactType))
                  }
                }
              }
    ...
    gradle.includedBuilds.forEach { includedBuild ->
      resolveAll.configure { it.dependsOn(includedBuild.task(":$RESOLVE_ALL_TASK")) }
    }
  }
}
...
constructor(
  ...
  @get:Internal val projectName = objects.property(String::class.java)

  @get:InputFiles val configurations = objects.fileCollection()
